...
rish -c "rm -rf '$ANDROID_BUNDLE'; mkdir -p '$ANDROID_BUNDLE'; cp -R '$LATEST_V1' '$ANDROID_BUNDLE/'; cp -R '$LATEST_V2' '$ANDROID_BUNDLE/'; cp -R '$LATEST_V3' '$ANDROID_BUNDLE/'; cp -R '$LATEST_V5' '$ANDROID_BUNDLE/'; rm -rf '$SHELL_STAGE'"
...
V5 student_decision.json = Cortex current-state projection under the same contract
V1 exit: $V1
V2 exit: $V2
V3 exit: $V3
V5 exit: $V5
...
echo; echo "DONE"; echo "Bundle folder: Downloads/$BUNDLE"; echo "ZIP: Downloads/$BUNDLE.zip"; echo "V1=$V1 V2=$V2 V3=$V3 V5=$V5"; echo; echo "Send $BUNDLE.zip back to ChatGPT. V5 must be adjudicated from cognitive_packet.json before student_decision.json is revealed/evaluated."
