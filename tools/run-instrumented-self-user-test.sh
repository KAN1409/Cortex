...
rish -c "pm install -r -t '$APP_APK'" >/dev/null
rish -c "pm install -r -t '$TEST_APK'" >/dev/null
...
if [[ ! -d "$HOME/storage/downloads" ]]; then
  echo "Termux storage link is missing. Run termux-setup-storage once."
  echo "The unzipped report is still available in Android Downloads as: $NAME"
  exit "$STATUS"
fi
