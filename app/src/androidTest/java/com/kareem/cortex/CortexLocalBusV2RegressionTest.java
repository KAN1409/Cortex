...
public final class CortexLocalBusV2RegressionTest {
    @Test public void negotiatesOnlyWhenSignalV2IsAdvertised() throws Exception {
        assertTrue(CortexLocalBusProtocolV2.relayAdvertisesSignalV2(new JSONArray().put("NOTIFICATIONS").put("CORTEX_SIGNAL_V2").toString()));
        assertFalse(CortexLocalBusProtocolV2.relayAdvertisesSignalV2(new JSONArray().put("NOTIFICATIONS").toString()));
    ...
    @Test public void adaptsV2ToCanonicalV1WithoutChangingEventIdentity() throws Exception {
        ...
        semantic.put("content", new JSONObject().put("title", "Alice").put("text", "hello")
                .put("expanded_text", JSONObject.NULL).put("conversation_title", "Alice")
                .put("messages", new JSONArray().put(new JSONObject().put("sender", "Alice").put("text", "hello").put("timestamp", now))));

        JSONArray actions = new JSONArray().put(new JSONObject().put("capability_id", "action_1").put("kind", "REPLY").put("requires_text_input", true));
        ...
        root.put("source", new JSONObject().put("type", "NOTIFICATION").put("package", "com.whatsapp").put("notification_key", "0|com.whatsapp|42|null|123"));
        root.put("semantic", semantic);
        root.put("action_capabilities", actions);
        root.put("compatibility", new JSONObject().put("v1_protocol", "CORTEX_INGEST_V1").put("v1_event_id", "sb_evt_v2_1"));
        ...
        assertEquals("signal-message-delta_abc", meta.getJSONObject("relay_semantic_v2").getString("logical_signal_id"));
        assertEquals("action_1", meta.getJSONArray("relay_action_capabilities_v1").getJSONObject(0).getString("capability_id"));
