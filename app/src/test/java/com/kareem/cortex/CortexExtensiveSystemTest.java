...
public class CortexExtensiveSystemTest {
    @Test public void cortexExtensiveSystemContract() throws Exception {
        ...
        try{
            Cursor quick=db.getReadableDatabase().rawQuery("PRAGMA quick_check(1)",null);assertTrue(quick.moveToFirst());assertEquals("ok",quick.getString(0).toLowerCase());quick.close();
            CognitiveStore.ensure(db);StatefulMeaningStore.ensure(db.getWritableDatabase());CommitmentLifecycleStore.ensure(db.getWritableDatabase());CognitiveShadowStore.ensure(db.getWritableDatabase());
            ...
            assertEquals(43,CortexCapabilityRegistry.all().size());Set<String> keys=new HashSet<>();
            for(CortexCapabilityRegistry.Capability capability:CortexCapabilityRegistry.all()){
                ...
                assertNotNull(state);assertNotNull(state.status);assertFalse("capability has no status: "+capability.key,state.status.trim().isEmpty());assertNotEquals("capability evaluator missing: "+capability.key,CortexCapabilityRegistry.NOT_VERIFIED,state.status);
            }
            ...
            assertEquals("ACTION",request.attentionKind);assertEquals("action_request",request.type);
            DeterministicSemanticRecovery.Classification security=DeterministicSemanticRecovery.classify("notification","Google","Critical security alert for saved passwords");
            assertEquals("ACTION",security.attentionKind);assertEquals("security_alert",security.type);
            ...
            assertNull(ordinary.attentionKind);assertEquals("conversation_message",ordinary.type);assertTrue(ordinary.confidence>=.75);

            // A bank/service future announcement is information, not something Cortex is waiting on.
            ...
            String trace=AttentionTraceExporter.export(db.getReadableDatabase());assertTrue(trace.contains("CORTEX_ATTENTION_TRACE_V4"));assertTrue(trace.contains("commitment_count"));assertTrue(trace.contains("overdue_commitment_count"));
            assertEquals("application/json",DiagnosticFileShare.mimeType("cortex-attention-trace.json"));assertEquals("text/plain",DiagnosticFileShare.mimeType("cortex-java-crash.txt"));
        }finally{db.close();}
    }
}
