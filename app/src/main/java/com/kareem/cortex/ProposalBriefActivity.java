...
public final class ProposalBriefActivity extends CortexOrbBriefActivity {
    ...
    @Override void derivedSection(String title,int color,List<PrimeBriefStore.Item> xs,String glyph,int limit){
        ...
        for(int i=0;i<n;i++){
            ...
            View rail=new View(this);rail.setBackground(CortexUi.round(this,color,Color.TRANSPARENT,999));LinearLayout.LayoutParams rp=new LinearLayout.LayoutParams(dp(2),dp(54));rp.setMargins(0,0,dp(10),0);main.addView(rail,rp);
            ...
            String result=tt+(body.isEmpty()?"":"\n"+body)+"\nAttention: "+x.attentionBand+" · "+x.attentionScore+"/100\nWhy now: "+x.whyNow+"\nState: "+x.state+" · confidence "+Math.round(x.confidence*100)+"%";LinearLayout host=new LinearLayout(this);host.setOrientation(LinearLayout.VERTICAL);host.setPadding(dp(13),0,dp(11),dp(5));card.addView(host);ProposalUi.attach(this,db,host,new ResultProposalEngine.Target("Today / "+title,"today_"+x.kind+"_"+x.id,tt,result,evidenceId,sourceType,cloud));
            card.addView(attentionActions(x),new LinearLayout.LayoutParams(-1,dp(48)));
    ...
    View attentionActions(PrimeBriefStore.Item x){
        LinearLayout row=new LinearLayout(this);row.setGravity(Gravity.CENTER_VERTICAL);row.setPadding(dp(10),0,dp(10),dp(8));
        TextView why=miniAction("WHY NOW",CortexUi.MUTED),done=miniAction("DONE",CortexUi.GREEN),snooze=miniAction("SNOOZE",CortexUi.ORANGE),dismiss=miniAction("NOT IMPORTANT",CortexUi.FAINT);
        why.setOnClickListener(v->showWhyNow(x));done.setOnClickListener(v->{if(resolveDerived(x.id)){AttentionLearning.record(db,x.id,"acted");try{CognitiveStore.feedback(db,"derived",x.id,"resolved_by_user","{}",AttentionLearning.VERSION);}catch(Throwable ignored){}refreshAsync();}});snooze.setOnClickListener(v->{AttentionLearning.snooze(db,x.id,System.currentTimeMillis()+3L*60L*60L*1000L);Toast.makeText(this,"Snoozed for 3 hours",Toast.LENGTH_SHORT).show();refreshAsync();});dismiss.setOnClickListener(v->{AttentionLearning.record(db,x.id,"dismissed");Toast.makeText(this,"Hidden until new evidence changes it",Toast.LENGTH_SHORT).show();refreshAsync();});
        row.addView(why,new LinearLayout.LayoutParams(0,dp(40),1));row.addView(done,new LinearLayout.LayoutParams(0,dp(40),1));row.addView(snooze,new LinearLayout.LayoutParams(0,dp(40),1));row.addView(dismiss,new LinearLayout.LayoutParams(0,dp(40),1));return row;
    }

    TextView miniAction(String text,int color){TextView t=CortexUi.plain(this,text,8,color);CortexUi.medium(t);t.setGravity(Gravity.CENTER);t.setBackground(CortexUi.round(this,Color.TRANSPARENT,Color.argb(30,255,255,255),12));LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(0,dp(40),1);p.setMargins(dp(2),0,dp(2),0);t.setLayoutParams(p);return t;}
    void showWhyNow(PrimeBriefStore.Item x){AttentionLearning.record(db,x.id,"opened");new android.app.AlertDialog.Builder(this).setTitle("Why now").setMessage(x.attentionBand+" · "+x.attentionScore+"/100\n\n"+x.whyNow+"\n\nRelevance decides what this item is. Attention decides whether it deserves space in Today right now.").setPositiveButton("OK",null).show();}
