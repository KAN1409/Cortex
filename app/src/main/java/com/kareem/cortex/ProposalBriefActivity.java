...
public final class ProposalBriefActivity extends CortexOrbBriefActivity {
    ...
    @Override void render(PrimeBriefStore.Snapshot s){
        ...
        if(!s.reviews.isEmpty())reviewSection(s.reviews,3);
        if(s.empty()){
    ...
    /** The Brief hero is the user's cognitive situation, not an input modality. */
    @Override View signalCard(PrimeBriefStore.Snapshot s){
        ContextPacketBuilder.Packet p=null;try{p=ContextPacketBuilder.buildLocal(db,280);}catch(Throwable ignored){}boolean has=p!=null&&p.available();int color=has?(p.confidence>=.82?CortexUi.GREEN:CortexUi.ORANGE):CortexUi.RED;
        LinearLayout card=CortexUi.card(this,22);card.setPadding(0,0,0,0);LinearLayout core=new LinearLayout(this);core.setGravity(Gravity.CENTER_VERTICAL);core.setPadding(dp(14),dp(14),dp(13),dp(14));
        View rail=new View(this);rail.setBackground(CortexUi.round(this,color,Color.TRANSPARENT,999));LinearLayout.LayoutParams rp=new LinearLayout.LayoutParams(dp(2),dp(64));rp.setMargins(0,0,dp(12),0);core.addView(rail,rp);
        core.addView(CortexUi.glyph(this,has?"brain":"wave",color,true),new LinearLayout.LayoutParams(dp(52),dp(52)));
        ...
        TextView h=CortexUi.plain(this,has?"CURRENT CONTEXT":"CURRENT SIGNALS",10,color);CortexUi.medium(h);if(android.os.Build.VERSION.SDK_INT>=21)h.setLetterSpacing(.09f);text.addView(h);
        int attention=s.actions.size()+s.waiting.size()+s.decisions.size();String main=has?p.title:(attention>0?"Cortex has "+attention+" current signal"+(attention==1?"":"s")+" that may need you.":"Cortex is listening for the next meaningful context.");TextView b=CortexUi.text(this,main,14,CortexUi.TEXT);CortexUi.medium(b);b.setMaxLines(2);b.setPadding(0,dp(4),0,0);text.addView(b);
        String detail="";if(has){if(!p.currentActivity.isEmpty()&&!LocalSemanticEmbedder.norm(p.currentActivity).equals(LocalSemanticEmbedder.norm(p.title)))detail=clipLocal(p.currentActivity,105);else if(!p.goal.isEmpty())detail=clipLocal(p.goal,105);if(!p.nextStep.isEmpty())detail+=(detail.isEmpty()?"":"  •  ")+"Next: "+clipLocal(p.nextStep,80);}TextView meta=CortexUi.plain(this,has?(detail.isEmpty()?"LIVE  •  CONTEXT  •  LOCAL":detail):"LIVE  •  CONTEXT  •  CURRENT",9,CortexUi.MUTED);meta.setMaxLines(2);meta.setPadding(0,dp(6),0,0);text.addView(meta);
        core.addView(statusChip(has?"● ACTIVE":"● LIVE",color),new LinearLayout.LayoutParams(-2,dp(34)));card.addView(core);
        if(has){card.setOnClickListener(v->{try{startActivity(new Intent(this,ContextNowActivity.class));}catch(Throwable ignored){CortexActionExecutor.openBrain(this,0,"What am I doing now, where did I leave off, and what should I continue next?");}});card.setOnLongClickListener(v->{CortexActionExecutor.openBrain(this,0,"Show my current context, open loop, background contexts, and the best next step.");return true;});}else{card.setOnClickListener(v->showComposed(false));card.setOnLongClickListener(v->{showComposed(true);return true;});}return card;
    }
    ...
    void reviewSection(List<ReviewQueueStore.Item> xs,int limit){
