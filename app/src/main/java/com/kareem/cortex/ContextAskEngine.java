...
public final class ContextAskEngine {
    ...
    public static GroundedAnswer tryAnswer(VaultDb db,String question){
        if(db==null||!matches(question))return null;ContextPacketBuilder.Packet p=ContextPacketBuilder.buildLocal(db,420);if(!p.available())return null;StringBuilder a=new StringBuilder();a.append("You’re currently in: ").append(p.title).append('.');if(!p.goal.isEmpty())a.append("\n\nGoal: ").append(p.goal);if(!p.currentActivity.isEmpty())a.append("\n\nCurrent activity: ").append(p.currentActivity);if(!p.openLoops.isEmpty())a.append("\n\nOpen loop: ").append(p.openLoops);if(!p.nextStep.isEmpty())a.append("\n\nNext: ").append(p.nextStep);if(!p.background.isEmpty()){a.append("\n\nBackground / suspended: ");for(int i=0;i<p.background.size()&&i<3;i++){if(i>0)a.append(" · ");a.append(p.background.get(i));}}return new GroundedAnswer(question,a.toString(),Math.max(.72,p.confidence),new ArrayList<>(),lines(p.openLoops),new ArrayList<>());
    }
