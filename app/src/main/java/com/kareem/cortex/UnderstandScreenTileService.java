...
public final class UnderstandScreenTileService extends TileService {
    ...
    @Override public void onClick(){super.onClick();CortexScreenAccessibilityService.Snapshot s=CortexScreenAccessibilityService.snapshot();if(s==null||!s.usable()){Toast.makeText(this,"Enable Cortex Screen Understanding first",Toast.LENGTH_SHORT).show();try{Intent i=new Intent(Settings.ACTION_ACCESSIBILITY_SETTINGS);i.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);startLegacyAndCollapse(i);}catch(Throwable ignored){}return;}new Thread(()->save(s),"CortexUnderstandScreen").start();}
    ...
    private void openResult(long id){android.os.Handler h=new android.os.Handler(getMainLooper());h.post(()->{try{Intent i=new Intent(this,ProposalCaptureResultActivity.class);i.putExtra("item_id",id);i.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK|Intent.FLAG_ACTIVITY_CLEAR_TOP);if(Build.VERSION.SDK_INT>=34){PendingIntent p=PendingIntent.getActivity(this,(int)(42000+(id%1000)),i,PendingIntent.FLAG_UPDATE_CURRENT|PendingIntent.FLAG_IMMUTABLE);startActivityAndCollapse(p);}else startLegacyAndCollapse(i);}catch(Throwable e){toast("Screen captured. Open Brief to see the result.");}});}
