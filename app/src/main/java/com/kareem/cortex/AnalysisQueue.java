...
public final class AnalysisQueue {
    ...
    private static void next(Context ctx,VaultDb db,Runnable changed){
        ...
        if("SCREENSHOT".equals(item.type)||"IMAGE".equals(item.type)){
            OcrAnalyzer.analyze(ctx,item,new OcrAnalyzer.Callback(){public void ok(AnalysisResult r){finish(db,item,r,changed);next(ctx,db,changed);}public void fail(Exception e){AnalysisQueue.fail(db,item.id,e,changed);next(ctx,db,changed);}});
        }else if("AUDIO".equals(item.type)){
            AudioAnalyzer.analyze(ctx,item,new AudioAnalyzer.Callback(){public void ok(AnalysisResult r){db.applyAnalysis(item.id,r);AudioStore.save(db,item.id,r);post(db,item,r);if(changed!=null)changed.run();next(ctx,db,changed);}public void fail(Exception e){AnalysisQueue.fail(db,item.id,e,changed);next(ctx,db,changed);}});
        }else if("FILE".equals(item.type)){
            try{finish(db,item,AttachmentAnalyzer.analyze(item),changed);}catch(Exception e){AnalysisQueue.fail(db,item.id,e,changed);}next(ctx,db,changed);
        }else{
            try{
                AnalysisResult r=LocalAnalyzer.analyze(item.rawText,"text/plain");
                guardPassiveSources(db,item,r);
                db.applyAnalysis(item.id,r);post(db,item,r);
    ...
    private static void finish(VaultDb db,KnowledgeItem item,AnalysisResult r,Runnable changed){db.applyAnalysis(item.id,r);post(db,item,r);if(changed!=null)changed.run();}
    private static void post(VaultDb db,KnowledgeItem item,AnalysisResult r){try{TemporalResolver.afterAnalysis(db,item.id);}catch(Exception ignored){}try{CoreBrainEngine.afterAnalysis(db,item.id);}catch(Exception ignored){}try{IntentionalCognitiveBridge.afterAnalysis(db,item,r);}catch(Exception ignored){}}
