...
public final class ReviewQueueStore {
    public static final String POLICY="review_queue_005";
    ...
    public static final class Item {
        public final long id,threadId,signalId,createdAt;public final String candidateKind,title,body,reason,state,sourceKey,semanticKey;public final double confidence;public final int importance;
        Item(long id,String candidateKind,String title,String body,String reason,String state,String sourceKey,String semanticKey,double confidence,int importance,long threadId,long signalId,long createdAt){this.id=id;this.candidateKind=n(candidateKind);this.title=n(title);this.body=n(body);this.reason=n(reason);this.state=n(state);this.sourceKey=n(sourceKey);this.semanticKey=n(semanticKey);this.confidence=confidence;this.importance=importance;this.threadId=threadId;this.signalId=signalId;this.createdAt=createdAt;}
    }

    public static long enqueue(VaultDb db,String candidateKind,String title,String body,double confidence,int importance,long threadId,long signalId,String reason,String source){return enqueue(db,candidateKind,title,body,confidence,importance,threadId,signalId,reason,source,body);}
    public static long enqueue(VaultDb db,String candidateKind,String title,String body,double confidence,int importance,long threadId,long signalId,String reason,String source,String semanticEvidence){
        String candidate=validCandidate(candidateKind);if(candidate.isEmpty())return 0;CognitiveStore.ensure(db);expireStale(db);String sourceKey=n(source),semanticKey=DerivedSemanticIdentity.key(candidate,semanticEvidence);
        Item existing=pendingForThreadCandidate(db,threadId,candidate,sourceKey,semanticKey);if(existing!=null){if(!refreshExisting(db,existing,body,confidence,importance,signalId,reason,sourceKey,semanticKey))return 0;return existing.id;}
        try{
            JSONObject meta=new JSONObject();meta.put("candidate_kind",candidate);meta.put("reason",n(reason));meta.put("policy_version",POLICY);meta.put("thread_id",threadId);meta.put("raw_signal_id",signalId);meta.put("source",sourceKey);meta.put("original_confidence",confidence);meta.put("semantic_key",semanticKey);
            String identity=semanticKey.isEmpty()?String.valueOf(signalId):semanticKey;String fp=Fingerprint.text("review|"+candidate+"|"+threadId+"|"+identity);long reviewId=CognitiveStore.addDerived(db,CognitiveTypes.DerivedKind.REVIEW,reviewTitle(title,candidate),body,"pending",confidence,importance,fp,meta.toString());
            if(reviewId<=0)return 0;
            if(!CognitiveStore.setDerivedRoutingChecked(db,reviewId,sourceKey,threadId,signalId,candidate,semanticKey))return 0;
            if(threadId>0&&!CognitiveStore.linkChecked(db,CognitiveTypes.ObjectType.THREAD,threadId,CognitiveTypes.ObjectType.DERIVED,reviewId,"needs_review",confidence,meta.toString()))return 0;
            if(signalId>0&&!CognitiveStore.linkChecked(db,CognitiveTypes.ObjectType.RAW_SIGNAL,signalId,CognitiveTypes.ObjectType.DERIVED,reviewId,CognitiveTypes.Relation.SUPPORTS,1.0,""))return 0;
            return reviewId;
    ...
    public static long confirm(VaultDb db,long reviewId){
        ...
        try{JSONObject meta=new JSONObject();meta.put("confirmed_from_review",reviewId);meta.put("review_policy",POLICY);meta.put("thread_id",item.threadId);meta.put("raw_signal_id",item.signalId);meta.put("source",item.sourceKey);meta.put("semantic_key",item.semanticKey);meta.put("user_confirmed",true);String fp=Fingerprint.text("review-confirmed|"+reviewId+"|"+candidate);long derived=CognitiveStore.addDerived(db,candidate,confirmedTitle(item.title,candidate),item.body,"open",Math.max(0.90,item.confidence),Math.max(55,item.importance),fp,meta.toString());if(derived<=0)return 0;if(!CognitiveStore.setDerivedRoutingChecked(db,derived,item.sourceKey,item.threadId,item.signalId,candidate,item.semanticKey))return 0;if(!linkConfirmed(db,item,reviewId,derived,meta.toString()))return 0;if(!resolve(db,reviewId,"confirmed"))return 0;CognitiveStore.feedback(db,CognitiveTypes.ObjectType.DERIVED,reviewId,"confirm","{\"candidate_kind\":\""+candidate+"\"}",POLICY);RelevanceEvaluationStore.userVerdict(db,item.signalId,"confirm",candidate,reviewId);return derived;}catch(Exception e){DiagnosticsLog.error(db,"ReviewQueueStore","confirm",e,"REVIEW_CONFIRM",0,item.threadId,item.signalId,0,0,null);return 0;}
    }
    ...
    private static boolean refreshExisting(VaultDb db,Item existing,String body,double confidence,int importance,long signalId,String reason,String source,String semanticKey){
        try{JSONObject meta=new JSONObject();meta.put("candidate_kind",existing.candidateKind);meta.put("reason",n(reason));meta.put("policy_version",POLICY);meta.put("thread_id",existing.threadId);meta.put("raw_signal_id",signalId);meta.put("source",n(source));meta.put("semantic_key",n(semanticKey));meta.put("original_confidence",Math.max(existing.confidence,confidence));ContentValues v=new ContentValues();v.put("body",n(body));v.put("confidence",Math.max(existing.confidence,confidence));v.put("importance",Math.max(existing.importance,importance));v.put("metadata_json",meta.toString());v.put("source_key",n(source));v.put("thread_id",existing.threadId);v.put("anchor_signal_id",signalId);v.put("candidate_kind",existing.candidateKind);v.put("semantic_key",n(semanticKey));v.put("updated_at",System.currentTimeMillis());if(db.getWritableDatabase().update("derived_items",v,"id=? AND kind='REVIEW' AND state='pending'",new String[]{String.valueOf(existing.id)})<=0)return false;if(signalId>0&&!CognitiveStore.linkChecked(db,CognitiveTypes.ObjectType.RAW_SIGNAL,signalId,CognitiveTypes.ObjectType.DERIVED,existing.id,CognitiveTypes.Relation.SUPPORTS,1.0,""))return false;return true;}catch(Exception e){DiagnosticsLog.error(db,"ReviewQueueStore","refresh_existing",e,"REVIEW_REFRESH",0,existing.threadId,signalId,0,0,null);return false;}
    }

    private static ArrayList<Item> pendingRaw(VaultDb db,int limit,String extraWhere,String[] extraArgs){ArrayList<Item> out=new ArrayList<>();String where="kind='REVIEW' AND state='pending'"+(extraWhere==null||"1=1".equals(extraWhere)?"":" AND ("+extraWhere+")");Cursor c=db.getReadableDatabase().query("derived_items",new String[]{"id","title","body","state","confidence","importance","metadata_json","created_at","source_key","thread_id","anchor_signal_id","candidate_kind","semantic_key"},where,extraArgs,null,null,"importance DESC, updated_at DESC",String.valueOf(Math.max(1,Math.min(100,limit))));while(c.moveToNext())out.add(from(c));c.close();return out;}
    private static boolean linkConfirmed(VaultDb db,Item item,long reviewId,long derived,String meta){if(!CognitiveStore.linkChecked(db,CognitiveTypes.ObjectType.DERIVED,reviewId,CognitiveTypes.ObjectType.DERIVED,derived,"confirmed_as",1.0,meta))return false;if(item.threadId>0&&!CognitiveStore.linkChecked(db,CognitiveTypes.ObjectType.THREAD,item.threadId,CognitiveTypes.ObjectType.DERIVED,derived,"produced",1.0,""))return false;if(item.signalId>0&&!CognitiveStore.linkChecked(db,CognitiveTypes.ObjectType.RAW_SIGNAL,item.signalId,CognitiveTypes.ObjectType.DERIVED,derived,CognitiveTypes.Relation.SUPPORTS,1.0,""))return false;return true;}
    private static boolean resolveWithFeedback(VaultDb db,long id,String state,String event,String value){Item x=get(db,id);if(x==null||!"pending".equals(x.state))return false;if(!resolve(db,id,state))return false;CognitiveStore.feedback(db,CognitiveTypes.ObjectType.DERIVED,id,event,value,POLICY);RelevanceEvaluationStore.userVerdict(db,x.signalId,event,x.candidateKind,id);return true;}
    private static boolean resolve(VaultDb db,long id,String state){long now=System.currentTimeMillis();ContentValues v=new ContentValues();v.put("state",state);v.put("resolved_at",now);v.put("updated_at",now);return db.getWritableDatabase().update("derived_items",v,"id=? AND kind='REVIEW' AND state='pending'",new String[]{String.valueOf(id)})>0;}

    private static Item get(VaultDb db,long id){CognitiveStore.ensure(db);expireStale(db);Cursor c=db.getReadableDatabase().query("derived_items",new String[]{"id","title","body","state","confidence","importance","metadata_json","created_at","source_key","thread_id","anchor_signal_id","candidate_kind","semantic_key"},"id=? AND kind='REVIEW'",new String[]{String.valueOf(id)},null,null,null,"1");Item x=c.moveToFirst()?from(c):null;c.close();return x;}
    private static Item from(Cursor c){long id=c.getLong(0),created=c.getLong(7),thread=c.getLong(9),signal=c.getLong(10);String title=n(c.getString(1)),body=n(c.getString(2)),state=n(c.getString(3)),meta=n(c.getString(6)),source=n(c.getString(8)),candidate=n(c.getString(11)),semantic=n(c.getString(12)),reason="";double confidence=c.getDouble(4);int importance=c.getInt(5);try{JSONObject o=new JSONObject(meta);reason=o.optString("reason","");if(candidate.isEmpty())candidate=o.optString("candidate_kind","");if(source.isEmpty())source=o.optString("source","");if(semantic.isEmpty())semantic=o.optString("semantic_key","");if(thread<=0)thread=o.optLong("thread_id",0);if(signal<=0)signal=o.optLong("raw_signal_id",0);}catch(Exception ignored){}return new Item(id,candidate,title,body,reason,state,source,semantic,confidence,importance,thread,signal,created);}
