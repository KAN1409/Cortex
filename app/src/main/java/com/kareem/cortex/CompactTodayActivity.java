...
public final class CompactTodayActivity extends CortexOrbBriefActivity {
    ...
    @Override void systemHeader(){
        ...
        CortexGlyphView settings=CortexUi.glyph(this,"settings",CortexUi.RED,false);settings.setOnClickListener(v->{try{startActivity(new Intent(this,SettingsActivity.class));}catch(Throwable ignored){}});row.addView(settings,new LinearLayout.LayoutParams(dp(42),dp(42)));content.addView(row);
    }
    ...
    @Override View audioCard(PrimeBriefStore.Snapshot s){
        ...
        TextView meta=CortexUi.plain(this,"Voice context  •  "+ageLocal(k.createdAt)+"  •  tap to play",9,CortexUi.MUTED);meta.setPadding(0,dp(3),0,0);text.addView(meta);TextView open=CortexUi.plain(this,"OPEN",8,CortexUi.RED);open.setGravity(Gravity.CENTER);card.addView(open,new LinearLayout.LayoutParams(dp(48),dp(32)));card.setOnClickListener(v->showVoicePlayer(s));return card;
    }
    ...
    @Override void derivedSection(String title,int color,List<PrimeBriefStore.Item> xs,String glyph,int limit){
        sectionTitle(title,color);int n=Math.min(limit,xs.size());for(int i=0;i<n;i++){
            ...
            LinearLayout main=new LinearLayout(this);main.setGravity(Gravity.CENTER_VERTICAL);main.setPadding(dp(12),dp(11),dp(12),dp(8));View rail=new View(this);rail.setBackground(CortexUi.round(this,color,Color.TRANSPARENT,999));LinearLayout.LayoutParams rp=new LinearLayout.LayoutParams(dp(3),dp(58));rp.setMargins(0,0,dp(11),0);main.addView(rail,rp);
            LinearLayout txt=new LinearLayout(this);txt.setOrientation(LinearLayout.VERTICAL);main.addView(txt,new LinearLayout.LayoutParams(0,-2,1));TextView h=CortexUi.text(this,clipLocal(tt,110),15,CortexUi.TEXT);CortexUi.medium(h);h.setMaxLines(3);txt.addView(h);if(!body.isEmpty()){TextView m=CortexUi.text(this,clipLocal(body,145),11,CortexUi.MUTED);m.setMaxLines(2);m.setPadding(0,dp(4),0,0);txt.addView(m);}TextView why=CortexUi.plain(this,x.attentionBand+"  •  "+x.attentionScore+"  •  "+clipLocal(x.whyNow,120),9,CortexUi.FAINT);why.setMaxLines(2);why.setPadding(0,dp(5),0,0);txt.addView(why);card.addView(main);main.setOnClickListener(v->{AttentionLearning.record(db,x.id,"opened");derivedDetail(x);});card.addView(compactActions(x),new LinearLayout.LayoutParams(-1,dp(38)));LinearLayout.LayoutParams cp=new LinearLayout.LayoutParams(-1,-2);if(i>0)cp.setMargins(0,dp(7),0,0);content.addView(card,cp);
    ...
    void reviewRow(int count){sectionTitle("NEEDS REVIEW",CortexUi.YELLOW);TextView row=CortexUi.text(this,count+" item"+(count==1?"":"s")+" need your judgement",12,CortexUi.TEXT);row.setGravity(Gravity.CENTER_VERTICAL);row.setPadding(dp(12),dp(10),dp(12),dp(10));row.setBackground(CortexUi.round(this,CortexUi.SURFACE,Color.argb(24,255,255,255),16));row.setOnClickListener(v->{try{startActivity(new Intent(this,ReviewQueueActivity.class));}catch(Throwable ignored){}});content.addView(row,new LinearLayout.LayoutParams(-1,dp(48)));}
