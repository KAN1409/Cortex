...
public final class CompactTodayActivity extends CortexOrbBriefActivity {
    ...
    @Override void systemHeader(){
        ...
        TextView hb=CortexUi.text(this,"Current situations synthesized from Relay, memory and your captures — old state retires when newer evidence changes the truth.",12,CortexUi.MUTED);hb.setPadding(0,dp(5),0,0);hero.addView(hb);
    ...
    @Override void render(PrimeBriefStore.Snapshot s){
        ...
        if(!audioItems.isEmpty()){sectionTitle("RECENT CAPTURE",CortexUi.MUTED);content.addView(audioCard(s),margins(0,0,0,0));}
        if(s.attentionEmpty()){
    ...
    private View brainStatusCard(){
        JSONObject d=CortexRelayBridgeV2.diagnosticSnapshot(this);boolean relay=d.optBoolean("connected",false);boolean model=LocalModelManager.installed(this);CognitiveAuthorityMode mode=CognitiveFeatureFlags.authorityMode(this);
        ...
        View dot=new View(this);int color=relay&&model&&mode==CognitiveAuthorityMode.V2_PRIMARY?CortexUi.BRAND:CortexUi.ORANGE;dot.setBackground(CortexUi.round(this,color,Color.TRANSPARENT,999));LinearLayout.LayoutParams dpv=new LinearLayout.LayoutParams(dp(7),dp(7));dpv.setMargins(0,0,dp(9),0);row.addView(dot,dpv);
        String title=relay?"Relay connected":"Relay waiting";TextView h=CortexUi.plain(this,title,11,CortexUi.TEXT);CortexUi.medium(h);row.addView(h,new LinearLayout.LayoutParams(0,-2,1));String brain=model?(mode==CognitiveAuthorityMode.V2_PRIMARY?"QWEN · PRIMARY":"QWEN · "+mode.name()):"MODEL NOT READY";TextView chip=CortexUi.chip(this,brain,color,false);row.addView(chip,new LinearLayout.LayoutParams(-2,dp(27)));card.addView(row);
        TextView sub=CortexUi.plain(this,"Evidence stays in the background. Only unresolved situations can appear on Now.",9,CortexUi.MUTED);sub.setPadding(dp(16),dp(4),0,0);card.addView(sub);return card;
    }
    ...
    private String friendlyTiming(PrimeBriefStore.Item x){String band=x.attentionBand==null?"":x.attentionBand.name();if("NOW".equals(band))return"Needs attention";if("WATCHING".equals(band))return"Waiting for a change";if("LATER".equals(band))return"Keep in view";return"Relevant context";}
