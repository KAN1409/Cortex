...
public class ShareImporter {
    ...
    private long saveAudio(Uri uri,String mime){String name=displayName(uri,"audio");String local=copy(uri,name);if(local.isEmpty())return 0;long id=db.insert("AUDIO","android_share",name,"","Voice & Audio","voice,audio,transcript",local,Fingerprint.file(local),fileMeta(local,mime,name));if(id<0)new File(local).delete();return id;}
    ...
    private String fileMeta(String path,String mime,String name){try{JSONObject j=new JSONObject();j.put("mime",mime==null?"":mime);j.put("name",name);j.put("bytes",new File(path).length());return j.toString();}catch(Exception e){return "{}";}}
