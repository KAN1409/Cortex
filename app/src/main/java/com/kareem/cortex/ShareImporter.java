...
public class ShareImporter {
    ...
    public long importSingle(Intent i){if(i==null||!Intent.ACTION_SEND.equals(i.getAction()))return 0;String mime=i.getType();Uri u=readUri(i);if(mime!=null&&mime.startsWith("image/")&&u!=null)return saveImage(u,mime);if(mime!=null&&mime.startsWith("audio/")&&u!=null)return saveAudio(u,mime);if(u!=null)return saveFile(u,mime);String text=i.getStringExtra(Intent.EXTRA_TEXT);if(text!=null&&text.length()>MAX_SHARED_TEXT_CHARS)return 0;return text!=null&&!text.trim().isEmpty()?saveText(text,mime):0;}
    public int importIntent(Intent i){if(i==null)return 0;String action=i.getAction(),mime=i.getType();int n=0;if(Intent.ACTION_SEND.equals(action)){if(importSingle(i)>0)n++;}else if(Intent.ACTION_SEND_MULTIPLE.equals(action)){ArrayList<Uri> us=i.getParcelableArrayListExtra(Intent.EXTRA_STREAM);if(us!=null)for(int idx=0;idx<us.size()&&idx<MAX_SHARED_ITEMS;idx++){Uri u=us.get(idx);long id=(mime!=null&&mime.startsWith("image/"))?saveImage(u,mime):(mime!=null&&mime.startsWith("audio/"))?saveAudio(u,mime):saveFile(u,mime);if(id>0)n++;}}return n;}
    ...
    private String copy(Uri uri,String name,String folder){
        File out=null;
        try{
