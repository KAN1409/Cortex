 ...
 * only when repeated meaningful evidence exists. Ambient UI/system state is never a working Context.
 */
public final class ContextResolver {
    ...
    public static ContextStateStore.ContextState refresh(VaultDb db){
        if(db==null)return null;ContextSchema.ensure(db);long now=System.currentTimeMillis();
        ContextStateStore.ContextState existing=ContextStateStore.primary(db);
        if(existing!=null&&CortexTruthPolicy.ambientContext(existing.title+" "+existing.summary,""))
            ContextStateStore.suspend(db,existing.id,"Truth policy: ambient/system signal is not a working context");
        Candidate best=best(db,now);
