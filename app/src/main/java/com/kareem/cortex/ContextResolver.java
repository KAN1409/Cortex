...
public final class ContextResolver {
    ...
    private static String derived(VaultDb db,ContextStateStore.ContextState context,String...kinds){Cursor c=null;try{String thread="";if(context.stableKey.startsWith("thread:"))thread=context.stableKey.substring(7);StringBuilder in=new StringBuilder();for(int i=0;i<kinds.length;i++){if(i>0)in.append(',');in.append('\'').append(kinds[i]).append('\'');}String sql="SELECT title,body FROM derived_items WHERE state='open' AND kind IN ("+in+")"+(thread.isEmpty()?"":" AND thread_id=?")+" ORDER BY importance DESC,updated_at DESC LIMIT 1";c=db.getReadableDatabase().rawQuery(sql,thread.isEmpty()?null:new String[]{thread});if(!c.moveToFirst()&& !thread.isEmpty()){c.close();c=db.getReadableDatabase().rawQuery("SELECT title,body FROM derived_items WHERE state='open' AND kind IN ("+in+") ORDER BY importance DESC,updated_at DESC LIMIT 1",null);if(!c.moveToFirst())return"";}else if(c==null||c.isAfterLast())return"";String title=n(c.getString(0)),body=n(c.getString(1));return !body.isEmpty()?clip(body,220):clip(title,220);}catch(Throwable ignored){return"";}finally{if(c!=null)c.close();}}
