...
public final class RelevanceDecisionStatusStore {
    public static final String VERSION="relevance_status_001";
    ...
    public static boolean modelStatus(VaultDb db,long signalId,String status){if(db==null||signalId<=0)return false;ensure(db);ContentValues v=new ContentValues();v.put("model_status",n(status));v.put("updated_at",System.currentTimeMillis());return db.getWritableDatabase().update("relevance_evaluations",v,"signal_id=?",new String[]{String.valueOf(signalId)})>0;}
