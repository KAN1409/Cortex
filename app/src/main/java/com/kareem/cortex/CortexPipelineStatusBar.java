...
public final class CortexPipelineStatusBar {
    ...
    public static LinearLayout build(Activity a,VaultDb vault){
        ...
        TextView detail=CortexUi.plain(a,"Observed "+s.raw+"  •  semantic waiting "+s.semanticWaiting+"  •  media queued "+s.mediaQueued+"  •  judged Now "+s.visibleNow+" / "+s.maxNow+"  •  policy "+s.policyVersion+"  •  legacy shadow "+s.nowSelected+"  •  Brain memory "+s.brainMemory,9,CortexUi.MUTED);detail.setPadding(0,CortexUi.dp(a,7),0,0);card.addView(detail);
        return card;
    }
    ...
    public static final class Snapshot{public long raw,complete,semanticWaiting,mediaQueued,processing,nowSelected,visibleNow,brainMemory,maxNow;public String policyVersion="local";}
}
