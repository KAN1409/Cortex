...
public final class CortexPipelineStatusBar {
    ...
    public static LinearLayout build(Activity a,VaultDb vault){
        ...
        TextView detail=CortexUi.plain(a,"Observed "+s.raw+"  •  semantic waiting "+s.semanticWaiting+"  •  media queued "+s.mediaQueued+"  •  visible Now "+s.visibleNow+"  •  cognitive selected "+s.nowSelected+"  •  Brain memory "+s.brainMemory,9,CortexUi.MUTED);detail.setPadding(0,CortexUi.dp(a,7),0,0);card.addView(detail);
        return card;
    }
    ...
    public static Snapshot read(VaultDb vault){
        ...
        try{PrimeBriefStore.Snapshot p=PrimeBriefStore.load(vault);s.visibleNow=p.actions.size()+p.waiting.size()+p.decisions.size()+p.worthKnowing.size();}catch(Throwable ignored){}s.processing=s.semanticWaiting+s.mediaQueued;return s;
    }
    ...
    public static final class Snapshot{public long raw,complete,semanticWaiting,mediaQueued,processing,nowSelected,visibleNow,brainMemory;}
}
