...
public final class CortexPipelineStatusBar {
    ...
    public static LinearLayout build(Activity a,VaultDb vault){
        ...
        TextView detail=CortexUi.plain(a,"Observed "+s.raw+"  •  semantic waiting "+s.semanticWaiting+"  •  media queued "+s.mediaQueued+"  •  Now "+s.nowSelected+"  •  Brain memory "+s.brainMemory,9,CortexUi.MUTED);detail.setPadding(0,CortexUi.dp(a,7),0,0);card.addView(detail);
        return card;
    }
    ...
    public static Snapshot read(VaultDb vault){
        ...
        s.nowSelected=scalar(db,"SELECT COUNT(*) FROM cognitive_shadow_decisions WHERE surface=1");
        s.brainMemory=scalar(db,"SELECT COUNT(*) FROM knowledge_items WHERE source='semantic_bridge'");
        s.processing=s.semanticWaiting+s.mediaQueued;return s;
    }
    ...
    public static final class Snapshot{public long raw,complete,semanticWaiting,mediaQueued,processing,nowSelected,brainMemory;}
}
