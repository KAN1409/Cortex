...
/** Adds and normalizes the large strict verification matrix after the master run. */
public final class CortexExhaustiveReportAugmenter {
    private static final long LIVE_FRESH_MS=24L*60L*60L*1000L;
    ...
    public static JSONObject augment(Context context,File report)throws Exception{
        ...
        JSONObject live=normalizeSuite(c,suite);suite.put("autopilot_live_results",live).put("autopilot_live_results_consumed",true);
        root.put("exhaustive_verification",suite).put("autopilot_live_results",live);
        ...
        int baseFail=summary.optInt("failed_steps",0),baseWarn=summary.optInt("warning_steps",0);
        ...
        summary.put("overall",overall)
            ...
            .put("autopilot_consumed",true)
            .put("autopilot_result_count",live.length())
            .put("coverage_note","Fresh Autopilot live PASS/FAIL results are consumed into protected rows. Any remaining protected row was not truthfully executable on this device/configuration.");
        JSONObject manifest=root.optJSONObject("max_data_manifest");if(manifest==null){manifest=new JSONObject();root.put("max_data_manifest",manifest);}manifest.put("exhaustive_verification_matrix",true).put("interactive_unblock_wizard",true).put("protected_live_test_layer",true).put("autopilot_live_test_layer",true).put("autopilot_live_results_consumed",true).put("suite_artifact_normalization",true).put("v61_product_regressions",true).put("v62_product_regressions",true);

        write(report,root.toString(2));summary.put("report_bytes",report.length()).put("report_file",report.getName());return summary;
    }

    private static JSONObject normalizeSuite(Context c,JSONObject suite)throws Exception{
        JSONArray in=suite.optJSONArray("tests");if(in==null)return new JSONObject();JSONArray out=new JSONArray();JSONObject liveSnapshot=new JSONObject();long now=System.currentTimeMillis();
        for(int i=0;i<in.length();i++){
            ...
            String key=liveKey(name);if(key!=null){JSONObject r=CortexLiveTestResultStore.get(c,key);if(r!=null){long at=r.optLong("at",0),age=at>0?Math.max(0,now-at):Long.MAX_VALUE;JSONObject copy=new JSONObject(r.toString()).put("age_ms",age).put("fresh",age<=LIVE_FRESH_MS);liveSnapshot.put(key,copy);JSONObject evidence=t.optJSONObject("evidence");if(evidence==null)evidence=new JSONObject();evidence.put("autopilot_live_result",copy);t.put("evidence",evidence);if(age<=LIVE_FRESH_MS){String s=r.optString("status","");if("PASS".equalsIgnoreCase(s))t.put("status",CortexExhaustiveVerificationSuite.EXECUTED_PASS).put("expected",t.optString("expected","")+" · verified by one-shot Autopilot");else if("FAIL".equalsIgnoreCase(s))t.put("status",CortexExhaustiveVerificationSuite.EXECUTED_FAIL).put("expected",t.optString("expected","")+" · executed by one-shot Autopilot");}}}
            out.put(t);
        }
        suite.put("tests",out);recount(suite,out);return liveSnapshot;
    }

    private static String liveKey(String name){
        if("Real microphone phrase + ASR".equals(name))return"mic_asr";
        if("Real notification capture".equals(name))return"notification_capture";
        if("Live screenshot-folder import".equals(name))return"screenshot_import";
        if("Reversible Calendar create/delete".equals(name))return"calendar_write";
        if("Backup external document write".equals(name))return"backup_external";
        if("Synthetic restore rollback".equals(name))return"restore_rollback";
        if("Email draft UI handoff".equals(name))return"email_handoff";
        if("SMS draft UI handoff".equals(name))return"sms_handoff";
        if("Dialer UI handoff".equals(name))return"dialer_handoff";
        if("Real Local Qwen inference".equals(name))return"local_qwen";
        if("Real Strong Vision provider call".equals(name))return"strong_vision";
        if("30-minute stability soak".equals(name))return"soak_30m";
        return null;
    }
