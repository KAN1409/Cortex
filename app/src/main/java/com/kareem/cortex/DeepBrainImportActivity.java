...
public final class DeepBrainImportActivity extends Activity {
    TextView state,openNow; volatile boolean destroyed=false; final ExecutorService worker=Executors.newSingleThreadExecutor();
    ...
    void build(){LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setGravity(Gravity.CENTER_HORIZONTAL);root.setPadding(dp(24),dp(32),dp(24),dp(28));root.setBackgroundColor(CortexUi.BG);TextView h=CortexUi.plain(this,"Apply ChatGPT to Cortex",25,CortexUi.TEXT);CortexUi.medium(h);root.addView(h);state=CortexUi.text(this,"Validating response…",13,CortexUi.MUTED);state.setGravity(Gravity.CENTER);state.setPadding(0,dp(18),0,dp(18));root.addView(state,new LinearLayout.LayoutParams(-1,-2));openNow=CortexUi.action(this,"Open Now",CortexUi.ACCENT,true);openNow.setEnabled(false);openNow.setOnClickListener(v->{startActivity(new Intent(this,CompactTodayActivity.class).addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP|Intent.FLAG_ACTIVITY_SINGLE_TOP));finish();});LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(-1,dp(48));p.setMargins(0,dp(8),0,0);root.addView(openNow,p);TextView deep=CortexUi.action(this,"Open Deep Brain",CortexUi.MUTED,false);deep.setOnClickListener(v->{startActivity(new Intent(this,DeepBrainActivity.class));finish();});LinearLayout.LayoutParams dpv=new LinearLayout.LayoutParams(-1,dp(46));dpv.setMargins(0,dp(8),0,0);root.addView(deep,dpv);TextView close=CortexUi.action(this,"Close",CortexUi.MUTED,false);close.setOnClickListener(v->finish());LinearLayout.LayoutParams cp=new LinearLayout.LayoutParams(-1,dp(46));cp.setMargins(0,dp(8),0,0);root.addView(close,cp);setContentView(root);CortexUi.fitSystemBars(this,root);}

    void handle(Intent i){if(i==null||!Intent.ACTION_SEND.equals(i.getAction())){state.setText("Nothing to apply.");return;}CharSequence text=i.getCharSequenceExtra(Intent.EXTRA_TEXT);String raw=text==null?"":text.toString();if(!raw.contains(CognitiveDeepBrainProtocolV4.RESPONSE_MARKER)){state.setText("Rejected safely: this share is not a CORTEX_RESPONSE_V1 Deep Brain response.");return;}state.setText("Validating request ID and allowed updates…");worker.execute(()->{VaultDb db=null;try{db=new VaultDb(getApplicationContext());CognitiveDeepBrainApplyV4.Result r=CognitiveDeepBrainApplyV4.apply(db,raw);post(()->{state.setText((r.answer.isEmpty()?"":r.answer+"\n\n")+r.human()+"\n\nPulse is ready to show the applied ranking.");openNow.setEnabled(true);});}catch(Throwable e){post(()->state.setText("Response rejected safely: "+safe(e.getMessage())));}finally{if(db!=null)try{db.close();}catch(Throwable ignored){}}});}
