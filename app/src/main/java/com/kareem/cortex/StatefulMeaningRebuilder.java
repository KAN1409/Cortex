...
public final class StatefulMeaningRebuilder {
    public static final String PROCESSOR_VERSION="stateful_rebuilder_002";
    private StatefulMeaningRebuilder(){}

    public static int run(VaultDb vault,int maxRows){
        if(vault==null)return 0;SQLiteDatabase db=vault.getWritableDatabase();StatefulMeaningStore.ensure(db);CommitmentLifecycleStore.ensure(db);int budget=Math.max(1,Math.min(500,maxRows)),done=0;
        done+=rebuildObservations(db,budget-done);if(done<budget)done+=rebuildSemantic(db,budget-done);
        // Commitment backfill has its own bounded allowance so existing projected history can be
        // learned immediately without changing the established return/budget contract above.
