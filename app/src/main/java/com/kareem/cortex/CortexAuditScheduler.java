...
/** One-tap audit orchestration. The automatic suite runs now and finalizes as soon as it finishes. */
public final class CortexAuditScheduler {
    ...
    public static long start(Context c){
        long tap=SystemClock.elapsedRealtime();Context app=c.getApplicationContext();VaultDb db=new VaultDb(app);CortexAuditStore.Run r=CortexAuditStore.start(app,db);long id=r.id;long now=System.currentTimeMillis();db.getWritableDatabase().execSQL("UPDATE cortex_audit_runs SET target_end_at=?,summary=? WHERE id=?",new Object[]{now+30L*60L*1000L,"Immediate Cortex full application audit",id});db.getWritableDatabase().execSQL("UPDATE cortex_audit_events SET detail=? WHERE run_id=? AND event='started'",new Object[]{"Button tap accepted. Immediate full-app audit created.",id});
        WorkManager wm=WorkManager.getInstance(app);Data d=new Data.Builder().putLong("run_id",id).build();OneTimeWorkRequest first=new OneTimeWorkRequest.Builder(CortexAuditWorker.class).setInputData(d).setBackoffCriteria(BackoffPolicy.LINEAR,30,TimeUnit.SECONDS).addTag("cortex-full-audit").build();OneTimeWorkRequest fin=new OneTimeWorkRequest.Builder(CortexAuditFinalizeWorker.class).setInputData(d).addTag("cortex-full-audit").build();wm.beginUniqueWork(initial(id),ExistingWorkPolicy.REPLACE,first).then(fin).enqueue();
        long ms=SystemClock.elapsedRealtime()-tap;try{CortexAuditStore.log(db,id,"info","audit_ui","work_scheduled","Start button acknowledged; immediate full-app audit + final summary queued in "+ms+" ms",new JSONObject().put("ui_schedule_latency_ms",ms).put("mode","immediate"));}catch(Exception ignored){}db.close();return id;
    }
