...
public final class StartupMaintenance {
    ...
    private static void run(Context context){
        if(StartupSafetyGate.active())return;VaultDb db=null;
        try{
            db=new VaultDb(context);CognitiveSchema.ensure(db.getWritableDatabase());StatefulMeaningStore.ensure(db.getWritableDatabase());CanonicalStateStore.ensure(db.getWritableDatabase());RelevanceDecisionStatusStore.ensure(db);PhoneContextStore.ensure(db);KnowledgeV2Schema.ensure(db.getWritableDatabase());if(PhoneUsageAccess.has(context))PhoneUsageAccess.syncRecent(context,db,System.currentTimeMillis()-2L*60L*60L*1000L);importLastCrash(context,db);AdjudicationRecovery.run(context,db);ContactSafetyMaintenance.run(db);EntityGraphMaintenance.run(db);EntityQualityMaintenance.run(db);IntentionalCognitiveBridge.backfill(db,250);
            StatefulMeaningRebuilder.run(db,500);
            CanonicalAttentionMaterializer.run(context,db);
            EnvironmentPreflight.run(context);AdjudicationRecovery.schedule(context);if(StatefulMeaningRebuilder.hasBacklog(db))StatefulMeaningScheduler.kick(context);
            VisualMemoryRuntime.enqueueCompletionMaintenance(context);SemanticModelInstallWorker.enqueue(context);KnowledgeV2Scheduler.enqueue(context);
