...
public final class SecondBrainEngine {
    ...
    public static GroundedAnswer ask(VaultDb db,String question){
        ...
        ArrayList<SemanticHit> semantic=SemanticIndex.searchForAsk(db,q,8);
        ArrayList<SemanticHit> structured=KnowledgeV2Grounding.search(db,q,8);
        ArrayList<SemanticHit> hits=KnowledgeV2Grounding.merge(semantic,structured,8);
        ArrayList<String> loops=new ArrayList<>(),decisions=new ArrayList<>();boolean wantsLoops=hasAny(q,"todo","task","open loop","follow up","pending","need to","لازم","محتاج","متابعة","معلق","ناقص","اعمل ايه","أعمل ايه");boolean wantsDecisions=hasAny(q,"decision","decided","agreed","approved","choice","قرار","قررنا","اتفقنا","اختيار","اعتمدنا");if(wantsLoops)for(BrainOpenLoop l:openLoops(db,80))if(relevant(q,l.action+" "+l.title)||sourceContains(hits,l.itemId))loops.add(l.action+(empty(l.due)?"":" — "+l.due)+" [M"+sourceNumber(hits,l.itemId)+"]");if(wantsDecisions)decisions.addAll(decisions(db,hits,12));if(hits.isEmpty()||hits.get(0).score<0.105)return new GroundedAnswer(q,"I don't have enough evidence in Cortex to answer that reliably.",0,hits,loops,decisions);
    ...
    public static ArrayList<KnowledgeItem> context(VaultDb db,BrainNode node,int limit){ArrayList<SemanticHit> semantic=SemanticIndex.search(db,node.label,Math.max(limit,8));ArrayList<SemanticHit> structured=KnowledgeV2Grounding.search(db,node.label,Math.max(limit,8));ArrayList<SemanticHit> hits=KnowledgeV2Grounding.merge(semantic,structured,limit);ArrayList<KnowledgeItem> out=new ArrayList<>();for(SemanticHit h:hits)out.add(h.item);return out;}
