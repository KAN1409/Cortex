...
public class SettingsActivity extends Activity {
    ...
    void build(){LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setBackgroundColor(CortexUi.BG);ScrollView sv=new ScrollView(this);LinearLayout body=new LinearLayout(this);body.setOrientation(LinearLayout.VERTICAL);body.setPadding(dp(20),dp(14),dp(20),dp(26));sv.addView(body);root.addView(sv,new LinearLayout.LayoutParams(-1,0,1));LinearLayout head=new LinearLayout(this);head.setOrientation(LinearLayout.HORIZONTAL);head.setGravity(Gravity.CENTER_VERTICAL);TextView back=CortexUi.plain(this,"‹",34,CortexUi.TEXT);back.setGravity(Gravity.CENTER);back.setOnClickListener(v->finish());head.addView(back,new LinearLayout.LayoutParams(dp(42),dp(48)));TextView h=CortexUi.plain(this,"Settings",29,CortexUi.TEXT);CortexUi.medium(h);head.addView(h,new LinearLayout.LayoutParams(0,-2,1));body.addView(head);
        body.addView(CortexUi.section(this,"Brain"));row(body,"External AI",GeminiKeyStore.has(this)?"Gemini configured · Brain defaults to Combined with local fallback":"Configure Gemini for Combined/External Brain; Combined still falls back locally",GeminiSettingsActivity.class);
        body.addView(CortexUi.section(this,"Capture & phone awareness"));row(body,"Phone context access","Notifications, current app/window context and recent app usage",PhoneContextAccessActivity.class);row(body,"Transcription","Voice transcription providers and preferences",AsrSettingsActivity.class);actionRow(body,"Screen understanding",CortexScreenAccessibilityService.connected()?"Ready · explicit Understand screen capture + local phone context":"Enable screen/window understanding",()->{try{startActivity(new Intent(Settings.ACTION_ACCESSIBILITY_SETTINGS));}catch(Throwable ignored){}});
        ...
        body.addView(CortexUi.section(this,"Advanced"));row(body,"Advanced diagnostics","External model health, phone-context health, runtime, audits, OCR and recovery tools",EnvironmentActivity.class);row(body,"Review queue","Resolve uncertain actions, waiting items, decisions and projects",ReviewQueueActivity.class);setContentView(root);CortexUi.fitSystemBars(this,root);}
