...
public final class CortexCapabilityRegistry {
    ...
    public static State evaluate(Context ctx,VaultDb db,Capability x){try{switch(x.key){
        ...
        case"visual_intelligence":{VisualInsightStore.ensure(db);VisualRecoveryStore.ensure(db);int done=VisualInsightStore.countDone(db),recovering=VisualInsightStore.countRecovering(db),waiting=VisualInsightStore.countWaitingProvider(db),terminal=VisualInsightStore.countFailed(db),rate=VisualInsightStore.countRateLimited(db);if(terminal>0)return failed(done+" done · "+recovering+" recovering · "+waiting+" waiting-provider · "+terminal+" terminal failure(s) · rate-limited="+rate);if(waiting>0&&!GeminiKeyStore.has(ctx))return setup(done+" done · "+waiting+" visual item(s) waiting for Gemini setup · "+recovering+" recovering");if(waiting>0)return ready(done+" done · "+waiting+" waiting-provider item(s) queued to resume · "+recovering+" recovering · no terminal failures");if(recovering>0)return ready(done+" done · "+recovering+" recovering · no terminal failures · rate-limited="+rate+" · open Visual recovery for details");return active(done+" visual understanding result(s) · no unresolved recovery items");}
        ...
        case"background_visual":{VisualInsightStore.ensure(db);VisualRecoveryStore.ensure(db);int terminal=VisualInsightStore.countFailed(db),recovering=VisualInsightStore.countRecovering(db),waiting=VisualInsightStore.countWaitingProvider(db),rate=VisualInsightStore.countRateLimited(db);String worker=VisualInsightStore.workerState(ctx);if(terminal>0)return failed(terminal+" terminal visual item(s) · "+recovering+" recovering · "+waiting+" waiting-provider · rate-limited="+rate+" · recovery available in Advanced diagnostics");if(waiting>0&&!GeminiKeyStore.has(ctx))return setup(waiting+" visual item(s) waiting for Gemini setup · worker="+safe(worker));if(waiting>0)return ready(waiting+" visual item(s) waiting for provider resume · "+recovering+" recovering · worker="+safe(worker));if(recovering>0)return ready(recovering+" visual item(s) recovering · rate-limited="+rate+" · worker="+safe(worker)+" · no terminal failures");return active("Background visual worker has no unresolved recovery items · worker="+safe(worker));}
