...
public final class CortexCapabilityRegistry {
    ...
    public static State evaluate(Context ctx,VaultDb db,Capability x){try{switch(x.key){
        ...
        case"notification_capture":{if(!PrivacyPolicy.canCollect(ctx,"notifications"))return setup("Notifications are set to Never collect");CortexLocalBusStoreV1.ensure(db);long accepted=count(db,"SELECT COUNT(*) FROM connector_ingest_events WHERE connector_id='second_brain' AND state='ACCEPTED'");return accepted>0?active("Cortex Relay Local Bus · "+accepted+" accepted Relay event(s)"):ready("Cortex Relay Local Bus receiver installed · waiting for Relay evidence");}
        case"audio_asr":return audioAsr(ctx);
        case"background_visual":{VisualInsightStore.ensure(db);int fail=VisualInsightStore.countFailed(db),rate=VisualInsightStore.countRateLimited(db);return fail>0?failed(fail+" failed visual item(s) · recovery available in Advanced diagnostics"):active("Background visual worker has no stored failures · rate-limited="+rate);}
