...
public final class CortexCapabilityRegistry {
    ...
    public static State evaluate(Context ctx,VaultDb db,Capability x){try{switch(x.key){
        ...
        case"ocr":return ArabicOcr.modelReady(ctx)?active("ML Kit Latin + bundled Arabic OCR available locally"):setup("Arabic OCR model is not ready");
        ...
        case"strong_vision":return ready("Paid cloud vision is disabled by v64. Local OCR remains operational; on-device multimodal Nano is not yet promoted as a production image-understanding path.");
        case"local_qwen":return localQwen(ctx);
        case"grounded_ask":return table(db,"knowledge_items")?active("Grounded Ask retrieves from the local Cortex Vault; local AI enrichment is optional"):failed("Grounded Ask cannot read the Cortex Vault");
        case"semantic_retrieval":return table(db,"knowledge_items")?active("Grounded semantic retrieval is wired to local Cortex memory"):failed("Cortex memory retrieval source is unavailable");
        ...
        case"notification_capture":{if(!PrivacyPolicy.canCollect(ctx,"notifications"))return setup("Notifications are set to Never collect");PhoneContextStore.ensure(db);long seen=count(db,"SELECT COUNT(*) FROM phone_context_events WHERE kind='notification_context'");return notificationEnabled(ctx)?active("Notification Listener enabled · "+seen+" bounded/redacted notification-context event(s) seen"):access("Enable Cortex Notification Access");}
        case"audio_asr":return audioAsr(ctx);
        case"background_visual":{VisualInsightStore.ensure(db);int fail=VisualInsightStore.countFailed(db),rate=VisualInsightStore.countRateLimited(db);return fail>0?failed(fail+" failed visual item(s) · recovery available in Advanced diagnostics"):active("Background local visual worker has no stored failures · rate-limited="+rate);}
        ...
        case"external_writes":return externalHandlers(ctx)>0?ready(externalHandlers(ctx)+" approval-first Android draft handler(s) available · Cortex never presses Send/Save"):setup("No email/SMS/calendar handlers resolved");
    ...
    private static State localQwen(Context ctx){
        if(!LocalModelManager.installed(ctx))return setup("Optional local Qwen model/runtime is not installed");
        if(!LocalModelManager.verified(ctx))return setup("Local Qwen files exist but the model has not passed verification");
        LocalLlmRuntime.State s=LocalLlmRuntime.state(ctx);if("failed".equals(s.state))return failed("Local Qwen runtime self-test failed"+(safe(s.error).isEmpty()?"":": "+safe(s.error)));if(!LocalLlmRuntime.ready(ctx))return ready("Local Qwen verified · run the local inference self-test before it is marked ACTIVE");return active("Local Qwen verified + real local inference ready"+(s.tokensPerSecond>0?" · "+String.format(Locale.US,"%.2f tok/s",s.tokensPerSecond):""));
    }
    ...
    private static State audioAsr(Context ctx){
        ...
        SystemAudioTranscriber.RuntimeStatus s=SystemAudioTranscriber.runtimeStatus(ctx);
        if(s.onDevice)return active("On-device Android speech available · Arabic/English switching configured · no paid API");
        if(s.platform)return ready("System speech fallback available · on-device recognizer not currently exposed · no Cortex paid API key required");
        return setup("No Android speech recognition service is currently available on this device runtime");
    }

    private static State soakState(VaultDb db){CortexAuditScheduler.SoakState s=CortexAuditScheduler.soakState(db);if(s==null)return ready("30-minute real stability soak available · tap capability #33 to start");if(s.active()){long left=Math.max(0,s.targetEndAt-System.currentTimeMillis());return active("Stability soak running · about "+Math.max(1,(left+59_999L)/60_000L)+" min remaining");}if("complete".equals(s.status))return active("Last real stability soak completed · tap capability #33 to run again");return ready("Stability soak available · last run status: "+safe(s.status));}
    private static State warmQwen(Context ctx){if(!LocalModelManager.verified(ctx))return setup("Optional local Qwen is not installed/verified");long age=0;try{age=LocalLlmBridge.cachedModelAgeMs();}catch(Throwable ignored){}if(age>0)return active("Production Qwen model cache is warm · age "+friendlyAge(age));return ready("Local Qwen verified · production cache is cold · tap capability #42 to warm it");}

    private static State componentState(Context c){try{PackageManager p=c.getPackageManager();p.getActivityInfo(new ComponentName(c,InputActivity.class),0);p.getActivityInfo(new ComponentName(c,CaptureResultActivity.class),0);p.getActivityInfo(new ComponentName(c,PhoneContextAccessActivity.class),0);p.getActivityInfo(new ComponentName(c,CapabilityMatrixActivity.class),0);p.getActivityInfo(new ComponentName(c,PromptLibraryActivity.class),0);p.getActivityInfo(new ComponentName(c,LocalBrainSettingsActivity.class),0);p.getServiceInfo(new ComponentName(c,NotificationCaptureService.class),0);p.getServiceInfo(new ComponentName(c,CortexScreenAccessibilityService.class),0);if(p.resolveContentProvider(c.getPackageName()+".shizuku",0)==null)return failed("Shizuku provider is not registered");return active("Core Cortex activities/services/providers resolve correctly · Shizuku remains optional");}catch(Throwable e){return failed("A core Android component is missing: "+safe(e.getMessage()));}}
    private static State permissionState(Context c){int miss=0;if(!granted(c,Manifest.permission.RECORD_AUDIO))miss++;if(!notificationEnabled(c))miss++;if(!PhoneUsageAccess.has(c))miss++;boolean a=CortexScreenAccessibilityService.connected()||accessibilityEnabled(c);if(!a)miss++;String sh=ShizukuContextBridge.granted()?"Shizuku optional enhancement granted":(ShizukuContextBridge.available()?"Shizuku running · optional permission not granted":"Shizuku optional/not running");String privacy=!PrivacyPolicy.canCollect(c,"phone_context")?" · phone context collection is disabled by privacy setting":"";return miss==0&&privacy.isEmpty()?active("Standard Android context stack enabled · "+sh):access(miss+" standard permission/special-access layer(s) need user approval"+privacy+" · "+sh);}
