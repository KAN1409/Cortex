...
public final class CortexDiagnosticExporter {
    public static final String VERSION="cortex_diagnostic_003";
    ...
    public static Exported export(Context context)throws Exception{
        ...
        try{
            SQLiteDatabase db=vault.getWritableDatabase();UniversalEventStore.ensure(db);CortexJudgmentTraceStore.ensure(db);CanonicalStateStore.ensure(db);
            JSONObject root=new JSONObject();root.put("schemaVersion",VERSION);root.put("generatedAt",System.currentTimeMillis());root.put("package",app.getPackageName());root.put("versionName",BuildConfig.VERSION_NAME);root.put("versionCode",BuildConfig.VERSION_CODE);root.put("activePolicy",CortexPersonalPolicy.current(app));
            ...
            JSONObject counts=new JSONObject().put("rawObservations",raw).put("semanticEvents",semantic).put("canonicalSupported",supported).put("canonicalRejected",rejected).put("pipelineStages",stages).put("knowledgeItems",knowledge).put("derivedItems",derived).put("activeSituations",situations).put("openAttentionItems",attention).put("memoryPromotions",memory).put("judgmentTrace",judgment).put("nowJudgments",nowJudgments).put("promotableSemanticEvents",promotable);root.put("tableCounts",counts);
            JSONObject visual=visualHealth(app);root.put("visualMemoryHealth",visual);root.put("pipelineHealth",pipelineHealth(raw,semantic,knowledge,stages,situations,attention,judgment,nowJudgments,promotable,memory,visual.optBoolean("modelInstalled",true),visual.optLong("semanticPending",0)));
            root.put("authority",new JSONObject().put("legacyClassification","ARCHIVE_ONLY").put("evidenceAttentionAuthority","NONE").put("triageAuthority",AttentionDecisionEngine.VERSION).put("finalAttentionAuthority",CortexAttentionJudge.VERSION));
