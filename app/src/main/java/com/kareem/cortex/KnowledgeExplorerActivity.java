...
public final class KnowledgeExplorerActivity extends Activity {
    ...
    void build(){
        ...
        ht.addView(CortexUi.text(this,"Structured memory with source, confidence and relationships.",11,CortexUi.MUTED));head.addView(ht,new LinearLayout.LayoutParams(0,-2,1));body.addView(head);

        SQLiteDatabase s=db.getReadableDatabase();
        int evidence=count(s,"kv2_evidence","knowledge_eligible=1"),facts=count(s,"kv2_facts","state='active'"),events=count(s,"kv2_events","status<>'dismissed'"),entities=countSemanticEntities(s),categories=count(s,"kv2_categories","state='active'");
        int[] p=processing(s);
        LinearLayout overview=CortexUi.card(this,20);TextView nums=CortexUi.text(this,
                evidence+" evidence  •  "+facts+" facts  •  "+events+" events\n"+entities+" identities  •  "+categories+" categories\n"+
                p[2]+" understood  •  "+p[0]+" pending  •  "+p[1]+" running  •  "+p[3]+" blocked  •  "+p[4]+" failed",12,CortexUi.TEXT);overview.addView(nums);
        ...
        Cursor es=s.rawQuery("SELECT id,kind,canonical_name FROM entity_nodes WHERE status='active' AND upper(kind) IN ('PERSON','PROJECT','ORGANIZATION','ORG','PRODUCT','PLACE') ORDER BY updated_at DESC LIMIT 120",null);
        ...
        Cursor fs=s.rawQuery("SELECT f.id,f.predicate,f.object_type,f.object_value,f.confidence,e.id,e.source_uri,e.observed_at FROM kv2_facts f LEFT JOIN kv2_fact_evidence l ON l.fact_id=f.id LEFT JOIN kv2_evidence e ON e.id=l.evidence_id WHERE f.state='active' ORDER BY f.updated_at DESC LIMIT 60",null);
    ...
    void entityCard(long id,String kind,String name){
        LinearLayout c=CortexUi.card(this,17);TextView t=CortexUi.text(this,name,14,CortexUi.TEXT);CortexUi.medium(t);c.addView(t);c.addView(CortexUi.plain(this,kind+"  •  tap for sources & relationships",10,CortexUi.LIME));c.setOnClickListener(v->entityDialog(id,name));body.addView(c,margin(7));
    }
    ...
    void entityDialog(long id,String name){
        ...
        int edges=0;while(c.moveToNext()){edges++;TextView x=CortexUi.text(this,c.getString(4)+"  •  "+Math.round(c.getDouble(5)*100)+"%\n"+c.getString(0)+" #"+c.getLong(1)+" → "+c.getString(2)+" #"+c.getLong(3),11,CortexUi.TEXT);x.setPadding(0,dp(6),0,dp(6));box.addView(x);}c.close();
        if(edges==0)box.addView(CortexUi.text(this,"No relationships inferred yet.",11,CortexUi.MUTED));
        new android.app.AlertDialog.Builder(this).setTitle(name).setView(sv).setNegativeButton("Close",null).show();
    }
