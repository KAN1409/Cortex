...
public final class SituationSheet {
    ...
    public static void show(Activity activity,CortexDb db,CortexDb.Row row,Runnable refresh){
        ...
        TextView attention=text(activity,row.type.replace('_',' ').toUpperCase(),10,BRAND,true);attention.setPadding(0,dp(activity,9),0,dp(activity,8));sheet.addView(attention);
        ...
        addWide(sheet,wide(activity,"Wrong / Cortex misunderstood",SURFACE2,AMBER),()->{SituationActions.markMisunderstood(db,row.id);if(source!=null&&source.isVoice()&&!source.transcript.isEmpty())editTranscript(activity,db,source,dialog,refresh);else Toast.makeText(activity,"Marked as misunderstood",Toast.LENGTH_SHORT).show();});
    ...
    private static void later(Activity a,CortexDb db,long id,Dialog d,Runnable refresh){String[] items={"30 minutes","1 hour","Tonight","Tomorrow morning"};new AlertDialog.Builder(a).setTitle("Bring this back later").setItems(items,(x,which)->{long now=System.currentTimeMillis(),until;if(which==0)until=now+30*60_000L;else if(which==1)until=now+60*60_000L;else{java.util.Calendar c=java.util.Calendar.getInstance();if(which==2){c.set(java.util.Calendar.HOUR_OF_DAY,19);c.set(java.util.Calendar.MINUTE,0);if(c.getTimeInMillis()<=now)c.add(java.util.Calendar.DATE,1);}else{c.add(java.util.Calendar.DATE,1);c.set(java.util.Calendar.HOUR_OF_DAY,8);c.set(java.util.Calendar.MINUTE,0);}c.set(java.util.Calendar.SECOND,0);c.set(java.util.Calendar.MILLISECOND,0);until=c.getTimeInMillis();}SituationActions.snooze(db,id,until);d.dismiss();run(refresh);}).show();}
    ...
    private static LinearLayout.LayoutParams top(Activity a,int top){LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(-1,dp(a,48));p.setMargins(0,dp(a,top),0,0);return p;}private static TextView text(Activity a,String value,int sp,int color,boolean bold){TextView t=new TextView(a);t.setText(value);t.setTextColor(color);t.setTextSize(sp);if(bold)t.setTypeface(Typeface.create("sans",Typeface.BOLD));return t;}private static void linkify(TextView t){Linkify.addLinks(t,Linkify.WEB_URLS);t.setLinksClickable(true);}private static GradientDrawable round(Activity a,int fill,int stroke,int radius,int width){GradientDrawable d=new GradientDrawable();d.setColor(fill);d.setCornerRadius(dp(a,radius));if(width>0)d.setStroke(dp(a,width),stroke);return d;}private static int dp(Activity a,int v){return Math.round(v*a.getResources().getDisplayMetrics().density);}private static String clip(String s,int n){String x=s==null?"":s.trim();return x.length()<=n?x:x.substring(0,n)+"…";}private static void run(Runnable r){if(r!=null)r.run();}
}
