...
/** Fresh capture sheet. Raw evidence stays separate until perception and Cortex Brain give it meaning. */
public final class CaptureActivity extends Activity {
    ...
    private void build() {
        ...
        TextView history=text("Recent captures  ›",12,TEXT,true);history.setGravity(Gravity.CENTER_VERTICAL);history.setPadding(dp(12),0,dp(12),0);history.setBackground(round(SURFACE2,BORDER,13,1));history.setOnClickListener(v->startActivity(new Intent(this,CaptureHistoryActivity.class)));LinearLayout.LayoutParams hp=new LinearLayout.LayoutParams(-1,dp(44));hp.setMargins(0,dp(10),0,0);sheet.addView(history,hp);
        TextView asr=text("Voice: Gemini 3.6 Flash primary · Groq Whisper fallback  ·  Photos: Gemini Vision → Cortex Brain",10,MUTED,false);asr.setPadding(0,dp(12),0,0);asr.setOnClickListener(v->startActivity(new Intent(this,AsrSettingsActivity.class)));sheet.addView(asr);
    ...
    private void importUri(Uri uri,String mime,boolean forceImage){setBusy("Importing safely…");io.execute(()->{long id=0;Exception error=null;try{id=copyAndStore(uri,mime,forceImage);}catch(Exception e){error=e;}final long evidenceId=id;final Exception failure=error;runOnUiThread(()->{if(destroyed)return;if(evidenceId>0){CortexDb.AttachmentEvidence ev=db.attachmentEvidence(evidenceId);if(ev!=null&&ev.kind.startsWith("AUDIO")){setBusy("Transcribing Egyptian Arabic + English…");CaptureAnalysisQueue.analyzeVoice(this,evidenceId,this::voiceDone);}else if(ev!=null&&ev.kind.startsWith("IMAGE")){setBusy("Reading photo · extracting visible text and objects…");ImageCapturePipeline.analyze(this,evidenceId,this::imageDone);}else{Toast.makeText(this,"Captured as evidence",Toast.LENGTH_SHORT).show();openHistory();}}else{setReady();Toast.makeText(this,failure==null?"Could not import":"Import failed: "+failure.getMessage(),Toast.LENGTH_LONG).show();}});});}

    private long copyAndStore(Uri uri,String mime,boolean forceImage)throws Exception{
        String name=displayName(uri,forceImage?"image":"file");String effective=mime==null?"":mime;boolean audio=effective.startsWith("audio/");boolean image=forceImage||effective.startsWith("image/");String folder=audio?"audio":"imports";File dir=new File(getFilesDir(),folder);if(!dir.exists()&&!dir.mkdirs())throw new Exception("Could not create capture directory");String safe=name.replaceAll("[^A-Za-z0-9._-]","_");if(safe.length()>80)safe=safe.substring(safe.length()-80);File out=new File(dir,UUID.randomUUID()+"_"+safe);try(InputStream in=getContentResolver().openInputStream(uri);FileOutputStream os=new FileOutputStream(out)){if(in==null)throw new Exception("Could not open selected item");byte[] b=new byte[8192];for(int n;(n=in.read(b))!=-1;)os.write(b,0,n);}JSONObject meta=new JSONObject();meta.put("mime",effective);meta.put("name",name);meta.put("bytes",out.length());meta.put("imported_at",System.currentTimeMillis());if(image){BitmapFactory.Options o=new BitmapFactory.Options();o.inJustDecodeBounds=true;BitmapFactory.decodeFile(out.getAbsolutePath(),o);meta.put("width",o.outWidth);meta.put("height",o.outHeight);}String kind=audio?"AUDIO_IMPORT":image?"IMAGE":"FILE";String body=audio?"Voice/audio file pending transcription":image?"Photo pending visual analysis":name;String state=audio?"pending_transcription":image?"pending_vision":"observed";return db.captureAttachment(kind,name,effective,out.getAbsolutePath(),body,meta.toString(),state);
    }
    ...
    private void importMultiple(Intent i){ArrayList<Uri> uris=i.getParcelableArrayListExtra(Intent.EXTRA_STREAM);if(uris==null||uris.isEmpty())return;setBusy("Importing shared files…");io.execute(()->{int count=0;for(Uri u:uris)try{if(copyAndStore(u,i.getType(),i.getType()!=null&&i.getType().startsWith("image/"))>0)count++;}catch(Exception ignored){}final int n=count;runOnUiThread(()->{Toast.makeText(this,n+" items captured",Toast.LENGTH_SHORT).show();openHistory();});});}

    private void imageDone(long evidenceId,ImageCapturePipeline.Outcome outcome){if(destroyed)return;state.setVisibility(View.VISIBLE);if(outcome==null||!outcome.visionSucceeded()){state.setTextColor(AMBER);Exception e=outcome==null?null:outcome.visionError;state.setText("Photo is safe, but visual analysis failed.\n"+clip(e==null?"Unknown vision error":e.getMessage(),260));Toast.makeText(this,"Photo preserved · open details to retry",Toast.LENGTH_LONG).show();handler.postDelayed(()->openCapture(evidenceId),700);return;}state.setTextColor(TEXT);String summary=outcome.vision.summary;String dest=outcome.brainSucceeded()?outcome.brainResult.destination():"Evidence · Brain retry available";state.setText("Cortex photo → "+dest+"\n\n"+clip(summary,600));Toast.makeText(this,"Photo understood · Cortex → "+dest,Toast.LENGTH_LONG).show();handler.postDelayed(()->openCapture(evidenceId),750);}
    ...
    private void openCapture(long id){Intent i=new Intent(this,CaptureDetailActivity.class);i.putExtra("evidence_id",id);startActivity(i);finish();}
    private void openHistory(){startActivity(new Intent(this,CaptureHistoryActivity.class));finish();}
