...
/** Same proven Cortex recorder contract: 16 kHz mono PCM16 WAV. */
public final class AudioCapture {
    ...
    public File start(Context ctx) throws Exception {
        ...
        if (!dir.exists() && !dir.mkdirs()) throw new IOException("Could not create audio directory");
        file = new File(dir, "voice_" + System.currentTimeMillis() + ".wav");
        ...
        thread = new Thread(() -> {
            ...
            try {
                while (running) {
                    ...
                    if (n > 0) { out.write(b, 0, n); pcmBytes += n; }
                }
    ...
    public File stop() throws Exception {
        ...
        if (thread != null) try { thread.join(1200); } catch (InterruptedException ignored) { Thread.currentThread().interrupt(); }
    ...
    private static void writeHeader(RandomAccessFile f, long data) throws IOException {
        ...
        long byteRate = RATE * channels * bits / 8;
        f.writeBytes("RIFF"); le32(f, 36 + data); f.writeBytes("WAVEfmt ");
        le32(f, 16); le16(f, 1); le16(f, channels); le32(f, RATE); le32(f, byteRate);
        le16(f, channels * bits / 8); le16(f, bits); f.writeBytes("data"); le32(f, data);
    }
