...
public final class VoiceLibraryActivity extends Activity {
    ...
    private void build(){
        ...
        TextView sub=CortexUi.text(this,"All voice recordings, automatically organized and analyzed while originals remain inspectable.",11,CortexUi.MUTED);sub.setPadding(dp(2),dp(4),dp(2),dp(12));body.addView(sub);
    ...
    private View card(KnowledgeItem k){
        LinearLayout card=new LinearLayout(this);card.setOrientation(LinearLayout.VERTICAL);card.setPadding(dp(14),dp(13),dp(14),dp(12));CortexUi.pressable(this,card,CortexUi.round(this,CortexUi.SURFACE_2,CortexUi.BORDER_SOFT,21));LinearLayout top=new LinearLayout(this);top.setGravity(Gravity.CENTER_VERTICAL);TextView play=CortexUi.plain(this,"▶",17,CortexUi.LIME);play.setGravity(Gravity.CENTER);play.setBackground(CortexUi.round(this,android.graphics.Color.argb(16,190,221,82),android.graphics.Color.argb(92,190,221,82),999));top.addView(play,new LinearLayout.LayoutParams(dp(42),dp(42)));LinearLayout tx=new LinearLayout(this);tx.setOrientation(LinearLayout.VERTICAL);LinearLayout.LayoutParams xp=new LinearLayout.LayoutParams(0,-2,1);xp.setMargins(dp(11),0,dp(8),0);top.addView(tx,xp);String title=safe(k.title);if(title.isEmpty()||"Voice recording".equalsIgnoreCase(title)){String text=first(k.summary,k.extractedText,k.rawText);title=text.isEmpty()?"Voice recording":clip(text,58);}TextView h=CortexUi.text(this,title,14,CortexUi.TEXT);CortexUi.medium(h);h.setMaxLines(2);tx.addView(h);long duration=CortexWaveformView.durationMs(k.attachmentPath);String meta=stamp(k.createdAt)+(duration>0?" · "+duration(duration):"");TextView m=CortexUi.plain(this,meta,9,CortexUi.MUTED);m.setPadding(0,dp(3),0,0);tx.addView(m);String state=friendlyState(k.status);int color=isReady(k.status)?CortexUi.LIME:(isProcessing(k.status)?CortexUi.YELLOW:CortexUi.MUTED);top.addView(CortexUi.chip(this,state,color,isProcessing(k.status)),new LinearLayout.LayoutParams(-2,dp(28)));card.addView(top);
