...
/** Premium voice evidence library backed by the canonical capture store. */
public final class VoiceLibraryActivity extends Activity {
    private VaultDb db;private LinearLayout list,filters;private String filter="all";private int dp(int v){return CortexUi.dp(this,v);}
    ...
    private void build(){
        ...
        LinearLayout head=new LinearLayout(this);head.setGravity(Gravity.CENTER_VERTICAL);TextView back=CortexUi.plain(this,"‹",34,CortexUi.TEXT);back.setGravity(Gravity.CENTER);back.setContentDescription("Back");back.setOnClickListener(v->finish());head.addView(back,new LinearLayout.LayoutParams(dp(40),dp(48)));LinearLayout tx=new LinearLayout(this);tx.setOrientation(LinearLayout.VERTICAL);head.addView(tx,new LinearLayout.LayoutParams(0,-2,1));tx.addView(CortexUi.eyebrow(this,"CORTEX · VOICE MEMORY",CortexUi.ORANGE));TextView h=CortexUi.plain(this,"Voice Library",30,CortexUi.TEXT);CortexUi.medium(h);tx.addView(h);body.addView(head);
        TextView sub=CortexUi.text(this,"Recordings stay tied to their transcript, understanding and source. Nothing here becomes attention by itself.",11,CortexUi.MUTED);sub.setPadding(dp(2),dp(4),dp(2),dp(12));body.addView(sub);
        TextView capture=CortexUi.action(this,"NEW VOICE CAPTURE",CortexUi.ORANGE,true);capture.setOnClickListener(v->{Intent i=new Intent(this,ProposalCaptureActivity.class);i.putExtra("mode","voice");startActivity(i);});body.addView(capture,new LinearLayout.LayoutParams(-1,dp(46)));
    ...
    private void render(){
        if(list==null||db==null)return;list.removeAllViews();filters.removeAllViews();ArrayList<KnowledgeItem> all=db.captureSearch("",1200);int total=0,ready=0,processing=0;for(KnowledgeItem k:all){if(k==null||!"AUDIO".equalsIgnoreCase(k.type))continue;total++;if(isReady(k.status))ready++;else if(isProcessing(k.status))processing++;}
        addFilter("All",total,"all",0);addFilter("Analyzed",ready,"ready",7);addFilter("Processing",processing,"processing",7);
        int n=0;for(KnowledgeItem k:all){if(k==null||!"AUDIO".equalsIgnoreCase(k.type))continue;if("ready".equals(filter)&&!isReady(k.status))continue;if("processing".equals(filter)&&!isProcessing(k.status))continue;View card=card(k);list.addView(card,margin(0,0,0,9));CortexMotion.enter(card,Math.min(n++,5));}
        if(n==0){LinearLayout empty=CortexUi.card(this,22);empty.addView(CortexUi.eyebrow(this,"VOICE MEMORY",CortexUi.MUTED));TextView eh=CortexUi.plain(this,total==0?"No voice captures yet":"Nothing in this filter",18,CortexUi.TEXT);CortexUi.medium(eh);eh.setPadding(0,dp(8),0,0);empty.addView(eh);TextView eb=CortexUi.text(this,total==0?"Record a thought and Cortex will keep the original WAV beside its transcript and analysis.":"Choose another filter to see your recordings.",11,CortexUi.MUTED);eb.setPadding(0,dp(6),0,0);empty.addView(eb);list.addView(empty);CortexMotion.enter(empty,0);}
    }

    private void addFilter(String title,int count,String key,int left){boolean on=key.equals(filter);TextView b=CortexUi.chip(this,title+" ("+count+")",on?CortexUi.LIME:CortexUi.MUTED,on);b.setGravity(Gravity.CENTER);b.setOnClickListener(v->{filter=key;CortexMotion.haptic(v,false);render();});LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(0,dp(34),1);p.setMargins(dp(left),0,0,0);filters.addView(b,p);}

    private View card(KnowledgeItem k){
        LinearLayout card=new LinearLayout(this);card.setOrientation(LinearLayout.VERTICAL);card.setPadding(dp(14),dp(13),dp(14),dp(12));CortexUi.pressable(this,card,CortexUi.round(this,CortexUi.SURFACE_2,CortexUi.BORDER_SOFT,21));LinearLayout top=new LinearLayout(this);top.setGravity(Gravity.CENTER_VERTICAL);TextView play=CortexUi.plain(this,"▶",17,CortexUi.LIME);play.setGravity(Gravity.CENTER);play.setBackground(CortexUi.round(this,android.graphics.Color.argb(18,190,221,82),android.graphics.Color.argb(86,190,221,82),999));top.addView(play,new LinearLayout.LayoutParams(dp(42),dp(42)));LinearLayout tx=new LinearLayout(this);tx.setOrientation(LinearLayout.VERTICAL);LinearLayout.LayoutParams xp=new LinearLayout.LayoutParams(0,-2,1);xp.setMargins(dp(11),0,dp(8),0);top.addView(tx,xp);String title=safe(k.title);if(title.isEmpty()||"Voice recording".equalsIgnoreCase(title)){String text=first(k.summary,k.extractedText,k.rawText);title=text.isEmpty()?"Voice recording":clip(text,58);}TextView h=CortexUi.text(this,title,14,CortexUi.TEXT);CortexUi.medium(h);h.setMaxLines(2);tx.addView(h);long duration=CortexWaveformView.durationMs(k.attachmentPath);String meta=stamp(k.createdAt)+(duration>0?" · "+duration(duration):"");TextView m=CortexUi.plain(this,meta,9,CortexUi.MUTED);m.setPadding(0,dp(3),0,0);tx.addView(m);String state=friendlyState(k.status);int color=isReady(k.status)?CortexUi.GREEN:(isProcessing(k.status)?CortexUi.YELLOW:CortexUi.MUTED);top.addView(CortexUi.chip(this,state,color,isProcessing(k.status)),new LinearLayout.LayoutParams(-2,dp(28)));card.addView(top);
        CortexWaveformView wave=new CortexWaveformView(this);wave.setAccent(CortexUi.LIME);wave.setAudioPath(k.attachmentPath);LinearLayout.LayoutParams wp=new LinearLayout.LayoutParams(-1,dp(46));wp.setMargins(dp(2),dp(10),dp(2),0);card.addView(wave,wp);String body=first(k.summary,k.extractedText,k.rawText);if(!body.isEmpty()){TextView p=CortexUi.text(this,clip(body,180),10,CortexUi.MUTED);p.setPadding(0,dp(8),0,0);p.setMaxLines(2);card.addView(p);}if(!safe(k.analysisError).isEmpty()){TextView err=CortexUi.text(this,"Needs review · "+clip(k.analysisError,120),9,CortexUi.YELLOW);err.setPadding(0,dp(6),0,0);card.addView(err);}card.setOnClickListener(v->{Intent i=new Intent(this,VoiceDetailActivity.class);i.putExtra("item_id",k.id);startActivity(i);});return card;
    }

    private boolean isReady(String s){String x=safe(s).toLowerCase(Locale.ROOT);return"analyzed".equals(x)||"done".equals(x)||"ready".equals(x)||"complete".equals(x);}private boolean isProcessing(String s){String x=safe(s).toLowerCase(Locale.ROOT);return x.isEmpty()||"queued".equals(x)||"analyzing".equals(x)||"processing".equals(x)||"pending".equals(x);}private String friendlyState(String s){if(isReady(s))return"Analyzed";if(isProcessing(s))return"Processing";String x=safe(s);return x.isEmpty()?"Captured":x.replace('_',' ');}
