...
public final class CognitiveShadowStore {
    ...
    public static Stats stats(VaultDb db){
        ...
        while(c.moveToNext()){
            ...
            s.total++;
            if("complete".equalsIgnoreCase(state)){s.analyzed++;if("BOTH_CONTEXT".equals(comparison)||"BOTH_IGNORE".equals(comparison)||"BOTH_DERIVE".equals(comparison))s.agreement++;if("V2_FOUND_MISSED_VALUE".equals(comparison))s.missedValue++;if("V2_DOWNGRADE".equals(comparison))s.downgrade++;if("IGNORE_DISAGREEMENT".equals(comparison))s.ignoreDisagreement++;}
            else if("skipped".equalsIgnoreCase(state)||"SKIPPED".equalsIgnoreCase(outcome))s.skipped++;
            else if("failed".equalsIgnoreCase(state))s.errors++;
            String feedback=feedback(db,id);JSONObject v2=o.optJSONObject("v2");String v2Disposition=v2==null?"":v2.optString("disposition","");
            if(!feedback.isEmpty()){s.rated++;if("SHADOW_V2_BETTER".equals(feedback))s.v2Better++;else if("SHADOW_LEGACY_BETTER".equals(feedback))s.legacyBetter++;else if("SHADOW_NEITHER".equals(feedback))s.neither++;if("DERIVE".equalsIgnoreCase(v2Disposition)){s.ratedDerives++;if("SHADOW_LEGACY_BETTER".equals(feedback)||"SHADOW_NEITHER".equals(feedback))s.falseDerives++;}}
        }
        c.close();return s;
    }
    ...
    public static final class Stats{
        public int total,analyzed,agreement,missedValue,downgrade,ignoreDisagreement,skipped,errors,rated,v2Better,legacyBetter,neither,ratedDerives,falseDerives;
