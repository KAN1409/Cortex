...
/** Capture home: reference-led hero, quick capture, evidence timeline and libraries. */
public final class CaptureHubActivity extends Activity {
    ...
    private void build(){
        ...
        FrameLayout heroFrame=new FrameLayout(this);heroFrame.setBackground(CortexUi.round(this,CortexUi.SURFACE,CortexUi.BORDER,CortexUi.R_HERO));heroFrame.setClipToOutline(true);if(android.os.Build.VERSION.SDK_INT>=21)heroFrame.setOutlineProvider(ViewOutlineProvider.BACKGROUND);
        CortexEnergyRibbonView art=new CortexEnergyRibbonView(this);heroFrame.addView(art,new FrameLayout.LayoutParams(-1,-1));
        LinearLayout hero=new LinearLayout(this);hero.setOrientation(LinearLayout.VERTICAL);hero.setPadding(dp(19),dp(18),dp(19),dp(18));hero.setBackgroundColor(Color.TRANSPARENT);hero.addView(CortexUi.eyebrow(this,"CAPTURE MEMORY",CortexUi.LIME));TextView hh=CortexUi.plain(this,"Turn moments into meaning",25,CortexUi.TEXT);CortexUi.medium(hh);hh.setPadding(0,dp(11),dp(72),0);hero.addView(hh);TextView hb=CortexUi.text(this,"Capture anything — voice, image, file or screen. Cortex keeps the original, then understands it separately.",12,CortexUi.MUTED);hb.setPadding(0,dp(6),dp(62),dp(13));hero.addView(hb);
        LinearLayout intelligence=new LinearLayout(this);intelligence.setGravity(Gravity.CENTER_VERTICAL);intelligence.setPadding(dp(11),dp(9),dp(11),dp(9));intelligence.setBackground(CortexUi.round(this,Color.argb(15,240,184,56),Color.argb(72,240,184,56),16));TextView sparkle=CortexUi.plain(this,"✦",16,CortexUi.YELLOW);sparkle.setGravity(Gravity.CENTER);intelligence.addView(sparkle,new LinearLayout.LayoutParams(dp(22),dp(22)));TextView it=CortexUi.plain(this,"Intelligence is always on · evidence stays private and inspectable",10,CortexUi.TEXT);CortexUi.medium(it);LinearLayout.LayoutParams ip=new LinearLayout.LayoutParams(0,-2,1);ip.setMargins(dp(7),0,0,0);intelligence.addView(it,ip);hero.addView(intelligence);heroFrame.addView(hero,new FrameLayout.LayoutParams(-1,-2));body.addView(heroFrame,new LinearLayout.LayoutParams(-1,dp(220)));CortexMotion.hero(heroFrame);

        body.addView(CortexUi.section(this,"Quick capture"));LinearLayout quick=new LinearLayout(this);quick.setOrientation(LinearLayout.HORIZONTAL);quick.addView(quick("Camera","photo",CortexUi.YELLOW,()->capture("photo")),new LinearLayout.LayoutParams(0,dp(82),1));quick.addView(quick("Voice","voice",CortexUi.LIME,()->capture("voice")),new LinearLayout.LayoutParams(0,dp(82),1));quick.addView(quick("Image","photo",CortexUi.TEXT,()->capture("photo")),new LinearLayout.LayoutParams(0,dp(82),1));quick.addView(quick("File","file",CortexUi.TEXT,()->capture("file")),new LinearLayout.LayoutParams(0,dp(82),1));quick.addView(quick("Screen","open",CortexUi.TEXT,()->open(PhoneContextAccessActivity.class)),new LinearLayout.LayoutParams(0,dp(82),1));body.addView(quick);CortexMotion.enter(quick,1);

        if(!recent.isEmpty()){
            LinearLayout recentHead=new LinearLayout(this);recentHead.setGravity(Gravity.CENTER_VERTICAL);recentHead.addView(CortexUi.section(this,"Recent evidence"),new LinearLayout.LayoutParams(0,-2,1));TextView all=CortexUi.plain(this,"See all",10,CortexUi.LIME);CortexUi.medium(all);all.setPadding(dp(10),dp(18),dp(2),dp(8));all.setOnClickListener(v->open(VaultActivity.class));recentHead.addView(all);body.addView(recentHead);
            int i=0;for(KnowledgeItem k:recent){View row=evidenceRow(k);body.addView(row);CortexMotion.enter(row,Math.min(4,i++));}
        }

        body.addView(CortexUi.section(this,"Libraries"));View vr=library("Voice Library",voice+" recording"+(voice==1?"":"s"),"voice",CortexUi.LIME,()->open(VoiceLibraryActivity.class));body.addView(vr,lp(0,0,0,8));View ir=library("Visual Memory",images+" captured here · full screenshot index lives in Memory","photo",CortexUi.TEXT,()->open(VisualMemoryActivity.class));body.addView(ir,lp(0,0,0,8));View fr=library("Files & documents",files+" imported source item"+(files==1?"":"s"),"file",CortexUi.TEXT,()->open(VaultActivity.class));body.addView(fr,lp(0,0,0,8));
    ...
    private LinearLayout evidenceRow(KnowledgeItem k){LinearLayout row=new LinearLayout(this);row.setGravity(Gravity.CENTER_VERTICAL);row.setPadding(dp(3),dp(10),dp(3),dp(10));String type=safe(k.type).toUpperCase(Locale.ROOT);String icon="AUDIO".equals(type)?"voice":(("IMAGE".equals(type)||"SCREENSHOT".equals(type))?"photo":"file");int color="AUDIO".equals(type)?CortexUi.LIME:(("IMAGE".equals(type)||"SCREENSHOT".equals(type))?CortexUi.YELLOW:CortexUi.TEXT);row.addView(CortexUi.glyph(this,icon,color,false),new LinearLayout.LayoutParams(dp(36),dp(36)));LinearLayout tx=new LinearLayout(this);tx.setOrientation(LinearLayout.VERTICAL);LinearLayout.LayoutParams tp=new LinearLayout.LayoutParams(0,-2,1);tp.setMargins(dp(11),0,dp(6),0);row.addView(tx,tp);TextView h=CortexUi.text(this,friendlyTitle(k),12,CortexUi.TEXT);CortexUi.medium(h);h.setMaxLines(1);tx.addView(h);TextView m=CortexUi.plain(this,stamp(k.createdAt)+" · "+friendlyState(k.status),9,CortexUi.MUTED);m.setPadding(0,dp(3),0,0);tx.addView(m);TextView arrow=CortexUi.plain(this,"›",22,CortexUi.FAINT);arrow.setGravity(Gravity.CENTER);row.addView(arrow,new LinearLayout.LayoutParams(dp(26),dp(36)));row.setOnClickListener(v->{Intent i=new Intent(this,VaultActivity.class);i.putExtra("item_id",k.id);startActivity(i);});return row;}
