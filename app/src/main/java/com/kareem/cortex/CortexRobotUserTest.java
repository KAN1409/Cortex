 ...
 *
 * Schema v3 treats a click, a visible reaction and functional completion as different facts.
 * User-triggered async work that registers with CortexSemanticOperation must reach a terminal state
 * before the crawler explores another control. Screen-state branch ledgers avoid suppressing a
 * control merely because the same Activity class was visited in another semantic state.
 */
public final class CortexRobotUserTest {
    ...
    private static final long FIRST_CHANGE_BUDGET_MS=1800L,ASYNC_SETTLE_BUDGET_MS=3000L,SEMANTIC_START_WINDOW_MS=850L;

    public interface Progress {void onStep(Step step);}

    public static final class Step {
        public int seq;public String path="",screenBefore="",action="",actionClass="",status="",screenAfter="",beforeText="",afterText="",detail="",error="",terminalKind="",terminalState="",terminalDetail="";
        public long durationMs,clickDispatchMs,firstVisualChangeMs,windowTransitionMs,asyncSettleMs,terminalMs;
        JSONObject json(){JSONObject o=new JSONObject();try{o.put("seq",seq);o.put("path",path);o.put("screen_before",screenBefore);o.put("action",action);o.put("action_class",actionClass);o.put("status",status);o.put("screen_after",screenAfter);o.put("before_text",beforeText);o.put("after_text",afterText);o.put("detail",detail);o.put("duration_ms",durationMs);o.put("click_dispatch_ms",clickDispatchMs);o.put("first_visual_change_ms",firstVisualChangeMs);o.put("window_transition_ms",windowTransitionMs);o.put("async_settle_ms",asyncSettleMs);o.put("terminal_kind",terminalKind);o.put("terminal_state",terminalState);o.put("terminal_ms",terminalMs);o.put("terminal_detail",terminalDetail);o.put("error",error);}catch(Exception ignored){}return o;}
    }

    public static final class Report {
        public long startedAt=System.currentTimeMillis(),finishedAt;public boolean complete=true,accessibilityUsed;public int pressed,completed,guarded,failed,resultFailed,timeouts,cancelled,external,noChange,screenCount,statusSkipped,scrolls;public String stopReason="";public final ArrayList<Step> steps=new ArrayList<>();
        public JSONObject json(){JSONObject root=new JSONObject();try{root.put("schema_version",3);root.put("suite","CORTEX_ROBOT_USER_TEST");root.put("started_at",startedAt);root.put("finished_at",finishedAt);root.put("duration_ms",Math.max(0,finishedAt-startedAt));root.put("complete",complete);root.put("stop_reason",stopReason);root.put("accessibility_used",accessibilityUsed);root.put("screen_count",screenCount);root.put("summary",new JSONObject().put("steps",steps.size()).put("pressed",pressed).put("completed",completed).put("guarded",guarded).put("failed",failed).put("result_failed",resultFailed).put("timeouts",timeouts).put("cancelled",cancelled).put("external",external).put("no_change",noChange).put("status_skipped",statusSkipped).put("scrolls",scrolls));JSONArray a=new JSONArray();for(Step s:steps)a.put(s.json());root.put("steps",a);}catch(Exception ignored){}return root;}
        public String markdown(){StringBuilder b=new StringBuilder();b.append("# Cortex Robot User Test\n\n").append("- Complete: **").append(complete?"YES":"NO").append("**\n").append("- Duration: ").append(Math.max(0,finishedAt-startedAt)).append(" ms\n").append("- Screens discovered: ").append(screenCount).append("\n").append("- Steps: ").append(steps.size()).append(" · pressed ").append(pressed).append(" · semantic completed ").append(completed).append(" · guarded ").append(guarded).append(" · failed ").append(failed).append(" · result-failed ").append(resultFailed).append(" · timeouts ").append(timeouts).append(" · cancelled ").append(cancelled).append(" · external ").append(external).append(" · no-change ").append(noChange).append("\n").append("- Passive status controls skipped: ").append(statusSkipped).append(" · scroll explorations ").append(scrolls).append("\n").append("- Accessibility crawler: ").append(accessibilityUsed?"YES":"NO — fallback View tree only").append("\n");if(!stopReason.isEmpty())b.append("- Stop reason: ").append(stopReason).append("\n");b.append("\n## Failures / guarded actions first\n\n");boolean any=false;for(Step s:steps)if("FAILED".equals(s.status)||"FAILED_RESULT".equals(s.status)||"TIMEOUT".equals(s.status)||s.status.startsWith("GUARDED")){any=true;append(b,s);}if(!any)b.append("None.\n\n");b.append("## Full user journey trace\n\n");for(Step s:steps)append(b,s);return b.toString();}
        private static void append(StringBuilder b,Step s){b.append("### ").append(s.seq).append(" · ").append(s.status).append(" · ").append(s.action).append("\n\n- Path: `").append(s.path.replace("`","'")).append("`\n- Before: ").append(s.screenBefore).append("\n- After: ").append(s.screenAfter).append("\n- Timing: dispatch ").append(s.clickDispatchMs).append(" ms · first visual ").append(s.firstVisualChangeMs).append(" ms · window ").append(s.windowTransitionMs).append(" ms · async settle ").append(s.asyncSettleMs).append(" ms");if(s.terminalMs>0)b.append(" · terminal ").append(s.terminalMs).append(" ms");b.append("\n");if(!s.terminalKind.isEmpty())b.append("- Semantic terminal: ").append(s.terminalKind).append(" · ").append(s.terminalState).append(s.terminalDetail.isEmpty()?"":" · "+s.terminalDetail.replace("\n"," ")).append("\n");if(!s.detail.isEmpty())b.append("- Result: ").append(s.detail.replace("\n"," ")).append("\n");if(!s.error.isEmpty())b.append("- Error: `").append(s.error.replace("`","'")).append("`\n");if(!s.afterText.isEmpty())b.append("- Visible result: ").append(clip(s.afterText,360).replace("\n"," · ")).append("\n");b.append('\n');}
    }
    ...
    private static final class SemanticOutcome {CortexSemanticOperation.Snapshot op;boolean robotTimeout;long observedMs;}
    ...
    public static Report run(Activity origin,Progress progress){
        ...
        Map<String,LinkedHashSet<String>> exploredByState=new LinkedHashMap<>();Set<String> filled=new HashSet<>(),screens=new LinkedHashSet<>(),scrollTried=new HashSet<>();ArrayDeque<Frame> stack=new ArrayDeque<>();
        try{
            ...
            while(r.steps.size()<MAX_STEPS&&screens.size()<MAX_SCREENS&&SystemClock.elapsedRealtime()<deadline){
                ...
                autofill(snap,filled);sleep(60);snap=snapshot(tracker.resumed,app);List<Target> actions=targets(snap,app,r);String sig=signature(snap,actions);screens.add(sig);r.screenCount=screens.size();updateStack(stack,sig,snap.screen);LinkedHashSet<String> explored=exploredByState.computeIfAbsent(sig,k->new LinkedHashSet<>());

                Target chosen=null;for(Target t:actions)if(!explored.contains(t.logicalKey(snap.screen))){chosen=t;break;}
                if(chosen==null){
                    String scrollKey=sig+"|scroll";
                    if(CortexScreenAccessibilityService.connected()&&!scrollTried.contains(scrollKey)){scrollTried.add(scrollKey);if(CortexScreenAccessibilityService.robotScrollForward()){r.scrolls++;sleep(420);Snap afterScroll=snapshot(tracker.resumed,app);if(afterScroll!=null&&!normalize(afterScroll.text).equals(normalize(snap.text)))continue;}}
                ...
                long semanticCursor=CortexSemanticOperation.cursor();long t0=SystemClock.elapsedRealtime();boolean clicked=false;String err="";try{clicked=click(chosen,origin);}catch(Throwable e){err=e.getClass().getSimpleName()+": "+safe(e.getMessage());}long dispatch=SystemClock.elapsedRealtime()-t0;
                ...
                Consequence c=observeConsequence(tracker,app,beforeScreen,beforeText,t0);Snap after=c.snap==null?snap:c.snap;SemanticOutcome semantic=null;
                if(app.getPackageName().equals(after.pkg)||after.pkg.isEmpty())semantic=observeSemantic(semanticCursor,deadline);
                if(semantic!=null){Snap terminalSnap=snapshot(tracker.resumed,app);if(terminalSnap!=null)after=terminalSnap;}

                String status="PRESSED",detail="Control executed";
                if(!app.getPackageName().equals(after.pkg)&&!after.pkg.isEmpty()){status="PRESSED_EXTERNAL";detail="Opened external/system surface; robot did not interact with that surface";r.external++;}
                    ...
                    CortexSemanticOperation.Snapshot op=semantic.op;String state=semantic.robotTimeout?CortexSemanticOperation.TIMEOUT:(op==null?"":op.state);String kind=op==null?"":op.kind;
                    if(CortexSemanticOperation.COMPLETED.equals(state)){status="COMPLETED";detail="Control executed and semantic operation reached its successful terminal state";r.completed++;r.pressed++;}
                    else if(CortexSemanticOperation.FAILED.equals(state)){status="FAILED_RESULT";detail="Control executed, but its semantic operation reached a failed terminal result";r.resultFailed++;r.failed++;}
                    else if(CortexSemanticOperation.TIMEOUT.equals(state)){status="TIMEOUT";detail="Control executed, but semantic completion did not arrive within the operation budget";r.timeouts++;r.failed++;}
                    else if(CortexSemanticOperation.CANCELLED.equals(state)){status="CANCELLED";detail="Semantic operation was explicitly cancelled/superseded";r.cancelled++;}
                    else {status="PRESSED_PENDING";detail="Control executed and semantic work is still pending";r.pressed++;}
                }else if(normalize(beforeText).equals(normalize(after.text))&&beforeScreen.equals(after.screen)){status="PRESSED_NO_CHANGE";detail="Click completed with no visible state change and no registered semantic operation";r.noChange++;}
                else r.pressed++;

                Step s=makeStep(r,path,beforeScreen,label,chosen.cls,status,after.screen,beforeText,after.text,detail,err);s.clickDispatchMs=dispatch;s.firstVisualChangeMs=c.firstVisualMs;s.windowTransitionMs=c.windowMs;s.asyncSettleMs=c.settleMs;s.durationMs=SystemClock.elapsedRealtime()-t0;
                if(semantic!=null&&semantic.op!=null){s.terminalKind=semantic.op.kind;s.terminalState=semantic.robotTimeout?CortexSemanticOperation.TIMEOUT:semantic.op.state;s.terminalMs=semantic.observedMs;s.terminalDetail=semantic.robotTimeout?"Robot terminal wait budget expired while operation remained "+semantic.op.state:semantic.op.detail;}
                emit(progress,s);
                if("PRESSED_EXTERNAL".equals(status)){back(tracker,origin);sleep(350);continue;}
                if("TIMEOUT".equals(status)){back(tracker,origin);sleep(450);continue;}
    ...
    private static SemanticOutcome observeSemantic(long cursor,long globalDeadline){
        long waitStart=SystemClock.elapsedRealtime(),startEnd=Math.min(globalDeadline,waitStart+SEMANTIC_START_WINDOW_MS);CortexSemanticOperation.Snapshot op=null;
        while(SystemClock.elapsedRealtime()<startEnd){op=CortexSemanticOperation.firstAfter(cursor);if(op!=null)break;sleep(60);}if(op==null)return null;
        long token=op.token,budget=CortexSemanticOperation.defaultTimeoutMs(op.kind),terminalEnd=Math.min(globalDeadline,SystemClock.elapsedRealtime()+budget);while(SystemClock.elapsedRealtime()<terminalEnd){CortexSemanticOperation.Snapshot now=CortexSemanticOperation.get(token);if(now!=null){op=now;if(now.terminal())break;}sleep(100);}SemanticOutcome out=new SemanticOutcome();out.op=CortexSemanticOperation.get(token);if(out.op==null)out.op=op;out.robotTimeout=out.op!=null&&!out.op.terminal();out.observedMs=SystemClock.elapsedRealtime()-waitStart;return out;
    }
    ...
    private static List<Target> pruneFallbackAncestors(List<Target> raw){if(raw==null||raw.size()<2)return raw==null?Collections.emptyList():raw;ArrayList<Target> out=new ArrayList<>();for(Target p:raw){boolean drop=false;if(safe(p.label).trim().isEmpty())for(Target c:raw)if(c!=p&&descendantPath(p.path,c.path)){drop=true;break;}if(!drop)out.add(p);}return out;}
    private static boolean descendantPath(String parent,String child){String p=safe(parent),c=safe(child);if(c.isEmpty())return false;if(p.isEmpty())return true;return c.startsWith(p+"/");}
