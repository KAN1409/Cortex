...
public class InputActivity extends Activity {
    ...
    void build(){
        ...
        // Voice is not one tile among many anymore. The Cortex orb is the single primary voice action.
        LinearLayout hero=new LinearLayout(this);hero.setOrientation(LinearLayout.VERTICAL);hero.setGravity(Gravity.CENTER_HORIZONTAL);hero.setPadding(0,dp(20),0,dp(10));
        CortexRingButton orb=new CortexRingButton(this);orb.setGlyph(CortexRingButton.Glyph.RECORD);orb.setAccent(CortexUi.RED);orb.setProgress(0f);orb.setOnClickListener(v->capture("voice"));hero.addView(orb,new LinearLayout.LayoutParams(dp(118),dp(118)));
        TextView prompt=CortexUi.plain(this,"What's on your mind?",22,CortexUi.TEXT);CortexUi.medium(prompt);prompt.setGravity(Gravity.CENTER);prompt.setPadding(0,dp(8),0,0);hero.addView(prompt);
        TextView hint=CortexUi.text(this,"Tap the orb to speak. Cortex keeps the original evidence, understands it, then proposes the useful next move.",11,CortexUi.MUTED);hint.setGravity(Gravity.CENTER);hint.setPadding(dp(18),dp(6),dp(18),0);hero.addView(hint);body.addView(hero);

        LinearLayout quick=new LinearLayout(this);quick.setOrientation(LinearLayout.HORIZONTAL);quick.setGravity(Gravity.CENTER);quick.setPadding(0,dp(6),0,0);
        addQuick(quick,"Text","text",CortexUi.YELLOW,"text",0);addQuick(quick,"Photo","photo",CortexUi.GREEN,"photo",7);addQuick(quick,"File","file",CortexUi.ORANGE,"file",7);body.addView(quick,new LinearLayout.LayoutParams(-1,dp(74)));
