...
public class InputActivity extends Activity {
    ...
    void build(){
        ...
        body.addView(section("EVERYWHERE CORTEX",CortexUi.GREEN));boolean screenEnabled=CortexScreenAccessibilityService.enabled(this),screenConnected=CortexScreenAccessibilityService.connected();String screenState=screenEnabled?(screenConnected?"READY · Screen understanding is enabled and connected.":"ENABLED · Android access is on; Cortex will reconnect automatically."):"SETUP NEEDED · Enable screen understanding once in Android Accessibility.";body.addView(featureCard("Understand this screen",screenState,"open",screenEnabled?CortexUi.GREEN:CortexUi.ORANGE,screenEnabled?"Open Quick Settings setup":"Enable screen understanding",v->{try{startActivity(new Intent(screenEnabled?"android.settings.QUICK_SETTINGS_SETTINGS":Settings.ACTION_ACCESSIBILITY_SETTINGS));}catch(Throwable e){try{startActivity(new Intent(Settings.ACTION_ACCESSIBILITY_SETTINGS));}catch(Throwable ignored){}}}),margins(0,0,0,0));body.addView(featureCard("From another app","READY · Android Share → Cortex accepts text, links, screenshots, audio and files and returns to the same understanding flow.","nodes",CortexUi.YELLOW,"Show share instruction",v->new AlertDialog.Builder(this).setTitle("Share to Cortex").setMessage("In any app, tap Share and choose Cortex. Cortex imports the original item safely, analyzes it, then opens the grounded result with model-generated next proposals.").setPositiveButton("Got it",null).show()),margins(0,9,0,0));body.addView(featureCard("Quick voice","Start the same one-tap voice capture now, or use the Cortex Voice Quick Settings tile from any app.","wave",CortexUi.RED,"Start quick voice now",v->capture("voice")),margins(0,9,0,0));
