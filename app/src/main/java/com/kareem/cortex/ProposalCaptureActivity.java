...
public final class ProposalCaptureActivity extends SatinCaptureActivity {
    ...
    @Override void showResult(long id){
        try{NexusScheduler.kick(this);}catch(Throwable ignored){}
        try{Intent i=new Intent(this,ProposalCaptureResultActivity.class);i.putExtra("item_id",id);startActivity(i);finish();}catch(Throwable e){Toast.makeText(this,"Captured successfully. Open Brief to see it.",Toast.LENGTH_LONG).show();try{startActivity(new Intent(this,PremiumHomeActivity.class));}catch(Throwable ignored){}finish();}
    }
}
