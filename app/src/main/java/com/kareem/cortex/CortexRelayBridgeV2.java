...
/** Authenticated process-local control/session bridge back to Cortex Relay. */
public final class CortexRelayBridgeV2 {
    ...
    static void registerAuthenticatedSession(Context context, CortexConnectorRegistryV1.Identity identity,
            Messenger relay, Messenger cortexReply, String capabilitiesJson, String selectedProtocol) {
    ...
    static void clearAuthenticatedSession(Context context, String connectorId) {
        ...
        if (context != null) context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit()
                .putBoolean("connected", false).putLong("disconnected_at", System.currentTimeMillis()).apply();
    }
    ...
    public static boolean isRelayV2Connected() { return hasAuthenticatedV2Session(SECOND_BRAIN); }

    public static boolean requestAction(Context context, String requestId, String logicalSignalId,
            String capabilityId, String inputText) {
    ...
    public static boolean updateMechanicalPolicy(Context context, long version, int retentionHours, JSONArray disabledNoiseRules) {
    ...
    private static boolean send(Context context, String connectorId, int what, JSONObject request,
        ...
        if (session == null || !CortexLocalBusProtocolV2.SIGNAL_PROTOCOL.equals(session.selectedProtocol)) {
            persistSendFailure(context, kind, "NO_V2_SESSION"); return false;
        }
        if (!supports(session, requiredCapability)) {
            persistSendFailure(context, kind, "CAPABILITY_NOT_ADVERTISED:" + requiredCapability); return false;
        }
        try {
    ...
    private static boolean supports(Session session, String capability) {
        ...
        try {
            ...
            for (int i = 0; i < caps.length(); i++) if (capability.equals(caps.optString(i, "").trim())) return true;
    ...
    static void recordControlResult(Context context, CortexConnectorRegistryV1.Identity identity, int what, String raw) {
        ...
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit()
                .putString("last_" + kind + "_result", result.toString())
                .putLong("last_" + kind + "_result_at", System.currentTimeMillis()).apply();
    ...
    static void recordSignal(Context context, CortexConnectorRegistryV1.Identity identity,
        ...
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit()
                .putString("last_v2_signal", snapshot.toString()).putLong("last_v2_signal_at", System.currentTimeMillis()).apply();
    }
    ...
    private static void persistSession(Context context, Session session) {
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit()
                .putBoolean("connected", true).putString("connector_id", session.connectorId)
                .putString("connector_package", session.packageName).putString("capabilities_json", session.capabilitiesJson)
                .putString("selected_protocol", session.selectedProtocol).putString("last_send_failure", "")
                .putLong("connected_at", session.connectedAt).apply();
    }

    private static void persistRequest(Context context, String kind, JSONObject request) {
        if (context == null) return;
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit().putString("last_request_kind", kind)
                .putString("last_request_json", request.toString()).putString("last_send_failure", "")
                .putLong("last_request_at", System.currentTimeMillis()).apply();
    }

    private static void persistSendFailure(Context context, String kind, String reason) {
        if (context == null) return;
        context.getSharedPreferences(PREFS, Context.MODE_PRIVATE).edit().putString("last_request_kind", kind)
                .putString("last_send_failure", reason).putLong("last_request_at", System.currentTimeMillis()).apply();
    }
    ...
    private static void logControl(Context context, CortexConnectorRegistryV1.Identity identity, String stage, JSONObject payload) {
        ...
        try {
            ...
            meta.put("connector_id", identity.connectorId); meta.put("connector_package", identity.packageName); meta.put("payload", payload);
    ...
    private static final class Session {
        ...
        Session(String connectorId, String packageName, Messenger relay, Messenger cortexReply,
            ...
            this.connectorId = connectorId; this.packageName = packageName; this.relay = relay; this.cortexReply = cortexReply;
            this.capabilitiesJson = capabilitiesJson; this.selectedProtocol = selectedProtocol; this.connectedAt = connectedAt;
        }
    }
}
