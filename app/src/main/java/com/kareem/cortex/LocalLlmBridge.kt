...
object LocalLlmBridge {
    ...
    private fun requireNativeAllowed() {
        if (StartupSafetyGate.active()) {
            throw IllegalStateException("Local LLM native bridge is quarantined in this recovery build")
        }
    }

    @JvmStatic
    fun runtimeInfo(): String {
        if (StartupSafetyGate.active()) return "native runtime quarantined"
        return Llama.getSystemInfo()
    }

    @JvmStatic
    fun selfTest(modelPath: String): SelfTestResult {
        if (StartupSafetyGate.active()) {
    ...
    @JvmStatic
    fun completeOnce(modelPath: String, prompt: String, systemPrompt: String, maxTokens: Int): CompletionResult {
        requireNativeAllowed()
        return runBlocking {
            val threads = min(4, max(2, Runtime.getRuntime().availableProcessors() - 2))
            val config = LlamaConfig(contextSize = 3072, threads = threads, gpuLayers = 0, temperature = 0.25f, topP = 0.9f, topK = 40)
