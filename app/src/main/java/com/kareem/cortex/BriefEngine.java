...
public final class BriefEngine {
    ...
    private static String build(VaultDb db,long window,String title){
        ...
        ArrayList<BrainOpenLoop> loops=SecondBrainEngine.openLoops(db,30);ArrayList<ReviewQueueStore.Item> reviews=ReviewQueueStore.pending(db,6);
        ...
        if(!reviews.isEmpty()){b.append("\nREVIEW\n");for(ReviewQueueStore.Item r:reviews){b.append("• Possible ").append(r.candidateKind.toLowerCase(Locale.US)).append(": ").append(r.title);if(r.reason!=null&&!r.reason.isEmpty())b.append(" — ").append(r.reason);b.append('\n');}}
