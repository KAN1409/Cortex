...
/** Filters Cortex evidence before an explicit Combined cloud route. Local-only or unknown evidence never leaves device. */
public final class CloudEvidencePolicy {
    ...
    /**
     * Explicit allow-list policy. Unknown/unclassified evidence is local-only by default.
     * This prevents a future importer/source type from silently becoming cloud-eligible.
     */
    public static boolean canSend(Context context,KnowledgeItem k){
        if(k==null)return false;String group=privacyGroup(k);if(group.isEmpty()||"unknown".equals(group))return false;return PrivacyPolicy.canUseCloud(context,group);
    }
    ...
    public static String privacyGroup(KnowledgeItem k){String type=n(k.type).toLowerCase(Locale.ROOT),source=n(k.source).toLowerCase(Locale.ROOT);
        ...
        if(type.contains("voice")||type.contains("audio")||source.contains("audio")||source.contains("voice")||"manual_recording".equals(source))return"audio";
        ...
        if("manual".equals(source)||"quick_capture".equals(source))return"manual";
        return"unknown";
    }
