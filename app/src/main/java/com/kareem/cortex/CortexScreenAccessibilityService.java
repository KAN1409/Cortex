...
public final class CortexScreenAccessibilityService extends AccessibilityService {
    ...
    public static final class RobotNode {
        public final String path,label,className,packageName;public final boolean enabled,editable,scrollable,derivedLabel;
        RobotNode(String p,String l,String c,String pkg,boolean e,boolean edit,boolean scroll,boolean derived){path=n(p);label=n(l);className=n(c);packageName=n(pkg);enabled=e;editable=edit;scrollable=scroll;derivedLabel=derived;}
    }
    ...
    private static void collectRobot(AccessibilityNodeInfo x,String path,List<RobotNode> out,int depth,boolean clickableOnly,boolean editableOnly){if(x==null||depth>28||out.size()>=180)return;boolean include=x.isVisibleToUser()&&!x.isPassword()&&((clickableOnly&&x.isClickable())||(editableOnly&&x.isEditable()));if(include){String direct=directLabel(x);boolean derived=direct.isEmpty();String lab=derived?descendantLabel(x,0,new int[]{0}):direct;if(lab.length()>180)lab=lab.substring(0,180)+"…";out.add(new RobotNode(path,lab,n(x.getClassName()==null?null:x.getClassName().toString()),n(x.getPackageName()==null?null:x.getPackageName().toString()),x.isEnabled(),x.isEditable(),x.isScrollable(),derived));}for(int i=0;i<x.getChildCount()&&out.size()<180;i++){AccessibilityNodeInfo child=x.getChild(i);if(child!=null)collectRobot(child,path.isEmpty()?String.valueOf(i):path+"/"+i,out,depth+1,clickableOnly,editableOnly);}}

    /**
     * Accessibility often marks both a layout container and its actual buttons clickable. When the
     * container has no own text/contentDescription we previously synthesized a huge descendant
     * label and the Robot treated the whole screen as one action. Keep the synthesized label only
     * when the node has no independently clickable descendant; explicit labels remain eligible.
     */
    private static List<RobotNode> pruneDerivedAncestors(List<RobotNode> raw){
        if(raw==null||raw.size()<2)return raw==null?Collections.emptyList():raw;ArrayList<RobotNode> out=new ArrayList<>();
        for(RobotNode p:raw){boolean ancestor=false;if(p.derivedLabel)for(RobotNode c:raw){if(c==p)continue;if(descendantPath(p.path,c.path)){ancestor=true;break;}}if(!ancestor)out.add(p);}
        return out;
    }
    private static boolean descendantPath(String parent,String child){String p=n(parent),c=n(child);if(c.isEmpty())return false;if(p.isEmpty())return true;return c.startsWith(p+"/");}
