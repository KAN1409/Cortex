...
/**
 * Authenticated Cortex endpoint for Relay. Local Bus V1 is the compatibility baseline; V2 is
 * selected only after an authenticated second_brain HELLO advertises CORTEX_SIGNAL_V2.
 */
public final class CortexLocalBusService extends Service {
    ...
    @Override public IBinder onBind(Intent intent) { return messenger == null ? null : messenger.getBinder(); }
    ...
    private boolean handle(Message msg) {
        ...
        if (msg.what == CortexLocalBusProtocolV1.MSG_HELLO) return handleHello(msg, data, identity);
        if (msg.what == CortexLocalBusProtocolV2.MSG_ACTION_RESULT || msg.what == CortexLocalBusProtocolV2.MSG_POLICY_RESULT) {
            if (!"second_brain".equals(identity.connectorId) || !CortexRelayBridgeV2.hasAuthenticatedV2Session(identity.connectorId)) {
                reply(msg, false, "V2_SESSION_REQUIRED", "Control result arrived outside negotiated Relay V2 session", "", 0);
                return true;
            }
            CortexRelayBridgeV2.recordControlResult(getApplicationContext(), identity, msg.what,
                    data.getString(CortexLocalBusProtocolV2.KEY_RESULT_JSON, ""));
            return true;
        }
        if (msg.what == CortexLocalBusProtocolV2.MSG_INGEST_V2) return handleV2Ingest(msg, data, identity);
        if (msg.what == CortexLocalBusProtocolV1.MSG_INGEST) return handleV1Ingest(msg, data, identity);
    ...
    private boolean handleHello(Message msg, Bundle data, CortexConnectorRegistryV1.Identity identity) {
        ...
        boolean selectV2 = "second_brain".equals(identity.connectorId) && CortexLocalBusProtocolV2.relayAdvertisesSignalV2(relayCapabilities);
        ...
        try {
            ...
            meta.put("connector_id", identity.connectorId); meta.put("package", identity.packageName);
            meta.put("sending_uid", msg.sendingUid); meta.put("selected_protocol", selectedProtocol);
            ...
            if (selectV2 && msg.replyTo != null && messenger != null) {
                CortexRelayBridgeV2.registerAuthenticatedSession(getApplicationContext(), identity, msg.replyTo, messenger,
    ...
    private boolean handleV1Ingest(Message msg, Bundle data, CortexConnectorRegistryV1.Identity identity) {
        ...
        try {
            event = CortexLocalBusProtocolV1.parseEvent(data.getString(CortexLocalBusProtocolV1.KEY_EVENT_JSON, ""));
            ...
            logInvalid(identity, e); reply(msg, false, "INVALID_EVENT", safe(e), "", 0); return true;
        }
    ...
    private boolean handleV2Ingest(Message msg, Bundle data, CortexConnectorRegistryV1.Identity identity) {
        ...
        try {
            v2 = CortexLocalBusProtocolV2.parseEvent(data.getString(CortexLocalBusProtocolV1.KEY_EVENT_JSON, ""));
            if (!identity.connectorId.equals(v2.connectorId)) throw new IllegalArgumentException("connector_id does not match caller identity");
            canonical = CortexLocalBusProtocolV2.toCanonicalV1(v2);
        } catch (Throwable e) {
            logInvalid(identity, e); reply(msg, false, "INVALID_V2_EVENT", safe(e), "", 0); return true;
        }
        return ingestCanonical(msg, identity, canonical, v2);
    }

    private boolean ingestCanonical(Message msg, CortexConnectorRegistryV1.Identity identity,
            CortexLocalBusProtocolV1.Event event, CortexLocalBusProtocolV2.Event v2Event) {
        VaultDb db = null;
        try {
            db = new VaultDb(getApplicationContext());
            ...
            if (CortexLocalBusStoreV1.alreadyAccepted(db, event.eventId)) {
                ...
                logDelivery(db, identity, event, "DUPLICATE_ACCEPTED", existing, v2Event == null ? "" : "protocol=CORTEX_SIGNAL_V2");
                if (v2Event != null) CortexRelayBridgeV2.recordSignal(getApplicationContext(), identity, v2Event, existing);
            ...
            logDelivery(db, identity, event, "RECEIVED", 0, v2Event == null ? "" : "protocol=CORTEX_SIGNAL_V2");
            ...
            if (result.accepted()) {
                ...
                logDelivery(db, identity, event, "ACCEPTED", result.signalId, v2Event == null ? "" : "protocol=CORTEX_SIGNAL_V2");
                if (v2Event != null) CortexRelayBridgeV2.recordSignal(getApplicationContext(), identity, v2Event, result.signalId);
                ...
                logDelivery(db, identity, event, result.status, result.signalId, v2Event == null ? "rejected" : "rejected; protocol=CORTEX_SIGNAL_V2");
            ...
            try {
                if (db != null) {
                    ...
                    DiagnosticsLog.error(db, "local_bus", "ingest", e, "INGEST_FAILED", 0, 0, 0, 0, 0, deliveryMeta(identity, event));
                }
    ...
    private void logDelivery(VaultDb db, CortexConnectorRegistryV1.Identity identity,
        ...
        try {
            JSONObject meta = deliveryMeta(identity, event); meta.put("detail", detail);
            DiagnosticsLog.info(db, "local_bus", "ingest_" + status.toLowerCase(), status, 0, 0, signalId, 0, 0, 0, meta);
        } catch (Throwable ignored) {}
    }
    ...
    private JSONObject deliveryMeta(CortexConnectorRegistryV1.Identity identity, CortexLocalBusProtocolV1.Event event) {
        JSONObject meta = new JSONObject();
        try {
            meta.put("connector_id", identity.connectorId); meta.put("connector_package", identity.packageName);
            meta.put("event_id", event.eventId); meta.put("source_type", event.sourceType);
            meta.put("source_package", event.sourcePackage); meta.put("occurred_at", event.occurredAt);
    ...
    private void logInvalid(CortexConnectorRegistryV1.Identity identity, Throwable e) {
        VaultDb db = null;
        try {
            db = new VaultDb(getApplicationContext()); JSONObject meta = new JSONObject();
            meta.put("connector_id", identity.connectorId); meta.put("connector_package", identity.packageName);
            DiagnosticsLog.error(db, "local_bus", "parse_event", e, "INVALID_EVENT", 0, 0, 0, 0, 0, meta);
        } catch (Throwable ignored) {} finally { close(db); }
    }

    private void logUnauthorised(int uid) {
        VaultDb db = null;
        try {
            db = new VaultDb(getApplicationContext()); JSONObject meta = new JSONObject(); meta.put("sending_uid", uid);
            DiagnosticsLog.warn(db, "local_bus", "caller_rejected", "unauthorized", "UNAUTHORIZED_CALLER", 0, 0, 0, 0, 0, meta);
        } catch (Throwable ignored) {} finally { close(db); }
    }

    private long existingSignalId(VaultDb db, String eventId) {
        android.database.Cursor c = db.getReadableDatabase().rawQuery(
                "SELECT signal_id FROM connector_ingest_events WHERE event_id=? LIMIT 1", new String[]{eventId});
    ...
    private void replyHello(Message incoming, boolean ok, String status, String detail, String selectedProtocol) {
        ...
        try {
            ...
            Bundle b = new Bundle(); b.putString(CortexLocalBusProtocolV1.KEY_STATUS, clean(status));
            b.putString(CortexLocalBusProtocolV1.KEY_DETAIL, clean(detail)); b.putString(CortexLocalBusProtocolV1.KEY_EVENT_ID, "");
            b.putLong(CortexLocalBusProtocolV1.KEY_SIGNAL_ID, 0L); b.putString(CortexLocalBusProtocolV2.KEY_SELECTED_PROTOCOL, clean(selectedProtocol));
            out.setData(b); incoming.replyTo.send(out);
        } catch (Throwable ignored) {}
    }
    ...
    private void reply(Message incoming, boolean ok, String status, String detail, String eventId, long signalId) {
        ...
        try {
            Message out = Message.obtain(null, ok ? CortexLocalBusProtocolV1.MSG_ACK : CortexLocalBusProtocolV1.MSG_ERROR);
            Bundle b = new Bundle(); b.putString(CortexLocalBusProtocolV1.KEY_STATUS, clean(status));
            b.putString(CortexLocalBusProtocolV1.KEY_DETAIL, clean(detail)); b.putString(CortexLocalBusProtocolV1.KEY_EVENT_ID, clean(eventId));
            b.putLong(CortexLocalBusProtocolV1.KEY_SIGNAL_ID, signalId); out.setData(b); incoming.replyTo.send(out);
        } catch (Throwable ignored) {}
    }
