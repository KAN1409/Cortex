...
public final class AudioAnalyzer {
        ...
        try{
            ...
            if(gemini){
                GeminiAudioTranscriber.transcribe(ctx,f,new GeminiAudioTranscriber.Callback(){
                    public void ok(TranscriptResult g){String warning=acceptabilityWarning(g);if(warning==null){prepareGeminiPrimary(g,groq);finish(ctx,g,cb);}else if(groq)runGroqFallback(ctx,f,cb,"Gemini rejected: "+warning);else cb.fail(retryable("Gemini returned an incomplete transcript: "+warning,null));}
    ...
    private static void finish(Context ctx,TranscriptResult t,Callback cb){
        try{
            ...
            String corrected=CorrectionEngine.apply(ctx,t.text);
            r.extractedText=LanguageBlockFormatter.format(corrected);
            r.summary=LanguageBlockFormatter.format(MixedBidiText.stripControls(r.summary));
            ...
            for(TranscriptResult.Segment s:t.segments)r.transcriptSegments.add(new AnalysisResult.TranscriptSegment(s.startMs,s.endMs,LanguageBlockFormatter.format(MixedBidiText.stripControls(s.text)),s.confidence));
