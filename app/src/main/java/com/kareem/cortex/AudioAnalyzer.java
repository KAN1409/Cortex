...
public final class AudioAnalyzer {
    ...
    private static void enrichWithGemini(TranscriptResult groq,TranscriptResult gemini,String geminiError){
        try{
            ...
            if(gemini!=null){
                JSONObject j=new JSONObject();j.put("label","gemini_2_5_flash");j.put("provider","gemini");j.put("engine",gemini.engine);j.put("language",gemini.language);j.put("score",round1(geminiBenchmarkScore(gemini)));j.put("warning",acceptabilityWarning(gemini)==null?"":acceptabilityWarning(gemini));j.put("coverage",0);j.put("coverage_note","Gemini generateContent does not return ASR segment timestamps; full audio was supplied");j.put("arabic_ratio",round3(scriptRatio(gemini.text,true)));j.put("latin_ratio",round3(scriptRatio(gemini.text,false)));j.put("text",gemini.text);j.put("raw_text",gemini.rawTranscript);arr.put(j);
                root.put("gemini_raw_provider",new JSONObject(gemini.rawProviderResponse));
            }else if(geminiError!=null)root.put("gemini_error",geminiError);
            root.put("benchmark_mode","Groq Whisper candidates + Gemini 2.5 Flash side-by-side; Groq winner remains selected in v24 unless Groq fails");
