...
public final class InboxPresentation {
    ...
    public static ArrayList<String> actionParts(ArrayList<String> raw){
        ...
        for(String source:raw){
            ...
            // Break only at semantic clause boundaries, not language switches.
            s=s.replaceAll("\\s+[•|]\\s+",". ");
            ...
            for(String c:clauses){
                ...
                if(!containsNear(out,x))out.add(concise(x,130));
            }
        }
        if(out.size()>4)return new ArrayList<>(out.subList(0,4));
        return out;
    }
    ...
    public static String due(ArrayList<String> raw){
        if(raw==null)return "";
        for(String s:raw){
            int i=nz(s).toLowerCase(Locale.US).lastIndexOf("due:");
            if(i>=0){String d=nz(s).substring(i+4).replace("•","").trim();if(!d.isEmpty())return d;}
        }
        return "";
    }
    ...
    public static String preview(KnowledgeItem k,ArrayList<String> actions){
        if(k==null)return "";
        String p=!blank(k.summary)?k.summary:(!blank(k.extractedText)?k.extractedText:k.rawText);
        p=cleanup(nz(p));
        if(actions!=null&&!actions.isEmpty()){
            // When structured actions exist, context should stay short and avoid
            // repeating the same full task paragraph beneath the action list.
            int stop=sentenceEnd(p);
            if(stop>25&&stop<170)p=p.substring(0,stop+1);
            return concise(p,170);
        }
        return concise(p,300);
    }

    private static String stripDue(String s){
        int i=s.toLowerCase(Locale.US).lastIndexOf("due:");
        if(i>=0){int bullet=s.lastIndexOf('•',i);s=s.substring(0,bullet>=0?bullet:i);}
        return cleanup(s);
    }
    private static int sentenceEnd(String s){
        int best=-1;for(char c:new char[]{'.','؟','!','\n'}){int i=s.indexOf(c);if(i>=0&&(best<0||i<best))best=i;}return best;
    }
