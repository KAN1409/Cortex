...
public class FeatureHubActivity extends Activity {
    ...
    void syncCalendar(){if(!PrivacyPolicy.canCollect(this,"calendar")){Toast.makeText(this,"Calendar is set to Never collect",Toast.LENGTH_LONG).show();return;}if(Build.VERSION.SDK_INT>=23&&checkSelfPermission(Manifest.permission.READ_CALENDAR)!=PackageManager.PERMISSION_GRANTED){requestPermissions(new String[]{Manifest.permission.READ_CALENDAR},REQ_CAL);return;}doCalendar();}
    void syncContacts(){if(!PrivacyPolicy.canCollect(this,"contacts")){Toast.makeText(this,"Contacts are set to Never collect",Toast.LENGTH_LONG).show();return;}if(Build.VERSION.SDK_INT>=23&&checkSelfPermission(Manifest.permission.READ_CONTACTS)!=PackageManager.PERMISSION_GRANTED){requestPermissions(new String[]{Manifest.permission.READ_CONTACTS},REQ_CONTACTS);return;}doContacts();}
    void doCalendar(){if(!PrivacyPolicy.canCollect(this,"calendar"))return;Toast.makeText(this,"Importing calendar…",Toast.LENGTH_SHORT).show();new Thread(()->{try{int n=IntegrationEngine.importCalendar(this,db);runOnUiThread(()->Toast.makeText(this,n+" events imported",Toast.LENGTH_LONG).show());}catch(Exception e){runOnUiThread(()->Toast.makeText(this,"Calendar import failed",Toast.LENGTH_LONG).show());}}).start();}
    void doContacts(){if(!PrivacyPolicy.canCollect(this,"contacts"))return;Toast.makeText(this,"Importing contacts…",Toast.LENGTH_SHORT).show();new Thread(()->{try{int n=IntegrationEngine.importContacts(this,db);runOnUiThread(()->Toast.makeText(this,n+" contacts imported",Toast.LENGTH_LONG).show());}catch(Exception e){runOnUiThread(()->Toast.makeText(this,"Contacts import failed",Toast.LENGTH_LONG).show());}}).start();}
    void pickFile(){if(!PrivacyPolicy.canCollect(this,"files")){Toast.makeText(this,"Files are set to Never collect",Toast.LENGTH_LONG).show();return;}Intent i=new Intent(Intent.ACTION_OPEN_DOCUMENT);i.addCategory(Intent.CATEGORY_OPENABLE);i.setType("*/*");startActivityForResult(i,REQ_FILE);}
