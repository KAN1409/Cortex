...
public final class CortexActionExecutor {
    ...
    public interface ProjectSelectionCallback { void onLinked(long projectId); void onCancelled(); }
    ...
    /** Compatibility overload retained for existing detail surfaces. */
    public static void chooseProject(Activity a,VaultDb db,long itemId,Runnable onLinked){
        chooseProject(a,db,itemId,new ProjectSelectionCallback(){
            @Override public void onLinked(long projectId){if(onLinked!=null)onLinked.run();}
            @Override public void onCancelled(){}
        });
    }

    /** User-confirmed local project link. Callback receives the exact project needed for receipt verification. */
    public static void chooseProject(Activity a,VaultDb db,long itemId,ProjectSelectionCallback callback){
        if(a==null||db==null||itemId<=0){if(callback!=null)callback.onCancelled();return;}ArrayList<Long> ids=new ArrayList<>();ArrayList<String> names=new ArrayList<>();
        try{Cursor c=db.getReadableDatabase().rawQuery("SELECT n.id,n.canonical_name FROM entity_nodes n WHERE n.status='active' AND upper(n.kind)='PROJECT' AND COALESCE(n.metadata_json,'') LIKE '%\"created_from\":\"project_candidate\"%' ORDER BY n.updated_at DESC LIMIT 100",null);while(c.moveToNext()){ids.add(c.getLong(0));names.add(safe(c.getString(1)));}c.close();}catch(Throwable e){toast(a,"Could not load projects");if(callback!=null)callback.onCancelled();return;}
        if(ids.isEmpty()){toast(a,"No confirmed projects yet");if(callback!=null)callback.onCancelled();return;}
        new AlertDialog.Builder(a).setTitle("Add this capture to project").setItems(names.toArray(new String[0]),(d,which)->{
            long projectId=ids.get(which);boolean ok=false;
            try{ok=CognitiveStore.linkChecked(db,"memory",itemId,"entity",projectId,"related_project",1.0,new JSONObject().put("user_confirmed",true).toString());if(ok)CognitiveStore.feedback(db,"memory",itemId,"linked_project",new JSONObject().put("project_id",projectId).toString(),"action_executor_002");}catch(Throwable ignored){}
            toast(a,ok?"Linked to "+names.get(which):"Could not link project");if(ok&&callback!=null)callback.onLinked(projectId);else if(!ok&&callback!=null)callback.onCancelled();
        }).setNegativeButton("Cancel",(d,w)->{if(callback!=null)callback.onCancelled();}).setOnCancelListener(d->{if(callback!=null)callback.onCancelled();}).show();
    }
