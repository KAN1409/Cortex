...
public final class EntityGraphMaintenance {
    public static final String VERSION="entity_graph_guard_001";
    private EntityGraphMaintenance(){}

    public static void run(VaultDb db){
        if(db==null)return;
        SQLiteDatabase s=db.getWritableDatabase();long now=System.currentTimeMillis();int peopleQuarantined=0,projectsQuarantined=0,identified=0;
        s.beginTransaction();
        try{
        ...
        try{JSONObject m=new JSONObject();m.put("people_quarantined",peopleQuarantined);m.put("projects_quarantined",projectsQuarantined);m.put("identified_contact_nodes",identified);m.put("policy",VERSION);DiagnosticsLog.info(db,"EntityGraphMaintenance","identity_guard","ok",0,0,0,0,0,0,m);}catch(Throwable ignored){}
    }
    ...
    private static String cleanContactName(String x){String s=n(x).replaceAll("(?i)\\s+(?:facebook\\s+|web\\s+)?phone$","").replaceAll("\\s+"," ").trim();return s;}
