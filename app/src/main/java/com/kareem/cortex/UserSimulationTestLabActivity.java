...
/** In-app launcher for quick repair regressions or strict full/autopilot release verification. */
public class UserSimulationTestLabActivity extends Activity {
    ...
    void build(){
        ...
        summary=CortexUi.text(this,"Termux Autopilot mode grants eligible Android permissions with rish, runs full verification, consumes live results into the master matrix, and keeps any truly unavailable OS/provider requirement explicit instead of fabricating PASS.",10,CortexUi.FAINT);summary.setTextIsSelectable(true);summary.setPadding(dp(2),dp(18),dp(2),0);body.addView(summary);
    ...
    void startRun(){if(running)return;latest=null;setBusy(true);run.setText("Running full exhaustive…");status.setText("Starting");detail.setText("Preparing strict user simulation");progress.setProgress(5);final boolean autopilot=getIntent()!=null&&getIntent().getBooleanExtra("autopilot",false);final boolean fullSoak=getIntent()==null||getIntent().getBooleanExtra("full_soak",true);
        new Thread(()->{try{
            CortexUserSimulationRunner.Result base=CortexUserSimulationRunner.run(getApplicationContext(),(pct,phase,msg)->ui(()->{progress.setProgress(autopilot?Math.min(520,pct*5):Math.min(940,pct*9));status.setText(phase);detail.setText(msg);}));
            latest=base.file;
            if(autopilot){ui(()->{status.setText("Autopilot live verification");detail.setText(fullSoak?"Running safe live tests + real 30-minute stability soak":"Running safe live tests");progress.setProgress(525);});CortexAutopilotLiveRunner.run(getApplicationContext(),fullSoak,(pct,phase,msg)->ui(()->{progress.setProgress(Math.min(925,525+pct*4));status.setText(phase);detail.setText(msg);}));}
            ui(()->{status.setText("Exhaustive matrix");detail.setText("Running strict assertions + repair regressions + consuming live results");progress.setProgress(950);});
