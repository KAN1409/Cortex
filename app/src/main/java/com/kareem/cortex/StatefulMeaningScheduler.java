...
/** Guarantees one stateful meaning drain at a time; retries are idempotent and best-effort. */
public final class StatefulMeaningScheduler {
    ...
    public static void kick(Context c){
        ...
        try{
            ...
            OneTimeWorkRequest r=new OneTimeWorkRequest.Builder(StatefulMeaningWorker.class)
                    .setConstraints(x)
                    .setBackoffCriteria(BackoffPolicy.EXPONENTIAL,15,TimeUnit.SECONDS)
                    .addTag(UNIQUE_WORK)
                    .build();
            WorkManager.getInstance(app).enqueueUniqueWork(UNIQUE_WORK,ExistingWorkPolicy.KEEP,r);
            CapabilitySupervisor.recordHealthy(app,CapabilitySupervisor.Capability.BACKGROUND_SCHEDULING);
        }catch(Throwable t){
            // Scheduling failure must never turn a successfully captured observation into a capture failure.
