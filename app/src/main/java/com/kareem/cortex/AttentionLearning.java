...
public final class AttentionLearning {
    public static final String VERSION="attention_learning_003";
    ...
    public static AttentionEngine.Decision apply(VaultDb db,PrimeBriefStore.Item item,AttentionEngine.Decision d){
        if(db==null||item==null||d==null||item.id<=0)return d;CognitiveStore.ensure(db);int positive=0,negative=0,snooze=0,acted=0,evalUp=0,evalDown=0;long snoozedUntil=0,dismissedAt=0;
        ...
        int delta=Math.min(8,positive*2)-Math.min(18,negative*6)-Math.min(10,snooze*4)+Math.min(18,evalUp*6)-Math.min(18,evalDown*6);if(delta==0)return d;int score=Math.max(0,Math.min(100,d.score+delta));return new AttentionEngine.Decision(score,band(score),learningReason(d.whyNow,positive,negative,snooze,evalUp,evalDown),d.urgency,d.consequence,d.responsibility,d.temporalPressure,d.openLoopPressure,d.novelty,d.confidence);
    }
    ...
    private static String learningReason(String original,int positive,int negative,int snooze,int evalUp,int evalDown){if(evalDown>0)return original+" Your evaluation says Cortex has been ranking this too high.";if(evalUp>0)return original+" Your evaluation says Cortex has been ranking this too low.";if(negative>0)return original+" Personal feedback has reduced its attention priority.";if(snooze>0)return original+" You previously snoozed this, so Cortex is surfacing it more cautiously.";if(positive>0)return original+" Your prior interactions suggest this is useful to surface.";return original;}
