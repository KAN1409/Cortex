...
public final class VisualRecoveryFixture {
    ...
    public static String verify(VaultDb db)throws Exception{
        ...
        String token="visual-recovery-fixture-"+System.nanoTime();try{
            ...
            // Provider setup is a pause, not another retry or a terminal conversion. Keep the ledger
            // intact, but exact retry-now must refuse to advance the item while waiting_provider.
            VisualInsightStore.saveState(db,id,"waiting_provider","safe","Synthetic provider setup pause");int pausedRetry=VisualRecoveryStore.retryRecoverableNow(db,id);VisualRecoveryStore.State paused=VisualRecoveryStore.get(db,id);if(pausedRetry!=0||paused==null||paused.attempts!=1||!paused.recoverable)throw new AssertionError("waiting-provider state spent or lost retry history");if(paused.createdAt!=originalCreated)throw new AssertionError("waiting-provider pause changed recovery creation time");
            VisualInsightStore.saveState(db,id,"retry_wait","safe","Synthetic provider restored");
            ...
            int reset=VisualRecoveryStore.resetTerminalBudget(db,id);VisualRecoveryStore.State fresh=VisualRecoveryStore.get(db,id);if(reset!=1||fresh==null||!fresh.recoverable||fresh.attempts!=0||!"explicit_reset".equals(fresh.failureKind))throw new AssertionError("terminal reset did not create zero-attempt fresh budget");if(fresh.nextRetryAt<=0)throw new AssertionError("fresh terminal-reset budget is not retry-eligible");VisualInsightStore.Insight v=VisualInsightStore.get(db,id);if(v==null||!"failed".equals(v.status))throw new AssertionError("terminal reset lost retry-eligible visual state");Cursor pc=sql.rawQuery("SELECT pipeline_version FROM visual_insights WHERE item_id=?",new String[]{String.valueOf(id)});int pipeline=pc.moveToFirst()?pc.getInt(0):VisualInsightStore.PIPELINE_VERSION;pc.close();if(pipeline>=VisualInsightStore.PIPELINE_VERSION)throw new AssertionError("terminal reset did not mark the item for pipeline retry");
            ...
            return"exact item only · waiting-provider pause preserves attempt 1 · retry-now stays 1→2→terminal 3 · reset→0→first real failure 1 · created_at stable until reset · rollback";
