...
public final class WorkProcurementLinker {
    public static final String VERSION="work_procurement_linker_004";
    ...
    private static ArrayList<Ref> refsForFile(SQLiteDatabase db,long fileId){
        ArrayList<Ref> out=new ArrayList<>();Cursor c=db.rawQuery("SELECT r.id,r.ref_type,r.normalized_value FROM work_procurement_refs r JOIN work_files f ON f.id=r.file_id WHERE r.file_id=? AND f.active_version_id>0 AND r.version_id=f.active_version_id ORDER BY r.id",new String[]{String.valueOf(fileId)});while(c.moveToNext())out.add(new Ref(c.getLong(0),s(c,1),s(c,2)));c.close();return out;
    }
    private static ArrayList<Follow> followForFile(SQLiteDatabase db,long fileId){
        ArrayList<Follow> out=new ArrayList<>();Cursor c=db.rawQuery("SELECT u.id,u.reference_type,u.reference_value,u.project_id FROM work_followup_records u JOIN work_files f ON f.id=u.file_id WHERE u.file_id=? AND f.active_version_id>0 AND u.version_id=f.active_version_id ORDER BY u.id",new String[]{String.valueOf(fileId)});while(c.moveToNext())out.add(new Follow(c.getLong(0),s(c,1),s(c,2),c.getLong(3)));c.close();return out;
    }
    private static ArrayList<Price> pricesForFile(SQLiteDatabase db,long fileId){ArrayList<Price> out=new ArrayList<>();Cursor c=db.rawQuery("SELECT p.id FROM work_price_records p JOIN work_files f ON f.id=p.file_id WHERE p.file_id=? AND f.active_version_id>0 AND p.version_id=f.active_version_id ORDER BY p.id",new String[]{String.valueOf(fileId)});while(c.moveToNext())out.add(new Price(c.getLong(0)));c.close();return out;}
    ...
    private static String normRef(String x){return WorkStructuredExtractor.normalizeReference(x);}
