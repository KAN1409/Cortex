...
public final class AdjudicationRecovery {
    ...
    private static int cognitiveAttempts(SQLiteDatabase sql,long signalId){Cursor c=sql.rawQuery("SELECT COUNT(*) FROM model_runs WHERE role=? AND input_hash=?",new String[]{CognitiveRunStoreV2.role(),CognitiveRunStoreV2.signalKey(signalId)});try{return c.moveToFirst()?c.getInt(0):0;}finally{c.close();}}
