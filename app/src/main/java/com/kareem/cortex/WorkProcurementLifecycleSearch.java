...
public final class WorkProcurementLifecycleSearch {
    ...
    public static void append(SQLiteDatabase db,String query,List<WorkVaultSearch.Hit> out,int max){
        ...
        Cursor c=db.rawQuery("SELECT id,ref_type,ref_value,file_id,project_id FROM work_procurement_refs WHERE normalized_value LIKE ? ORDER BY confidence DESC,id DESC LIMIT 20",new String[]{"%"+token+"%"});
    ...
    private static void appendForRef(SQLiteDatabase db,long refId,List<WorkVaultSearch.Hit> out,int max){
        Cursor c=db.rawQuery("SELECT from_kind,from_id,to_kind,to_id,relation,confidence,evidence_rule,source_file_id FROM work_procurement_links WHERE (from_kind='REF' AND from_id=?) OR (to_kind='REF' AND to_id=?) ORDER BY confidence DESC,id DESC LIMIT 40",new String[]{String.valueOf(refId),String.valueOf(refId)});
