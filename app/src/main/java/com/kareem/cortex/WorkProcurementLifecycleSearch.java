...
public final class WorkProcurementLifecycleSearch {
    ...
    private static WorkVaultSearch.Hit fileHit(SQLiteDatabase db,long fileId,String relation,String rule,double conf){
        ...
        Cursor c=db.rawQuery("SELECT f.id,f.display_name,f.document_uri,p.document_type,p.confidence FROM work_files f LEFT JOIN work_document_profiles p ON p.file_id=f.id WHERE f.id=? LIMIT 1",new String[]{String.valueOf(fileId)});
