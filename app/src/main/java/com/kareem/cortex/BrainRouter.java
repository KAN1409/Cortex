...
public final class BrainRouter {
    ...
    private static LocalAskRouter.Result cloud(Context ctx,VaultDb db,String question,String mode,LocalAskRouter.Progress progress){
        long wall=SystemClock.elapsedRealtime();long job=createJob(db,question,mode);boolean combined="combined".equals(mode);GroundedAnswer grounded=emptyGrounding(question);long retrieval=0;
        try{
            AiJobStore.start(db,job,"Understanding request","understanding");emit(progress,job,"Understanding request",8);
            if(combined){AiJobStore.progress(db,job,"Searching your Cortex","retrieval",24,"Selecting private evidence for Combined mode");emit(progress,job,"Searching your Cortex",24);long rt=SystemClock.elapsedRealtime();grounded=SecondBrainEngine.ask(db,question);retrieval=SystemClock.elapsedRealtime()-rt;AiJobStore.linkSources(db,job,grounded);}
            AiJobStore.progress(db,job,combined?"Combining sources":"Using external AI","external",combined?48:36,combined?"Sending selected evidence to configured Gemini":"No Cortex memory is sent in External mode");emit(progress,job,combined?"Combining sources":"Using external AI",combined?48:36);
            ...
            JSONObject out=new JSONObject().put("answer",x.text).put("provider","gemini").put("source_mode",mode).put("source_count",grounded.sources.size()).put("total_ms",total);
            AiJobStore.modelRun(db,job,1,"primary","cloud","gemini-3.6-flash",mode,"complete",Fingerprint.text(question+"|"+mode),x.durationMs,0,0,0.78,new JSONObject().put("source_count",grounded.sources.size()).toString(),"");
            AiJobStore.complete(db,job,out.toString(),"Answer ready",combined?"Combined private evidence + external AI":"External AI answer; no Cortex memory sent");emit(progress,job,"Answer ready",100);
