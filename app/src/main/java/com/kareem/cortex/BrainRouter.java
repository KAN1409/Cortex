...
public final class BrainRouter {
    ...
    private static LocalAskRouter.Result cloud(Context ctx,VaultDb db,String question,String mode,long focalItemId,LocalAskRouter.Progress progress){
        long wall=SystemClock.elapsedRealtime();long job=createJob(db,question,mode,focalItemId);boolean combined="combined".equals(mode);GroundedAnswer grounded=emptyGrounding(question);long retrieval=0;int privateFound=0;KnowledgeItem focal=null,cloudFocal=null;
        try{
            ...
            ExternalBrainProvider.Result x=ExternalBrainProvider.ask(ctx,question,grounded,combined,cloudFocal);long total=SystemClock.elapsedRealtime()-wall;
            JSONObject out=new JSONObject().put("answer",x.text).put("provider","gemini").put("model",x.model).put("source_mode",mode).put("source_count",grounded.sources.size()).put("private_found",privateFound).put("focal_item_id",focalItemId).put("focal_sent",cloudFocal!=null).put("withheld_local_only",Math.max(0,privateFound-grounded.sources.size())).put("total_ms",total);
            AiJobStore.modelRun(db,job,1,"primary","cloud",x.model,mode,"complete",Fingerprint.text(question+"|"+mode+"|"+focalItemId),x.durationMs,0,0,0.78,new JSONObject().put("source_count",grounded.sources.size()).put("focal_sent",cloudFocal!=null).put("withheld_local_only",Math.max(0,privateFound-grounded.sources.size())).toString(),"");
            ...
            try{DiagnosticsLog.info(db,"BrainRouter","external_answer","ok",focalItemId,0,0,job,0,total,new JSONObject().put("model",x.model).put("mode",mode).put("focal_sent",cloudFocal!=null));}catch(Throwable ignored){}
            ...
            long total=SystemClock.elapsedRealtime()-wall;String err=t.getClass().getSimpleName()+(t.getMessage()==null?"":": "+t.getMessage());AiJobStore.fail(db,job,err,"External route unavailable");try{DiagnosticsLog.error(db,"BrainRouter","external_route",t,"EXTERNAL_MODEL",focalItemId,0,0,job,0,new JSONObject().put("mode",mode).put("model",GeminiModelConfig.generationModel(ctx)));}catch(Throwable ignored){}
            if(combined){
                try{
                    emit(progress,job,"External unavailable · using your Cortex",72);LocalAskRouter.Result local=LocalAskRouter.fast(ctx,db,question,progress);String answer="External AI is unavailable right now, so Brain answered from your Cortex data only.\n\n"+local.answer;return new LocalAskRouter.Result(local.jobId,local.grounded,answer,"combined-local-fallback",err,"combined",local.tokensPerSecond,local.tokensGenerated,local.durationMs,SystemClock.elapsedRealtime()-wall,local.retrievalMs,local.promptBuildMs,local.modelLoadMs,local.generationMs,local.cacheHit);
