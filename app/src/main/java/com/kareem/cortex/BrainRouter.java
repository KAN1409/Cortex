...
public final class BrainRouter {
    ...
    private static LocalAskRouter.Result cloud(Context ctx,VaultDb db,String question,String mode,long focalItemId,LocalAskRouter.Progress progress){
        long wall=SystemClock.elapsedRealtime();long job=createJob(db,question,mode,focalItemId);boolean combined="combined".equals(mode);GroundedAnswer grounded=emptyGrounding(question);long retrieval=0;int privateFound=0;KnowledgeItem focal=null,cloudFocal=null;String phoneContext="";boolean phoneSent=false;
        try{
            ...
            if(combined){
                ...
                if(PrivacyPolicy.canCollect(ctx,"phone_context")){try{PhoneContextStore.ensure(db);phoneContext=PhoneContextStore.recentSummary(db,30L*60L*1000L,8);phoneSent=!phoneContext.isEmpty()&&PrivacyPolicy.canUseCloud(ctx,"phone_context");}catch(Throwable ignored){phoneContext="";phoneSent=false;}}
            }
            ...
            AiJobStore.modelRun(db,job,1,"primary","cloud",x.model,mode,"complete",Fingerprint.text(question+"|"+mode+"|"+focalItemId),x.durationMs,0,0,0.78,new JSONObject().put("source_count",grounded.sources.size()).put("focal_sent",cloudFocal!=null).put("phone_context_sent",phoneSent).put("withheld_local_only",Math.max(0,privateFound-grounded.sources.size())).toString(),"");
            ...
            try{DiagnosticsLog.info(db,"BrainRouter","external_answer","ok",focalItemId,0,0,job,0,total,new JSONObject().put("model",x.model).put("mode",mode).put("focal_sent",cloudFocal!=null).put("phone_context_sent",phoneSent));}catch(Throwable ignored){}
