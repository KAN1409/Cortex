...
public final class CortexAttentionOrchestrator {
    public static final String VERSION="attention_orchestrator_002";
    ...
    public static void onSignalCaptured(VaultDb db,long signalId,long threadId){
        if(db==null||signalId<=0)return;try{CortexAttentionSchema.ensure(db);AttentionSemantics.Result s=AttentionSemantics.extract(db,signalId,threadId);if(s==null)return;long effectiveThread=s.threadId;
            ...
            if(existing!=null&&s.cancellation){OpenLoopStore.cancel(db,existing.id,signalId,Math.max(.80,s.confidence));AttentionFeedStore.removeLoop(db,existing.id);persistAssessmentRemoval(db,existing.id);return;}
            ...
            if(existing!=null&&isLikelyResolution(s,existing)){OpenLoopStore.resolve(db,existing.id,signalId,"matching_outgoing_action",Math.max(.78,s.confidence));AttentionFeedStore.removeLoop(db,existing.id);persistAssessmentRemoval(db,existing.id);return;}
