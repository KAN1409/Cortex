...
public final class SignalThreadStore {
    ...
    private static void archiveStaleBaseThread(VaultDb db,String kind,String source,String external,long occurredAt,long now){
        Cursor c=db.getReadableDatabase().query("signal_threads",new String[]{"id","last_event_at","state"},"kind=? AND source=? AND external_key=?",new String[]{kind,n(source),external},null,null,"last_event_at DESC","1");
    ...
    public static String recentContext(VaultDb db,long threadId,int maxSignals){
        if(threadId<=0)return"";int lim=Math.max(1,Math.min(20,maxSignals));Cursor c=db.getReadableDatabase().query("raw_signals",new String[]{"title","body"},"thread_id=?",new String[]{String.valueOf(threadId)},null,null,"occurred_at DESC",String.valueOf(lim));ArrayList<String> parts=new ArrayList<>();
