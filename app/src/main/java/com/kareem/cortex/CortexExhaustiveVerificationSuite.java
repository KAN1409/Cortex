...
public final class CortexExhaustiveVerificationSuite {
    ...
    public static JSONObject run(Context context,VaultDb db,String runId)throws Exception{
        ...
        try{
            ...
            PhoneContextStore.ensure(db);long pc=PhoneContextStore.record(db,"app_transition","x_suite",c.getPackageName(),"Cortex","InputActivity","test",token,System.currentTimeMillis(),new JSONObject().put("run_id",runId));PhoneContextStore.Event pcRead=PhoneContextStore.byId(db,pc);boolean pcFound=pcRead!=null&&pcRead.id==pc&&token.equals(pcRead.text);check(a,"Phone context","Round-trip","Phone-context event must read back by exact row id",pcFound,new JSONObject().put("id",pc).put("found",pcFound).put("readback_id",pcRead==null?0:pcRead.id).put("readback_text",pcRead==null?"":pcRead.text));
