...
public final class ProjectCandidateStore {
    ...
    public static boolean confirm(VaultDb db,long candidateId){
        ...
        String name=EntityQualityPolicy.cleanProjectName(projectName(title,body));if(!EntityQualityPolicy.plausibleProject(name))return false;long now=System.currentTimeMillis();String key="project|"+LocalSemanticEmbedder.norm(name);String meta="{\"created_from\":\"project_candidate\",\"candidate_id\":"+candidateId+",\"explicit_user_action\":true}";
        ContentValues v=new ContentValues();v.put("kind","PROJECT");v.put("canonical_name",name);v.put("normalized_key",key);v.put("status","active");v.put("metadata_json",meta);v.put("created_at",now);v.put("updated_at",now);
        ...
        ContentValues promote=new ContentValues();promote.put("kind","PROJECT");promote.put("canonical_name",name);promote.put("status","active");promote.put("metadata_json",meta);promote.put("updated_at",now);sql.update("entity_nodes",promote,"id=?",new String[]{String.valueOf(entity)});
