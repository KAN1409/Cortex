...
public final class ProjectCandidateStore {
    ...
    public static boolean confirm(VaultDb db,long candidateId){
        ...
        String name=projectName(title,body);if(name.isEmpty())return false;long now=System.currentTimeMillis();String key="project|"+LocalSemanticEmbedder.norm(name);
        ContentValues v=new ContentValues();v.put("kind","PROJECT");v.put("canonical_name",name);v.put("normalized_key",key);v.put("status","active");v.put("metadata_json","{\"created_from\":\"project_candidate\",\"candidate_id\":"+candidateId+"}");v.put("created_at",now);v.put("updated_at",now);
        ...
        ContentValues done=new ContentValues();done.put("state","confirmed");done.put("resolved_at",now);done.put("updated_at",now);sql.update("derived_items",done,"id=? AND kind='PROJECT_CANDIDATE'",new String[]{String.valueOf(candidateId)});
