...
public class PremiumHomeActivity extends Activity {
    ...
    int visibleActionCount(){Cursor c=db.getReadableDatabase().rawQuery("SELECT COUNT(*) FROM actions a JOIN knowledge_items k ON k.id=a.item_id WHERE a.status='open' AND k.type NOT IN ('SCREENSHOT','IMAGE')",null);int n=c.moveToFirst()?c.getInt(0):0;c.close();return n;}
