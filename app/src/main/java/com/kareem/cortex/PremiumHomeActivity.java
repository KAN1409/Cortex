...
public class PremiumHomeActivity extends Activity {
    ...
    void refreshAsync(){final int generation=++refreshGeneration;new Thread(()->{try{ArrayList<FeatureStore.InboxEntry> needs=FeatureStore.needs(db,80);int actions=count("actions","status='open'"),mem=count("knowledge_items","1=1");ArrayList<KnowledgeItem> xs=db.lexicalSearch("",6);runOnUiThread(()->{if(generation!=refreshGeneration||isFinishing())return;needsMetric.setText(String.valueOf(needs.size()));actionMetric.setText(String.valueOf(actions));memoryCount.setText(mem+" memories");heroStatus.setText(needs.isEmpty()?"CLEAR":needs.size()+" NEED");heroStatus.setTextColor(needs.isEmpty()?CortexUi.SAGE:CortexUi.ACCENT);renderRecent(xs);});}catch(Throwable ignored){}},"CortexHomeRefresh").start();}
