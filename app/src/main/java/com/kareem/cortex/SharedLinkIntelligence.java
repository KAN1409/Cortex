...
/** Turns shared URLs into useful Cortex knowledge with explicit pending/understood/failed state. */
public final class SharedLinkIntelligence {
    ...
    public static void enrichAsync(Context ctx,VaultDb db,long itemId,String sharedText){
        final String url=firstUrl(sharedText);if(itemId<=0||url.isEmpty())return;Context app=ctx.getApplicationContext();new Thread(()->{try{enrich(app,db,itemId,url,sharedText);}catch(Throwable ignored){}},"cortex-link-intel").start();
    }

    public static void reprocessPendingAsync(Context ctx){
        Context app=ctx.getApplicationContext();new Thread(()->{VaultDb v=null;try{v=new VaultDb(app);reprocessPending(app,v,20);}catch(Throwable ignored){}finally{if(v!=null)try{v.close();}catch(Throwable ignored){}}},"cortex-link-backfill").start();
    }

    public static int reprocessPending(Context ctx,VaultDb vault,int limit){
        ArrayList<long[]> ids=new ArrayList<>();ArrayList<String> texts=new ArrayList<>();
        Cursor c=vault.getReadableDatabase().rawQuery("SELECT id,raw_text,summary,metadata_json FROM knowledge_items WHERE source='android_share' AND (type='LINK' OR raw_text LIKE 'http%' OR summary LIKE 'http%') ORDER BY updated_at DESC LIMIT ?",new String[]{String.valueOf(Math.max(1,limit*4))});
        try{while(c.moveToNext()){
            long id=c.getLong(0);String raw=n(c.getString(1)),sum=n(c.getString(2)),meta=n(c.getString(3));String text=!firstUrl(raw).isEmpty()?raw:sum;String url=firstUrl(text);if(url.isEmpty())continue;
            String state="";try{state=new JSONObject(meta.isEmpty()?"{}":meta).optString("link_content_state","");}catch(Throwable ignored){}
            boolean urlOnly=n(raw).equals(url)||n(sum).equals(url)||n(raw).isEmpty();if("understood".equals(state)&&!urlOnly)continue;ids.add(new long[]{id});texts.add(text);if(ids.size()>=limit)break;
        }}finally{c.close();}
        int done=0;for(int i=0;i<ids.size();i++){try{String text=texts.get(i);enrich(ctx,vault,ids.get(i)[0],firstUrl(text),text);done++;}catch(Throwable ignored){}}return done;
    }

    static void enrich(Context ctx,VaultDb vault,long itemId,String url,String sharedText)throws Exception{
        SQLiteDatabase db=vault.getWritableDatabase();long now=System.currentTimeMillis();markPending(db,itemId,url,now);
        Fetch f;try{f=fetch(url);}catch(Throwable t){markFailed(db,itemId,url,now,t.getClass().getSimpleName()+": "+n(t.getMessage()));return;}
        ...
        String source=host(f.finalUrl.isEmpty()?url:f.finalUrl);String title=clean(f.title);if(title.isEmpty())title=source.isEmpty()?"Shared link":source;String body=clean(f.text);
        if(body.length()<80){meta.put("link_content_state","fetch_failed").put("failure_reason","no_readable_content").put("extraction_chars",body.length());ContentValues v=new ContentValues();v.put("type","LINK");v.put("title",title);v.put("summary","Shared link from "+(source.isEmpty()?"the web":source)+". Cortex could not extract enough readable content yet; the URL is preserved for retry.");v.put("category","Links & Research");v.put("tags","link,shared,web,pending_content");v.put("metadata_json",meta.toString());v.put("status","pending");v.put("analysis_error","link_content_unavailable");v.put("updated_at",now);db.update("knowledge_items",v,"id=?",new String[]{String.valueOf(itemId)});return;}
        String summary=summarize(body,title,source),useful=usefulTakeaway(body,title,source);meta.put("useful_takeaway",useful).put("extraction_chars",body.length()).put("link_content_state","understood");
        ContentValues v=new ContentValues();v.put("type","LINK");v.put("title",title);v.put("extracted_text",clip(body,7000));v.put("summary",summary);v.put("category","Links & Research");v.put("tags","link,shared,web,understood");v.put("metadata_json",meta.toString());v.put("status","analyzed");v.put("analysis_error","");v.put("updated_at",now);db.update("knowledge_items",v,"id=?",new String[]{String.valueOf(itemId)});try{SemanticIndex.indexItem(vault,itemId);}catch(Throwable ignored){}
    }
    ...
    private static Fetch fetch(String input)throws Exception{URL u=new URL(input);HttpURLConnection c=(HttpURLConnection)u.openConnection();c.setInstanceFollowRedirects(true);c.setConnectTimeout(9000);c.setReadTimeout(12000);c.setRequestProperty("User-Agent","Mozilla/5.0 CortexLink/1.1");c.setRequestProperty("Accept","text/html,application/xhtml+xml,text/plain;q=0.8,*/*;q=0.5");int status=c.getResponseCode();String ct=n(c.getContentType());InputStream raw=status>=200&&status<400?c.getInputStream():c.getErrorStream();String html=raw==null?"":read(raw,512000);String finalUrl=c.getURL()==null?input:c.getURL().toString();c.disconnect();String title=match(html,"(?is)<title[^>]*>(.*?)</title>");if(title.isEmpty())title=meta(html,"og:title");String text=htmlToText(html);return new Fetch(status,ct,finalUrl,decode(title),text);}
