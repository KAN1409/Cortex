...
public final class CortexRingButton extends View {
    ...
    private final Paint fill=new Paint(Paint.ANTI_ALIAS_FLAG),inner=new Paint(Paint.ANTI_ALIAS_FLAG),border=new Paint(Paint.ANTI_ALIAS_FLAG),track=new Paint(Paint.ANTI_ALIAS_FLAG),glow=new Paint(Paint.ANTI_ALIAS_FLAG),arc=new Paint(Paint.ANTI_ALIAS_FLAG),idleArc=new Paint(Paint.ANTI_ALIAS_FLAG),icon=new Paint(Paint.ANTI_ALIAS_FLAG);
    private final RectF ring=new RectF(),glyphRect=new RectF();private final Path glyphPath=new Path();private Glyph glyph=Glyph.PLAY;private float progress=0f;private boolean pressedDown=false;private final float density;private int accent=CortexUi.RED;
    ...
    public CortexRingButton(Context c,AttributeSet a){super(c,a);density=getResources().getDisplayMetrics().density;setLayerType(LAYER_TYPE_SOFTWARE,null);setClickable(true);setFocusable(true);
        fill.setColor(Color.rgb(18,18,20));fill.setStyle(Paint.Style.FILL);fill.setShadowLayer(11*density,0,6*density,Color.argb(190,0,0,0));
        ...
        track.setColor(Color.rgb(48,48,52));track.setStyle(Paint.Style.STROKE);track.setStrokeCap(Paint.Cap.ROUND);track.setStrokeWidth(2.4f*density);
        glow.setColor(accent);glow.setStyle(Paint.Style.STROKE);glow.setStrokeCap(Paint.Cap.ROUND);glow.setStrokeWidth(4.1f*density);applyGlowShadow(accent);
        arc.setColor(accent);arc.setStyle(Paint.Style.STROKE);arc.setStrokeCap(Paint.Cap.ROUND);arc.setStrokeWidth(2.2f*density);idleArc.setStyle(Paint.Style.STROKE);idleArc.setStrokeCap(Paint.Cap.ROUND);idleArc.setStrokeWidth(2.2f*density);
    ...
    private void drawGlyph(Canvas c,float cx,float cy,float s){Path p=glyphPath;p.reset();switch(glyph){
        ...
        case STOP:{icon.setStyle(Paint.Style.FILL);float q=s*.24f;RectF stop=glyphRect;stop.set(cx-q,cy-q,cx+q,cy+q);c.drawRoundRect(stop,s*.07f,s*.07f,icon);icon.setStyle(Paint.Style.STROKE);break;}
        ...
        case RECORD:{float bodyW=s*.22f,bodyH=s*.38f;RectF mic=glyphRect;mic.set(cx-bodyW,cy-bodyH,cx+bodyW,cy+s*.08f);c.drawRoundRect(mic,bodyW,bodyW,icon);p.moveTo(cx-s*.36f,cy);p.cubicTo(cx-s*.34f,cy+s*.32f,cx+s*.34f,cy+s*.32f,cx+s*.36f,cy);c.drawPath(p,icon);c.drawLine(cx,cy+s*.31f,cx,cy+s*.48f,icon);c.drawLine(cx-s*.18f,cy+s*.48f,cx+s*.18f,cy+s*.48f,icon);break;}
    }}
