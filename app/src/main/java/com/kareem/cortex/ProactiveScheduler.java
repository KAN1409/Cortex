...
public final class ProactiveScheduler {
    ...
    public static void reminder(Context c,long actionId,String title,String text,long whenMs){if(StartupSafetyGate.active()||c==null)return;long delay=Math.max(1000,whenMs-System.currentTimeMillis());Data d=new Data.Builder().putInt("id",(int)(actionId%Integer.MAX_VALUE)).putString("title",title).putString("text",text).build();OneTimeWorkRequest r=new OneTimeWorkRequest.Builder(ReminderWorker.class).setInputData(d).setInitialDelay(delay,TimeUnit.MILLISECONDS).addTag("cortex_reminder_"+actionId).build();WorkManager.getInstance(c).enqueue(r);}
}
