...
public class VaultDb extends SQLiteOpenHelper {
    ...
    private void createSemanticTables(SQLiteDatabase db){db.execSQL("CREATE TABLE IF NOT EXISTS memory_chunks(id INTEGER PRIMARY KEY AUTOINCREMENT,item_id INTEGER NOT NULL,chunk_index INTEGER NOT NULL,chunk_text TEXT NOT NULL,created_at INTEGER NOT NULL)");db.execSQL("CREATE INDEX IF NOT EXISTS idx_chunks_item ON memory_chunks(item_id)");db.execSQL("CREATE TABLE IF NOT EXISTS embeddings(id INTEGER PRIMARY KEY AUTOINCREMENT,chunk_id INTEGER NOT NULL UNIQUE,provider TEXT NOT NULL,version TEXT NOT NULL,dims INTEGER NOT NULL,vector BLOB NOT NULL,updated_at INTEGER NOT NULL)");db.execSQL("CREATE INDEX IF NOT EXISTS idx_embed_provider ON embeddings(provider,version)");}
    private void createVisionTables(SQLiteDatabase db){db.execSQL("CREATE TABLE IF NOT EXISTS vision_fields(id INTEGER PRIMARY KEY AUTOINCREMENT,item_id INTEGER NOT NULL,field_key TEXT NOT NULL,field_value TEXT NOT NULL,confidence REAL DEFAULT 0,created_at INTEGER NOT NULL)");db.execSQL("CREATE INDEX IF NOT EXISTS idx_vision_item ON vision_fields(item_id)");db.execSQL("CREATE INDEX IF NOT EXISTS idx_vision_key ON vision_fields(field_key)");}
    ...
    public void markAnalyzing(long id){status(id,"analyzing","");}public void markFailed(long id,String error){status(id,"analysis_failed",error==null?"Unknown analysis error":error);}public void markFailedRetryable(long id,String error){status(id,"failed_retryable",error==null?"Retryable analysis error":error);}public void retry(long id){status(id,"queued","");}
    ...
    public void applyAnalysis(long itemId,AnalysisResult r){SQLiteDatabase db=getWritableDatabase();long now=System.currentTimeMillis();db.beginTransaction();try{ContentValues v=new ContentValues();v.put("title",r.title);v.put("summary",r.summary);v.put("category",r.category);v.put("tags",r.tags);v.put("extracted_text",r.extractedText);v.put("status","analyzed");v.put("analysis_error","");v.put("updated_at",now);db.update("knowledge_items",v,"id=?",new String[]{String.valueOf(itemId)});ContentValues a=new ContentValues();a.put("item_id",itemId);a.put("engine",r.engine);a.put("version",r.version);a.put("output_json",r.toJson());a.put("created_at",now);db.insert("analyses",null,a);db.delete("entities","item_id=?",new String[]{String.valueOf(itemId)});for(AnalysisResult.Entity e:r.entities){ContentValues x=new ContentValues();x.put("item_id",itemId);x.put("kind",e.kind);x.put("value",e.value);x.put("confidence",e.confidence);x.put("created_at",now);db.insert("entities",null,x);}db.delete("actions","item_id=?",new String[]{String.valueOf(itemId)});for(AnalysisResult.Action ac:r.actions){ContentValues x=new ContentValues();x.put("item_id",itemId);x.put("action_text",ac.text);x.put("due_text",ac.dueText);x.put("status","open");x.put("created_at",now);db.insert("actions",null,x);}db.delete("vision_fields","item_id=?",new String[]{String.valueOf(itemId)});for(AnalysisResult.VisionField f:r.visionFields){if(f.value==null||f.value.trim().isEmpty())continue;ContentValues x=new ContentValues();x.put("item_id",itemId);x.put("field_key",f.key);x.put("field_value",f.value);x.put("confidence",f.confidence);x.put("created_at",now);db.insert("vision_fields",null,x);}db.setTransactionSuccessful();}finally{db.endTransaction();}try{SemanticIndex.indexItem(this,itemId);}catch(Exception ignored){}}
