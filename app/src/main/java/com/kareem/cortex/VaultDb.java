...
public class VaultDb extends SQLiteOpenHelper {
    ...
    public VaultDb(Context c){ super(c,DB,null,3); }

    @Override public void onCreate(SQLiteDatabase db){ createV3(db); }
    ...
    public void markAnalyzing(long id){status(id,"analyzing","");}public void markFailed(long id,String error){status(id,"analysis_failed",error==null?"Unknown analysis error":error);}public void retry(long id){status(id,"queued","");}
    ...
    public void applyAnalysis(long itemId,AnalysisResult r){
        SQLiteDatabase db=getWritableDatabase();long now=System.currentTimeMillis();db.beginTransaction();try{
            ...
            db.delete("actions","item_id=?",new String[]{String.valueOf(itemId)});for(AnalysisResult.Action ac:r.actions){ContentValues x=new ContentValues();x.put("item_id",itemId);x.put("action_text",ac.text);x.put("due_text",ac.dueText);x.put("status","open");x.put("created_at",now);db.insert("actions",null,x);}db.setTransactionSuccessful();
    ...
    public int pendingCount(){Cursor c=getReadableDatabase().rawQuery("SELECT COUNT(*) FROM knowledge_items WHERE status IN ('queued','analyzing')",null);int n=c.moveToFirst()?c.getInt(0):0;c.close();return n;}public int failedCount(){Cursor c=getReadableDatabase().rawQuery("SELECT COUNT(*) FROM knowledge_items WHERE status='analysis_failed'",null);int n=c.moveToFirst()?c.getInt(0):0;c.close();return n;}

    public ArrayList<KnowledgeItem> search(String q){
        if(q==null||q.trim().isEmpty())return lexicalSearch("",400);
        ArrayList<SemanticHit> hits=SemanticIndex.search(this,q,120);ArrayList<KnowledgeItem> out=new ArrayList<>();for(SemanticHit h:hits)out.add(h.item);return out;
    }
