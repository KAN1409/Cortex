...
public final class WorkVaultIndexer {
    ...
    public static Result indexPending(Context context,VaultDb vault,long sourceId){
        return indexPending(context,vault,sourceId,Integer.MAX_VALUE,Long.MAX_VALUE,0);
    }

    public static Result indexPending(Context context,VaultDb vault,long sourceId,int maxFiles,long maxMillis,long afterFileId){
        Result out=new Result();SQLiteDatabase db=vault.getWritableDatabase();WorkVaultIndexSchema.ensure(db);
        int cap=maxFiles<=0?1:maxFiles;long started=System.currentTimeMillis();
        ArrayList<FileRow> rows=new ArrayList<>();
        Cursor c=db.rawQuery(
                "SELECT f.id,f.document_uri,f.display_name,f.extension,COALESCE(f.fingerprint,''),f.state,f.active_version_id,"+
                "COALESCE(v.fingerprint,''),COALESCE(v.state,'') FROM work_files f LEFT JOIN work_file_versions v ON v.id=f.active_version_id "+
                "WHERE f.source_id=? AND f.id>? AND f.state IN ('new','modified','parse_failed','needs_ocr') ORDER BY f.id ASC LIMIT ?",
                new String[]{String.valueOf(sourceId),String.valueOf(Math.max(0,afterFileId)),String.valueOf(cap)});
        while(c.moveToNext())rows.add(new FileRow(c.getLong(0),c.getString(1),c.getString(2),c.getString(3),c.getString(4),c.getString(5),c.getLong(6),c.getString(7),c.getString(8)));c.close();
        out.total=rows.size();out.lastFileId=Math.max(0,afterFileId);
        for(FileRow f:rows){
            if(Thread.currentThread().isInterrupted()){out.interrupted=true;break;}
            WorkVaultResourceGovernor.Decision decision=WorkVaultResourceGovernor.current(context);
            if(decision==WorkVaultResourceGovernor.Decision.PAUSE){out.pausedByGovernor=true;break;}
            out.lastFileId=f.id;
            if("modified".equals(f.state)&&f.activeVersionId>0&&"complete".equals(f.activeVersionState)&&!f.fingerprint.isEmpty()&&f.fingerprint.equals(f.activeFingerprint)){
                mark(db,f.id,"indexed",VERSION,System.currentTimeMillis());out.skippedUnchanged++;
            }else if(!WorkDocumentParser.supported(f.ext)){
                mark(db,f.id,"unsupported","",0);out.unsupported++;
            }else{
                try{
            ...
            if(maxMillis>0&&maxMillis!=Long.MAX_VALUE&&System.currentTimeMillis()-started>=maxMillis){out.batchLimited=true;break;}
        }
        out.remaining=countPendingAfter(db,sourceId,out.lastFileId);
        return out;
    }

    private static int countPendingAfter(SQLiteDatabase db,long sourceId,long afterFileId){
        Cursor c=db.rawQuery("SELECT COUNT(*) FROM work_files WHERE source_id=? AND id>? AND state IN ('new','modified','parse_failed','needs_ocr')",new String[]{String.valueOf(sourceId),String.valueOf(Math.max(0,afterFileId))});
