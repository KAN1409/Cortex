...
public final class VisualRecoveryStore {
    ...
    static int retryRecoverableNow(VaultDb db,long exactItemId){
        if(db==null)return 0;ensure(db);long now=System.currentTimeMillis();SQLiteDatabase sql=db.getWritableDatabase();ArrayList<Long> ids=new ArrayList<>();Cursor c;if(exactItemId>0)c=sql.rawQuery("SELECT item_id FROM visual_recovery WHERE recoverable=1 AND item_id=? LIMIT 1",new String[]{String.valueOf(exactItemId)});else c=sql.rawQuery("SELECT item_id FROM visual_recovery WHERE recoverable=1 ORDER BY updated_at DESC LIMIT 100",null);while(c.moveToNext())ids.add(c.getLong(0));c.close();if(ids.isEmpty())return 0;
    ...
    public static int countTerminal(VaultDb db){ensure(db);return count(db,"SELECT COUNT(*) FROM visual_recovery WHERE recoverable=0");}
