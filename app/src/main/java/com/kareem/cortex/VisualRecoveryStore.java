...
public final class VisualRecoveryStore {
    ...
    private static final String LEDGER_ACTIVE_STATUS_FILTER="COALESCE(v.status,'') NOT IN ('done','local_only','skipped','waiting_provider')";
    ...
    /** Due retry entries are paused while an item is explicitly waiting for provider setup. */
    public static int activateDue(VaultDb db){
        ...
        String active="COALESCE(vi.status,'') NOT IN ('done','local_only','skipped','waiting_provider')";
        return db.getWritableDatabase().update("visual_insights",v,"item_id IN (SELECT r.item_id FROM visual_recovery r LEFT JOIN visual_insights vi ON vi.item_id=r.item_id WHERE r.recoverable=1 AND r.next_retry_at>0 AND r.next_retry_at<=? AND (r.attempt_count<? OR r.failure_kind='provider_rate_limit') AND "+active+") AND status NOT IN ('done','local_only','skipped','waiting_provider')",new String[]{String.valueOf(now),String.valueOf(VisualFailurePolicy.MAX_TRANSIENT_ATTEMPTS)});
    }

    /**
     * Explicit diagnostics retry for recoverable items. Attempts are deliberately preserved so a
     * manual "retry now" cannot turn bounded recovery into an infinite retry loop. Waiting-provider
     * items are intentionally excluded until provider setup is available again.
     */
    ...
    static int retryRecoverableNow(VaultDb db,long exactItemId){
        if(db==null)return 0;ensure(db);long now=System.currentTimeMillis();SQLiteDatabase sql=db.getWritableDatabase();ArrayList<Long> ids=new ArrayList<>();Cursor c;if(exactItemId>0)c=sql.rawQuery("SELECT r.item_id FROM visual_recovery r LEFT JOIN visual_insights v ON v.item_id=r.item_id WHERE r.recoverable=1 AND r.item_id=? AND "+LEDGER_ACTIVE_STATUS_FILTER+" LIMIT 1",new String[]{String.valueOf(exactItemId)});else c=sql.rawQuery("SELECT r.item_id FROM visual_recovery r LEFT JOIN visual_insights v ON v.item_id=r.item_id WHERE r.recoverable=1 AND "+LEDGER_ACTIVE_STATUS_FILTER+" ORDER BY r.updated_at DESC LIMIT 100",null);while(c.moveToNext())ids.add(c.getLong(0));c.close();if(ids.isEmpty())return 0;
        boolean own=!sql.inTransaction();if(own)sql.beginTransaction();int changed=0;try{for(long id:ids){ContentValues r=new ContentValues();r.put("next_retry_at",now);r.put("updated_at",now);sql.update("visual_recovery",r,"item_id=? AND recoverable=1",new String[]{String.valueOf(id)});ContentValues v=new ContentValues();v.put("pipeline_version",LEGACY_PIPELINE);v.put("status","failed");v.put("error","Explicit retry-now requested from Advanced diagnostics; bounded attempt history preserved");v.put("updated_at",now);changed+=sql.update("visual_insights",v,"item_id=? AND status NOT IN ('done','local_only','skipped','waiting_provider')",new String[]{String.valueOf(id)});}if(own)sql.setTransactionSuccessful();}finally{if(own)sql.endTransaction();}return changed;
    }

    /**
     * Explicit fresh budget for terminal items after the user fixes the underlying cause. The new
     * ledger begins recoverable at attempt 0 and is due immediately, so legacy adoption cannot spend
     * an attempt before real work occurs. Waiting-provider items are not terminal reset candidates.
     */
    ...
    private static int resetTerminalBudget(VaultDb db,int limit,long exactItemId){
        if(db==null)return 0;ensure(db);SQLiteDatabase sql=db.getWritableDatabase();ArrayList<Long> ids=new ArrayList<>();Cursor c;if(exactItemId>0)c=sql.rawQuery("SELECT r.item_id FROM visual_recovery r LEFT JOIN visual_insights v ON v.item_id=r.item_id WHERE r.recoverable=0 AND r.item_id=? AND "+LEDGER_ACTIVE_STATUS_FILTER+" LIMIT 1",new String[]{String.valueOf(exactItemId)});else c=sql.rawQuery("SELECT r.item_id FROM visual_recovery r LEFT JOIN visual_insights v ON v.item_id=r.item_id WHERE r.recoverable=0 AND "+LEDGER_ACTIVE_STATUS_FILTER+" ORDER BY r.updated_at DESC LIMIT ?",new String[]{String.valueOf(Math.max(1,Math.min(100,limit)))});while(c.moveToNext())ids.add(c.getLong(0));c.close();if(ids.isEmpty())return 0;long now=System.currentTimeMillis();boolean own=!sql.inTransaction();if(own)sql.beginTransaction();int changed=0;try{for(long id:ids){ContentValues r=new ContentValues();r.put("item_id",id);r.put("attempt_count",0);r.put("failure_kind","explicit_reset");r.put("recoverable",1);r.put("next_retry_at",now);r.put("next_action","Fresh bounded retry budget starts now after explicit user review.");r.put("last_error","");r.put("created_at",now);r.put("updated_at",now);sql.insertWithOnConflict("visual_recovery",null,r,SQLiteDatabase.CONFLICT_REPLACE);ContentValues v=new ContentValues();v.put("pipeline_version",LEGACY_PIPELINE);v.put("status","failed");v.put("error","Explicit terminal retry budget reset after user review; fresh attempt budget=0");v.put("updated_at",now);int updated=sql.update("visual_insights",v,"item_id=? AND status NOT IN ('done','local_only','skipped','waiting_provider')",new String[]{String.valueOf(id)});if(updated>0)changed++;}if(own)sql.setTransactionSuccessful();}finally{if(own)sql.endTransaction();}return changed;
    }
    ...
    public static long nextDelayMs(VaultDb db){ensure(db);long now=System.currentTimeMillis();Cursor c=db.getReadableDatabase().rawQuery("SELECT MIN(r.next_retry_at) FROM visual_recovery r LEFT JOIN visual_insights v ON v.item_id=r.item_id WHERE r.recoverable=1 AND r.next_retry_at>? AND "+LEDGER_ACTIVE_STATUS_FILTER,new String[]{String.valueOf(now)});long at=c.moveToFirst()&&!c.isNull(0)?c.getLong(0):0;c.close();return at<=0?-1:Math.max(1_000L,at-now);}
    public static int countRecoverable(VaultDb db){ensure(db);return count(db,"SELECT COUNT(*) FROM visual_recovery r LEFT JOIN visual_insights v ON v.item_id=r.item_id WHERE r.recoverable=1 AND "+LEDGER_ACTIVE_STATUS_FILTER);}
    public static int countTerminal(VaultDb db){ensure(db);return count(db,"SELECT COUNT(*) FROM visual_recovery r LEFT JOIN visual_insights v ON v.item_id=r.item_id WHERE r.recoverable=0 AND "+LEDGER_ACTIVE_STATUS_FILTER);}
