...
public final class OpenLoopStore {
    ...
    private static boolean terminal(VaultDb db,long loopId,long signalId,String state,String relation,double confidence){if(db==null||loopId<=0)return false;long now=System.currentTimeMillis();ContentValues v=new ContentValues();v.put("state",state);v.put("updated_at",now);v.put("resolved_at",now);boolean ok=db.getWritableDatabase().update("open_loops",v,"id=? AND state IN ('OPEN','WAITING','DUE','OVERDUE')",new String[]{String.valueOf(loopId)})>0;if(ok){if(signalId>0)attachEvidence(db,loopId,signalId,relation,confidence);AttentionFeedStore.removeLoop(db,loopId);AttentionActionStore.closeForLoop(db,loopId,state);db.getWritableDatabase().delete("attention_assessments","entity_type='open_loop' AND entity_id=?",new String[]{String.valueOf(loopId)});}return ok;}
    ...
    public static Loop activeForThread(VaultDb db,long threadId){List<Loop> xs=activeForThreadAll(db,threadId,1);return xs.isEmpty()?null:xs.get(0);}
    public static List<Loop> activeForThreadAll(VaultDb db,long threadId,int limit){CortexAttentionSchema.ensure(db);ArrayList<Loop> out=new ArrayList<>();Cursor c=db.getReadableDatabase().query("open_loops",cols(),"thread_id=? AND state IN ('OPEN','WAITING','DUE','OVERDUE')",new String[]{String.valueOf(threadId)},null,null,"updated_at DESC",String.valueOf(Math.max(1,limit)));while(c.moveToNext())out.add(new Loop(c));c.close();return out;}
