...
public final class UniversalSemanticWorker extends Worker {
    ...
    @NonNull @Override public Result doWork(){
        VaultDb vault=null;try{
            vault=new VaultDb(getApplicationContext());UniversalEventStore.ensure(vault.getWritableDatabase());
            if(!LocalModelManager.installed(getApplicationContext()))return Result.success();
            int processed=0;
    ...
    private void process(VaultDb v,Row r){
        ...
        try{
            ...
            if(!"NONE".equals(att)&&!"INFO".equals(att))UniversalEventStore.attention(v.getWritableDatabase(),r.eventId,0,att,subject,summary,priority,conf,r.source,reason);
