...
public final class UniversalSemanticWorker extends Worker {
    ...
    @NonNull @Override public Result doWork(){
        VaultDb vault=null;try{
            ...
            if(!LocalModelManager.installed(app)){
                ...
                if("failed".equals(st.state)){markQueueBlocked(vault,"Local runtime self-test failed: "+st.error);return Result.success();}
                return Result.retry();
            }
            // A repaired runtime makes previously blocked semantic work eligible again.
            ContentValues recover=new ContentValues();recover.put("semantic_state","waiting");recover.put("reason","Local runtime recovered; semantic refinement resumed");vault.getWritableDatabase().update("ue_semantic_events",recover,"semantic_state='blocked' AND superseded_by=0",null);
            int processed=0;while(processed<12){Row row=next(vault);if(row==null)break;process(vault,row);processed++;}
    ...
    private long projectSituation(VaultDb v,Row r,String type,String subject,String summary,int priority,double conf,String attention,String reason){String t=type==null?"":type.toLowerCase();boolean open=!"NONE".equals(att)&&!"INFO".equals(att)||t.contains("commitment")||t.contains("request")||t.contains("waiting")||t.contains("decision");if(!open)return 0;String kind=t.contains("decision")?"decision":(t.contains("commitment")?"commitment":(t.contains("request")?"open_request":"context"));String key=Fingerprint.text("ue-situation|"+r.source+"|"+kind+"|"+CanonicalPresentation.cleanBody(subject).toLowerCase());JSONObject m=new JSONObject();try{m.put("semantic_event_id",r.eventId).put("reason",reason);}catch(Exception ignored){}long id=UniversalEventStore.upsertSituation(v.getWritableDatabase(),key,kind,subject,summary,"open",priority,conf,r.occurredAt,m);UniversalEventStore.linkSituationEvent(v.getWritableDatabase(),id,r.eventId,"supports");return id;}
