...
public final class StatefulMeaningStore {
    ...
    public static void ensure(SQLiteDatabase db){
        ...
        db.execSQL("CREATE TABLE IF NOT EXISTS ue_situation_state_v2(situation_id INTEGER PRIMARY KEY,correlation_key TEXT NOT NULL UNIQUE,last_fact_hash TEXT,last_transition_kind TEXT,member_count INTEGER NOT NULL DEFAULT 0,opened_at INTEGER NOT NULL,updated_at INTEGER NOT NULL)");
    ...
    public static long correlate(SQLiteDatabase db,long semanticEventId,long sourceInstanceId,String source,String semanticType,String subject,String summary,double confidence,long occurredAt){
        ensure(db);long when=occurredAt>0?occurredAt:System.currentTimeMillis(),now=System.currentTimeMillis();String key=StatefulMeaningPolicy.correlationKey(semanticType,subject,summary,source,when),factHash=Fingerprint.text(n(semanticType)+"|"+LocalSemanticEmbedder.norm(n(subject)+" "+n(summary)));
        long situationId=0;String previousHash="";int members=0;Cursor c=db.rawQuery("SELECT situation_id,last_fact_hash,member_count FROM ue_situation_state_v2 WHERE correlation_key=? LIMIT 1",new String[]{key});if(c.moveToFirst()){situationId=c.getLong(0);previousHash=n(c.getString(1));members=c.getInt(2);}c.close();
        ...
        if(situationId<=0){situationId=UniversalEventStore.upsertSituation(db,key,"stateful",title,body,"open",priority(semanticType),confidence,when,new JSONObject());ContentValues st=new ContentValues();st.put("situation_id",situationId);st.put("correlation_key",key);st.put("last_fact_hash",factHash);st.put("last_transition_kind","OPENED");st.put("member_count",1);st.put("opened_at",when);st.put("updated_at",now);db.insertOrThrow("ue_situation_state_v2",null,st);recordSituationTransition(db,situationId,semanticEventId,"OPENED",title,body,confidence,"new correlation key",when);}
        else{
            String transition=previousHash.equals(factHash)?"SUPPORTING_EVIDENCE":(StatefulMeaningPolicy.materialChange(previousHash,factHash)?"MATERIAL_UPDATE":"SUPPORTING_EVIDENCE");ContentValues st=new ContentValues();st.put("last_fact_hash",factHash);st.put("last_transition_kind",transition);st.put("member_count",members+1);st.put("updated_at",now);db.update("ue_situation_state_v2",st,"situation_id=?",new String[]{String.valueOf(situationId)});if(StatefulMeaningPolicy.materialTransition(transition))recordSituationTransition(db,situationId,semanticEventId,transition,title,body,confidence,"material correlated fact",when);
        }
        ContentValues member=new ContentValues();member.put("situation_id",situationId);member.put("semantic_event_id",semanticEventId);member.put("source_instance_id",sourceInstanceId);member.put("relation","supports");member.put("confidence",confidence);member.put("evidence_json",new JSONObject().toString());member.put("created_at",now);db.insertWithOnConflict("ue_situation_members_v2",null,member,SQLiteDatabase.CONFLICT_IGNORE);UniversalEventStore.linkSituationEvent(db,situationId,semanticEventId,"supports");return situationId;
    }
    ...
    private static void bump(SQLiteDatabase db,String name){long now=System.currentTimeMillis();db.execSQL("INSERT INTO ue_projection_revisions(name,revision,updated_at) VALUES(?,1,?) ON CONFLICT(name) DO UPDATE SET revision=revision+1,updated_at=excluded.updated_at",new Object[]{name,now});}
