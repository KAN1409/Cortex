...
public class SatinBriefActivity extends PremiumHomeActivity {
    ...
    static final class BriefGlyphView extends View {
        final Paint p=new Paint(Paint.ANTI_ALIAS_FLAG),dot=new Paint(Paint.ANTI_ALIAS_FLAG),accentStroke=new Paint(Paint.ANTI_ALIAS_FLAG);final Path glyphPath=new Path();final RectF glyphRect=new RectF();final String kind;final float d;final int accent;
        BriefGlyphView(android.content.Context c,String k,int color){super(c);kind=k;accent=color;d=getResources().getDisplayMetrics().density;p.setColor(Color.rgb(207,211,219));p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(1.6f*d);p.setStrokeCap(Paint.Cap.ROUND);p.setStrokeJoin(Paint.Join.ROUND);dot.setColor(color);dot.setStyle(Paint.Style.FILL);accentStroke.setStyle(Paint.Style.STROKE);accentStroke.setStrokeCap(Paint.Cap.ROUND);setClickable(true);}
        @Override protected void onDraw(Canvas c){super.onDraw(c);float w=getWidth(),h=getHeight(),cx=w/2,cy=h/2,s=Math.min(w,h)*.28f;Path q=glyphPath;q.reset();if(kind.startsWith("wave")){for(int i=-2;i<=2;i++){float x=cx+i*s*.38f,hh=s*(i==0?1f:(Math.abs(i)==1?.72f:.45f));c.drawLine(x,cy-hh,x,cy+hh,p);}if("waveRing".equals(kind)){RectF r=glyphRect;r.set(cx-s*1.5f,cy-s*1.5f,cx+s*1.5f,cy+s*1.5f);Paint a=accentStroke;a.setColor(accent);a.setStrokeWidth(2*d);c.drawArc(r,-90,225,false,a);}}else if("action".equals(kind)){c.drawRect(cx-s,cy-s,cx+s*.7f,cy+s*.7f,p);c.drawLine(cx,cy,cx+s,cy-s,p);c.drawLine(cx+s*.45f,cy-s,cx+s,cy-s,p);c.drawLine(cx+s,cy-s,cx+s,cy-s*.45f,p);}else if("waiting".equals(kind)){RectF r=glyphRect;r.set(cx-s,cy-s,cx+s,cy+s);c.drawArc(r,-70,300,false,p);c.drawLine(cx,cy,cx,cy-s*.55f,p);c.drawLine(cx,cy,cx+s*.45f,cy,p);}else if("decision".equals(kind)){c.drawCircle(cx,cy+s*.65f,s*.18f,p);c.drawCircle(cx-s*.65f,cy-s*.65f,s*.18f,p);c.drawCircle(cx+s*.65f,cy-s*.65f,s*.18f,p);c.drawLine(cx,cy+s*.47f,cx,cy,p);c.drawLine(cx,cy,cx-s*.65f,cy-s*.47f,p);c.drawLine(cx,cy,cx+s*.65f,cy-s*.47f,p);}else if("nodes".equals(kind)){for(int i=0;i<7;i++){double a=i*Math.PI*2/7;c.drawCircle(cx+(float)Math.cos(a)*s*.75f,cy+(float)Math.sin(a)*s*.75f,s*.10f,p);}c.drawCircle(cx,cy,s*.13f,p);}else{c.drawRect(cx-s*.8f,cy-s,cx+s*.55f,cy+s,p);for(int i=-1;i<=1;i++)c.drawLine(cx-s*.45f,cy+i*s*.45f,cx+s*.25f,cy+i*s*.45f,p);}c.drawCircle(cx+s*1.08f,cy-s*1.02f,2.4f*d,dot);}
    }
}
