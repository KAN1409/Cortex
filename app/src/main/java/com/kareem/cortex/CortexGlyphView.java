...
/**
 * Cortex production icon language, traced from the approved reference boards.
 * Monoline, rounded, AMOLED-first. Navigation glyphs are bare; utility/card glyphs
 * can sit on compact plates. No bitmap crops are used so the assets stay sharp.
 */
public final class CortexGlyphView extends View {
    ...
    public CortexGlyphView(Context c,String kind,int accent,boolean dot){
        super(c);this.kind=kind==null?"info":kind;this.accent=accent;this.showDot=dot;d=getResources().getDisplayMetrics().density;
        setLayerType(LAYER_TYPE_SOFTWARE,null);
        plate.setColor(CortexUi.BG);plate.setStyle(Paint.Style.FILL);
        edge.setColor(CortexUi.BORDER);edge.setStyle(Paint.Style.STROKE);edge.setStrokeWidth(1*d);
        glyph.setColor(CortexUi.TEXT);glyph.setStyle(Paint.Style.STROKE);glyph.setStrokeWidth(1.72f*d);glyph.setStrokeCap(Paint.Cap.ROUND);glyph.setStrokeJoin(Paint.Join.ROUND);
    ...
    @Override protected void onDraw(Canvas c){
        super.onDraw(c);float w=getWidth(),h=getHeight(),r=Math.min(w,h);
        if(!bare()){RectF b=new RectF(1.5f*d,1.5f*d,w-1.5f*d,h-1.5f*d);float cr=r*.22f;c.drawRoundRect(b,cr,cr,plate);c.drawRoundRect(b,cr,cr,edge);}
        float cx=w*.5f,cy=h*.5f,s=r*(bare()?.285f:.23f);drawGlyph(c,cx,cy,s);
        if(showDot)c.drawCircle(w*.79f,h*.21f,Math.max(2.3f*d,r*.055f),accentPaint);
    }

    private void drawGlyph(Canvas c,float cx,float cy,float s){Path p=new Path();RectF r;switch(kind){
        case "brand":{float rr=s*1.12f;for(int i=0;i<6;i++){double a=Math.PI/3*i-Math.PI/6;float x=cx+(float)Math.cos(a)*rr,y=cy+(float)Math.sin(a)*rr;if(i==0)p.moveTo(x,y);else p.lineTo(x,y);}p.close();Paint gp=copy(accent,Paint.Style.STROKE,2.05f*d);c.drawPath(p,gp);Path cc=new Path();r=new RectF(cx-s*.48f,cy-s*.47f,cx+s*.48f,cy+s*.47f);c.drawArc(r,55,250,false,gp);c.drawLine(cx+s*.18f,cy-s*.40f,cx+s*.48f,cy-s*.24f,gp);break;}
        case "menu":{Paint g=copy(CortexUi.TEXT,Paint.Style.STROKE,1.55f*d);RectF b=new RectF(cx-s*1.25f,cy-s*1.25f,cx+s*1.25f,cy+s*1.25f);c.drawRoundRect(b,s*.34f,s*.34f,edge);for(int i=-1;i<=1;i++){float y=cy+i*s*.55f;c.drawCircle(cx-s*.62f,y,s*.075f,g);c.drawLine(cx-s*.30f,y,cx+s*.68f,y,g);}break;}
        ...
        // MAIN DOCK — approved reference silhouettes.
        ...
        // Atlas keeps the existing route/label but now uses the approved People/Projects visual language.
        case "nav_atlas":{drawPeopleProjects(c,cx,cy,s);break;}
        case "nav_brain":{drawBrain(c,cx,cy,s);break;}

        // CAPTURE CENTER — approved Text / Voice / Photo-File / Link family.
        case "text":{Paint g=copy(CortexUi.TEXT,Paint.Style.STROKE,1.65f*d);c.drawLine(cx-s*.72f,cy-s*.78f,cx+s*.72f,cy-s*.78f,g);c.drawLine(cx,cy-s*.78f,cx,cy+s*.76f,g);c.drawLine(cx-s*.28f,cy+s*.76f,cx+s*.28f,cy+s*.76f,g);break;}
        ...
        case "attachment":{Paint g=copy(CortexUi.TEXT,Paint.Style.STROKE,1.70f*d);r=new RectF(cx-s*.56f,cy-s*1.02f,cx+s*.56f,cy+s*1.02f);c.drawArc(r,214,255,false,g);r=new RectF(cx-s*.20f,cy-s*.72f,cx+s*.30f,cy+s*.66f);c.drawArc(r,210,270,false,g);break;}
        case "link":{Paint g=copy(CortexUi.TEXT,Paint.Style.STROKE,1.70f*d);r=new RectF(cx-s*.98f,cy-s*.58f,cx+s*.10f,cy+s*.44f);c.drawArc(r,132,188,false,g);r=new RectF(cx-s*.10f,cy-s*.44f,cx+s*.98f,cy+s*.58f);c.drawArc(r,-48,188,false,g);c.drawLine(cx-s*.32f,cy+s*.22f,cx+s*.32f,cy-s*.22f,g);break;}
    ...
    private void drawPeopleProjects(Canvas c,float cx,float cy,float s){
        Paint g=copy(glyph.getColor(),Paint.Style.STROKE,glyph.getStrokeWidth());
        c.drawCircle(cx-s*.14f,cy-s*.48f,s*.31f,g);
        RectF body=new RectF(cx-s*.72f,cy-s*.02f,cx+s*.45f,cy+s*.78f);c.drawArc(body,194,154,false,g);
        // small companion silhouette gives the exact People/Projects feel without clutter.
        c.drawCircle(cx+s*.58f,cy-s*.28f,s*.22f,g);
        RectF body2=new RectF(cx+s*.24f,cy+s*.04f,cx+s*.94f,cy+s*.58f);c.drawArc(body2,198,142,false,g);
    }

    private void drawBrain(Canvas c,float cx,float cy,float s){
        Paint g=copy(glyph.getColor(),Paint.Style.STROKE,glyph.getStrokeWidth());
        Path l=new Path();l.moveTo(cx,cy-s*.88f);l.cubicTo(cx-s*.75f,cy-s*.98f,cx-s*.92f,cy-s*.30f,cx-s*.72f,cy);l.cubicTo(cx-s*.98f,cy+s*.42f,cx-s*.50f,cy+s*.92f,cx,cy+s*.78f);c.drawPath(l,g);
        Path r=new Path();r.moveTo(cx,cy-s*.88f);r.cubicTo(cx+s*.75f,cy-s*.98f,cx+s*.92f,cy-s*.30f,cx+s*.72f,cy);r.cubicTo(cx+s*.98f,cy+s*.42f,cx+s*.50f,cy+s*.92f,cx,cy+s*.78f);c.drawPath(r,g);
        c.drawLine(cx,cy-s*.72f,cx,cy+s*.72f,g);c.drawLine(cx-s*.48f,cy-s*.22f,cx-s*.12f,cy-s*.02f,g);c.drawLine(cx+s*.48f,cy+s*.20f,cx+s*.12f,cy+s*.02f,g);
    }
