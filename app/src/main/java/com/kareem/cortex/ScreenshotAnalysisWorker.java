...
/** Processes a small screenshot batch synchronously so WorkManager owns the lifetime. */
public class ScreenshotAnalysisWorker extends Worker {
    ...
    @NonNull @Override public Result doWork(){
        Context ctx=getApplicationContext();VaultDb db=new VaultDb(ctx);requeueStale(db);int done=0;
        try{
            while(done<BATCH&&!isStopped()){
                ...
                OcrAnalyzer.analyze(ctx,item,new OcrAnalyzer.Callback(){public void ok(AnalysisResult r){ok.set(r);latch.countDown();}public void fail(Exception e){bad.set(e);latch.countDown();}});
                boolean returned=latch.await(150,TimeUnit.SECONDS);if(!returned){db.markFailedRetryable(item.id,"OCR worker timeout");done++;continue;}
                AnalysisResult r=ok.get();if(r!=null){db.applyAnalysis(item.id,r);try{TemporalResolver.afterAnalysis(db,item.id);}catch(Exception ignored){}try{CoreBrainEngine.afterAnalysis(db,item.id);}catch(Exception ignored){}}
                else{Exception e=bad.get();String msg=e==null?"Unknown OCR failure":e.getMessage();if(msg!=null&&msg.startsWith("RETRYABLE:"))db.markFailedRetryable(item.id,msg.substring("RETRYABLE:".length()).trim());else db.markFailed(item.id,msg);}
                done++;
            }
            if(hasQueued(db))ScreenshotWorkScheduler.continueChain(ctx);
    ...
    private static KnowledgeItem next(VaultDb db){
        Cursor c=db.getReadableDatabase().rawQuery("SELECT * FROM knowledge_items WHERE status='queued' AND type IN ('SCREENSHOT','IMAGE') ORDER BY created_at ASC LIMIT 1",null);KnowledgeItem k=null;if(c.moveToFirst())k=from(c);c.close();return k;
    }
