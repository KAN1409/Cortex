...
public class NotificationCaptureService extends NotificationListenerService {
    ...
    @Override public void onNotificationRemoved(StatusBarNotification sbn){removed(sbn,0);}
    @Override public void onNotificationRemoved(StatusBarNotification sbn,RankingMap rankingMap,int reason){removed(sbn,reason);}

    private void removed(StatusBarNotification sbn,int reason){
        if(sbn==null)return;VaultDb db=null;
        try{
            ...
            if(sbn.getGroupKey()!=null)m.put("group_key",sbn.getGroupKey());
    ...
    private void ingest(StatusBarNotification sbn,String captureMode){VaultDb db=null;try{
        ...
        String hint=inferKind(n,e);JSONObject meta=new JSONObject();meta.put("capture_kind","android_notification");meta.put("capture_mode",captureMode);meta.put("package",pkg);meta.put("app_label",app);meta.put("posted_at",sbn.getPostTime());meta.put("notification_id",sbn.getId());meta.put("notification_tag",sbn.getTag()==null?"":sbn.getTag());meta.put("tag",sbn.getTag()==null?"":sbn.getTag());meta.put("ongoing",ongoing);meta.put("clearable",sbn.isClearable());meta.put("has_visible_text",!visible.isEmpty());meta.put("flags",n.flags);meta.put("priority",n.priority);meta.put("visibility",n.visibility);meta.put("number",n.number);meta.put("when",n.when);meta.put("color",n.color);meta.put("platform_hint",hint);meta.put("notification_kind",hint);
        if(sbn.getKey()!=null)meta.put("notification_key",sbn.getKey());if(sbn.getGroupKey()!=null)meta.put("group_key",sbn.getGroupKey());if(n.category!=null)meta.put("category",n.category);if(Build.VERSION.SDK_INT>=20){meta.put("local_only",(n.flags&Notification.FLAG_LOCAL_ONLY)!=0);meta.put("group_summary",(n.flags&Notification.FLAG_GROUP_SUMMARY)!=0);if(n.getGroup()!=null)meta.put("group",n.getGroup());if(n.getSortKey()!=null)meta.put("sort_key",n.getSortKey());}
        if(Build.VERSION.SDK_INT>=26&&n.getChannelId()!=null)meta.put("channel_id",n.getChannelId());if(Build.VERSION.SDK_INT>=26)meta.put("timeout_after",n.getTimeoutAfter());if(Build.VERSION.SDK_INT>=26&&n.getShortcutId()!=null)meta.put("shortcut_id",n.getShortcutId());
        meta.put("title",title);meta.put("text",str(e.getCharSequence(Notification.EXTRA_TEXT)));meta.put("big_text",big);meta.put("text_lines",lines);meta.put("messages",messages);put(meta,"sub_text",e.getCharSequence(Notification.EXTRA_SUB_TEXT));put(meta,"summary_text",e.getCharSequence(Notification.EXTRA_SUMMARY_TEXT));put(meta,"info_text",e.getCharSequence(Notification.EXTRA_INFO_TEXT));put(meta,"title_big",e.getCharSequence(Notification.EXTRA_TITLE_BIG));put(meta,"conversation_title",Build.VERSION.SDK_INT>=24?e.getCharSequence(Notification.EXTRA_CONVERSATION_TITLE):null);put(meta,"template",e.getString(Notification.EXTRA_TEMPLATE));
        ...
        // PhoneContext is observability only; the universal/stateful ledgers remain the source of truth.
    ...
    private static String joinMessages(JSONArray a){StringBuilder b=new StringBuilder();for(int i=0;i<a.length();i++){JSONObject m=a.optJSONObject(i);if(m==null)continue;String sender=m.optString("sender",""),text=m.optString("text","");if(text.isEmpty())continue;if(b.length()>0)b.append("\n");if(!sender.isEmpty())b.append(sender).append(": ");b.append(text);if(b.length()>1600)break;}return b.toString();}
    private static void put(JSONObject o,String key,Object value){try{String x=value==null?"":String.valueOf(value).trim();if(!x.isEmpty())o.put(key,x);}catch(Throwable ignored){}}
    ...
    private static String inferKind(Notification n,Bundle e){String c=n==null||n.category==null?"":n.category;if(Notification.CATEGORY_MESSAGE.equals(c)||e.getParcelableArray(Notification.EXTRA_MESSAGES)!=null)return"message";if(Notification.CATEGORY_EMAIL.equals(c))return"email";if(Notification.CATEGORY_CALL.equals(c))return"call";if(Notification.CATEGORY_PROMO.equals(c))return"promotion";if(Notification.CATEGORY_SOCIAL.equals(c))return"social";if(Notification.CATEGORY_RECOMMENDATION.equals(c))return"recommendation";if(Notification.CATEGORY_PROGRESS.equals(c))return"progress";if(Notification.CATEGORY_SERVICE.equals(c))return"service";if(Notification.CATEGORY_TRANSPORT.equals(c))return"media";if(Notification.CATEGORY_ALARM.equals(c))return"alarm";if(Notification.CATEGORY_REMINDER.equals(c))return"reminder";if(Notification.CATEGORY_EVENT.equals(c))return"event";return"notification";}
