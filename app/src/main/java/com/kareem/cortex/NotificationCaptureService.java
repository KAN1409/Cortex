...
/** All allowed notifications enter the raw-signal layer first; only selected signals reach Vault. */
public class NotificationCaptureService extends NotificationListenerService {
    @Override public void onNotificationPosted(StatusBarNotification sbn){
        try{
            ...
            String big=str(e.getCharSequence(Notification.EXTRA_BIG_TEXT));if(!big.isEmpty())text=big;
            if(title.isEmpty()&&text.isEmpty())return;
            String body=(title+(title.isEmpty()||text.isEmpty()?"":"\n")+text).trim();if(body.isEmpty())return;
            String pkg=sbn.getPackageName()==null?"":sbn.getPackageName();boolean ongoing=(n.flags&Notification.FLAG_ONGOING_EVENT)!=0;
