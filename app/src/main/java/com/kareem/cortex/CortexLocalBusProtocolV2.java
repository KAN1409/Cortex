 ...
 *
 * V2 is negotiated explicitly during HELLO. V1 remains accepted unchanged. V2 notification
 * envelopes are adapted into the canonical V1 ingest event so Cortex keeps one trusted evidence
 * admission path, one event-id dedupe boundary and one correlated ACK contract.
 */
public final class CortexLocalBusProtocolV2 {
    ...
    public static Event parseEvent(String raw) {
        ...
        try {
            ...
            if (!SIGNAL_PROTOCOL.equals(clean(root.optString("protocol", "")))) {
                throw new IllegalArgumentException("unsupported V2 protocol");
            }
            if (!SIGNAL_SCHEMA.equals(clean(root.optString("schema", "")))) {
                throw new IllegalArgumentException("unsupported V2 schema");
            }
            ...
            if (occurredAt > System.currentTimeMillis() + 24L * 60L * 60L * 1000L) {
                throw new IllegalArgumentException("occurred_at is implausibly future");
            }
    ...
    /**
     * Convert a validated V2 signal into the existing V1 Event shape without losing V2 evidence.
     * The exact event_id is preserved, so duplicate handling and ACK correlation stay unchanged.
     */
    public static CortexLocalBusProtocolV1.Event toCanonicalV1(Event event) {
        ...
        try {
            ...
            metadata.put("local_bus_signal_v2", new JSONObject()
                    .put("protocol", SIGNAL_PROTOCOL)
                    .put("schema", SIGNAL_SCHEMA)
                    .put("received_as_v2", true));
    ...
    public static final class Event {
        ...
        Event(String eventId, String connectorId, String sourceType, String sourcePackage, long occurredAt,
              JSONObject root, JSONObject source, JSONObject semantic) {
