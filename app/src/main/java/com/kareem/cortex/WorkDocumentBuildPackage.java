...
public final class WorkDocumentBuildPackage {
    public static final String VERSION="work_document_build_package_001";
    ...
    private static JSONArray refs(SQLiteDatabase db,String filter){
        JSONArray a=new JSONArray();String sql="SELECT r.ref_type,r.ref_value,r.confidence,f.display_name,f.document_uri,p.canonical_name FROM work_procurement_refs r JOIN work_files f ON f.id=r.file_id LEFT JOIN work_projects p ON p.id=r.project_id";
        ArrayList<String> args=new ArrayList<>();if(!filter.isEmpty()){sql+=" WHERE lower(COALESCE(p.canonical_name,'')) LIKE ?";args.add("%"+filter.toLowerCase(Locale.ROOT)+"%");}sql+=" ORDER BY r.created_at DESC LIMIT 120";
    ...
    private static JSONArray facts(SQLiteDatabase db,String filter){
        ...
        ArrayList<String> args=new ArrayList<>();if(!filter.isEmpty()){sql+=" WHERE lower(COALESCE(p.canonical_name,'')) LIKE ?";args.add("%"+filter.toLowerCase(Locale.ROOT)+"%");}sql+=" ORDER BY x.created_at DESC LIMIT 250";
    ...
    private static JSONArray prices(SQLiteDatabase db,String filter){
        ...
        ArrayList<String> args=new ArrayList<>();if(!filter.isEmpty()){sql+=" WHERE lower(COALESCE(p.canonical_name,'')) LIKE ?";args.add("%"+filter.toLowerCase(Locale.ROOT)+"%");}sql+=" ORDER BY f.modified_at DESC,x.created_at DESC LIMIT 250";
    ...
    private static JSONArray sources(SQLiteDatabase db,String filter){
        JSONArray a=new JSONArray();String sql="SELECT DISTINCT f.display_name,f.document_uri,f.mime_type,f.modified_at,p.canonical_name FROM work_files f LEFT JOIN work_facts x ON x.file_id=f.id LEFT JOIN work_projects p ON p.id=x.project_id WHERE f.state IN ('indexed','needs_ocr')";
