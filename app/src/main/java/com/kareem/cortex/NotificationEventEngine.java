 ...
 * Cortex semantic truth. Exact redelivery of the same Android observation is idempotent.
 */
public final class NotificationEventEngine {
    ...
    public static final class Result {
        ...
        public final boolean shouldUnderstand,exactDuplicateDelivery;
        Result(long raw,long stream,long semantic,String tr,String tech,String hint,boolean understand){this(raw,stream,semantic,tr,tech,hint,understand,false);}
        Result(long raw,long stream,long semantic,String tr,String tech,String hint,boolean understand,boolean duplicate){
            rawId=raw;streamId=stream;semanticEventId=semantic;transition=safe(tr);technicalType=safe(tech);platformHint=safe(hint);shouldUnderstand=understand;exactDuplicateDelivery=duplicate;
        }
    }
    ...
    public static Result ingest(VaultDb db,String pkg,String appLabel,String eventType,String title,String body,long occurredAt,JSONObject meta){
        ...
        long rawId=0;boolean began=false;
        try{
    ...
    private static long existingDeliveryId(SQLiteDatabase s,String deliveryKey,String streamKey,String eventType,String hash,long when){
        Cursor c=s.rawQuery("SELECT raw_observation_id FROM notification_delivery_dedup WHERE delivery_key=? LIMIT 1",new String[]{deliveryKey});long id=c.moveToFirst()?c.getLong(0):0;c.close();if(id>0)return id;
        c=s.rawQuery("SELECT id FROM notification_raw_observations WHERE stream_key=? AND event_type=? AND content_hash=? AND occurred_at=? ORDER BY id ASC LIMIT 1",new String[]{streamKey,eventType,hash,String.valueOf(when)});id=c.moveToFirst()?c.getLong(0):0;c.close();if(id>0)rememberDelivery(s,deliveryKey,id,System.currentTimeMillis());return id;
    }
    private static void rememberDelivery(SQLiteDatabase s,String key,long rawId,long now){ContentValues v=new ContentValues();v.put("delivery_key",key);v.put("raw_observation_id",rawId);v.put("created_at",now);s.insertWithOnConflict("notification_delivery_dedup",null,v,SQLiteDatabase.CONFLICT_IGNORE);}
    private static String deliveryKey(String streamKey,String eventType,String hash,long when){return Fingerprint.text(streamKey+"|"+eventType+"|"+hash+"|"+when);}
