...
public class CortexOrbBriefActivity extends SatinBriefActivity {
    ...
    @Override View audioCard(PrimeBriefStore.Snapshot s){
        ...
        LinearLayout top=new LinearLayout(this);top.setGravity(Gravity.CENTER_VERTICAL);BriefGlyphView source=new BriefGlyphView(this,"waveRing",CortexUi.ACCENT);source.setBackground(CortexUi.round(this,CortexUi.SURFACE,Color.argb(20,255,255,255),18));top.addView(source,new LinearLayout.LayoutParams(dp(62),dp(62)));
        ...
        LinearLayout hero=new LinearLayout(this);hero.setOrientation(LinearLayout.VERTICAL);hero.setGravity(Gravity.CENTER_HORIZONTAL);hero.setPadding(0,dp(12),0,dp(7));recordOrb=new CortexRingButton(this);recordOrb.setGlyph(CortexRingButton.Glyph.RECORD);recordOrb.setAccent(CortexUi.SIGNAL);hero.addView(recordOrb,new LinearLayout.LayoutParams(dp(112),dp(112)));recordStatus=CortexUi.plain(this,"Record new memory",11,CortexUi.MUTED);CortexUi.medium(recordStatus);recordStatus.setGravity(Gravity.CENTER);recordStatus.setPadding(0,dp(3),0,0);hero.addView(recordStatus,new LinearLayout.LayoutParams(-1,dp(28)));TextView hint=CortexUi.plain(this,"CAPTURE IS PRIMARY  ·  PLAYBACK STAYS ABOVE",8,CortexUi.FAINT);hint.setGravity(Gravity.CENTER);hero.addView(hint,new LinearLayout.LayoutParams(-1,dp(22)));card.addView(hero);recordOrb.setOnClickListener(v->toggleRecordOrb());syncRecordOrb();ui.removeCallbacks(recordPulse);ui.post(recordPulse);
