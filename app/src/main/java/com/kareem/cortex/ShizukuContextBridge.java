...
public final class ShizukuContextBridge {
    ...
    public static Snapshot captureProcessSnapshot(Context context,VaultDb db){
        if(context==null||db==null)return new Snapshot(false,0,0,"Missing Cortex context/database");if(!available())return new Snapshot(false,0,0,"Shizuku service is not running");if(!granted())return new Snapshot(false,0,0,"Shizuku permission is not granted");Object process=null;try{
            String[] cmd={"/system/bin/sh","-c","ps -A -o USER,PID,NAME 2>/dev/null || ps -A"};Method start=Shizuku.class.getDeclaredMethod("newProcess",String[].class,String[].class,String.class);start.setAccessible(true);process=start.invoke(null,new Object[]{cmd,null,null});if(process==null)return new Snapshot(false,0,0,"Shizuku did not create the read-only process snapshot");InputStream in=(InputStream)invoke(process,"getInputStream");String output=readBounded(in,MAX_OUTPUT_BYTES);int exit=waitFor(process);ArrayList<Proc> rows=parse(output);if(rows.isEmpty())return new Snapshot(false,0,0,"No process rows could be parsed; previous running-process state was kept unchanged");ArrayList<PhoneContextStore.ProcessInfo> current=new ArrayList<>();LinkedHashSet<String> names=new LinkedHashSet<>();for(Proc p:rows){if(names.size()>=MAX_ROWS)break;if(p.name.isEmpty()||!names.add(p.name))continue;current.add(new PhoneContextStore.ProcessInfo(p.name,labelForProcess(context,p.name),p.user,p.pid));}if(current.isEmpty())return new Snapshot(false,rows.size(),0,"Process output contained no usable names; previous state was kept unchanged");long now=System.currentTimeMillis();int active=PhoneContextStore.updateProcessSnapshot(db,current,"shizuku",now);JSONObject summary=new JSONObject();summary.put("exit_code",exit);summary.put("parsed_processes",rows.size());summary.put("active_unique",active);summary.put("read_only",true);summary.put("shizuku_uid",safeUid());PhoneContextStore.record(db,"process_snapshot","shizuku","","System process snapshot","","snapshot",active+" visible running process name(s)",now,summary);return new Snapshot(exit==0||!rows.isEmpty(),rows.size(),active,(exit==0?"Read-only Shizuku process snapshot complete":"Process command returned "+exit)+" · "+rows.size()+" parsed · "+active+" active unique process name(s)");
        }catch(Throwable e){return new Snapshot(false,0,0,e.getClass().getSimpleName()+": "+safe(rootMessage(e)));}finally{destroy(process);}}
