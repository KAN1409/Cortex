...
public final class CognitiveDeepBrainPacketBuilderV4 {
    ...
    public static Packet build(Context context,VaultDb db,String question) {
        ...
        try {
            ...
            JSONObject compact=compact(root);
            String compactContextJson=compact.toString();
            String shareText=CognitiveDeepBrainProtocolV4.buildShareText(requestId,q,root);
            String compactText=CognitiveDeepBrainProtocolV4.buildCompactPasteText(requestId,q,compact);
            String exportJson=CognitiveDeepBrainProtocolV4.buildJsonFile(requestId,q,compact);
    ...
    private static JSONObject compact(JSONObject full)throws Exception{
        ...
        out.put("request_id",full.optString("request_id",""));out.put("generated_at",full.optLong("generated_at",0));out.put("question",full.optString("question",""));
        JSONObject phone=full.optJSONObject("current_phone_context");if(phone!=null&&phone.length()>0)out.put("phone",phone);
        JSONArray sOut=new JSONArray(),s=full.optJSONArray("situations");if(s!=null)for(int i=0;i<s.length();i++){JSONObject x=s.optJSONObject(i);if(x==null)continue;JSONObject y=new JSONObject();copy(y,x,"id","kind","state","headline","relevant_from","relevant_until","attention_score","interruption_score","confidence");String e=clip(x.optString("explanation",""),260);if(!e.isEmpty())y.put("explanation",e);sOut.put(y);}out.put("situations",sOut);
        ...
        JSONArray fOut=new JSONArray(),f=full.optJSONArray("facts");if(f!=null)for(int i=0;i<f.length();i++){JSONObject x=f.optJSONObject(i);if(x==null)continue;JSONObject y=new JSONObject();copy(y,x,"id","subject_world_id","predicate","grounding","confidence","valid_from","valid_until");y.put("value",clip(x.optString("value",""),280));fOut.put(y);}out.put("facts",fOut);
        return out;
    }
