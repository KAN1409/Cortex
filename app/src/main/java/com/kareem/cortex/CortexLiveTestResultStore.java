...
/** Tiny local-only result ledger for explicit protected live tests. No secrets or binary data. */
public final class CortexLiveTestResultStore {
    ...
    public static void record(Context c,String key,boolean pass,String detail){record(c,key,pass?"PASS":"FAIL",detail);}
    public static void record(Context c,String key,String status,String detail){if(c==null||key==null||key.trim().isEmpty())return;try{JSONObject o=new JSONObject().put("key",key).put("status",status==null?"":status).put("detail",detail==null?"":detail).put("at",System.currentTimeMillis());prefs(c).edit().putString(key,o.toString()).apply();}catch(Exception ignored){}}
