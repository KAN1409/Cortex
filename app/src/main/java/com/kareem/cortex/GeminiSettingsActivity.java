...
public class GeminiSettingsActivity extends Activity {
    ...
    private void show(){
        ...
        TextView note=new TextView(this);note.setText("Gemini powers configured cloud routes such as Brain External/Combined and strong visual understanding. The API key is encrypted with Android Keystore. The generation model can be changed at runtime without rebuilding Cortex.");note.setTextSize(15);box.addView(note);
        ...
        EditText modelField=new EditText(this);modelField.setHint("Generation model");modelField.setSingleLine(true);modelField.setText(GeminiModelConfig.generationModel(this));modelField.setSelection(modelField.length());box.addView(modelField);
