...
public class VisualIntelligenceWorker extends Worker {
    ...
    @NonNull @Override public Result doWork(){Context ctx=getApplicationContext();VaultDb db=new VaultDb(ctx);VisualInsightStore.ensure(db);long start=VisualInsightStore.backgroundStart(ctx);int done=0;try{
        while(done<BATCH&&!isStopped()){
            ...
            if(t.sensitive){VisualInsightStore.saveState(db,k.id,"local_only","local_only",t.reason);VisualInsightStore.setWorker(ctx,"running",k.id,"Protected locally",t.reason);done++;continue;}
            if(t.ephemeral&&t.valueScore<25){VisualInsightStore.saveState(db,k.id,"skipped","safe",t.reason);VisualInsightStore.setWorker(ctx,"running",k.id,"Skipped low-value screen",t.reason);done++;continue;}
