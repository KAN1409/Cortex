...
public final class FeatureStore {
    ...
    public static void snooze(VaultDb db,long itemId,long until){ensure(db);ensureRow(db,itemId);ContentValues v=new ContentValues();v.put("snoozed_until",until);v.put("updated_at",System.currentTimeMillis());db.getWritableDatabase().update("smart_inbox",v,"item_id=?",new String[]{String.valueOf(itemId)});}
    public static void dismissAttention(VaultDb db,long itemId){ensure(db);ensureRow(db,itemId);ContentValues v=new ContentValues();v.put("attention_dismissed",1);v.put("reviewed",1);v.put("updated_at",System.currentTimeMillis());db.getWritableDatabase().update("smart_inbox",v,"item_id=?",new String[]{String.valueOf(itemId)});}
    ...
    private static boolean hasAny(String t,String... keys){for(String k:keys)if(t.contains(k.toLowerCase(Locale.US)))return true;return false;}
    private static String join(ArrayList<String> xs){StringBuilder s=new StringBuilder();for(String x:xs){if(x==null||x.isEmpty())continue;if(s.length()>0)s.append(" • ");s.append(x);}return s.toString();}
