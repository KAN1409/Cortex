...
public final class CortexUi {
    ...
    public static TextView eyebrow(Activity a,String label,int color){TextView v=plain(a,label==null?"":label.toUpperCase(java.util.Locale.ROOT),9,color);medium(v);if(Build.VERSION.SDK_INT>=21)v.setLetterSpacing(.13f);return v;}
    ...
    public static int semanticFor(String k){if(k==null)return LIME;k=k.toLowerCase(java.util.Locale.ROOT);if(k.contains("urgent")||k.contains("review")||k.contains("failed")||k.contains("error"))return RED;if(k.contains("wait")||k.contains("remind")||k.contains("process"))return YELLOW;if(k.contains("input")||k.contains("capture")||k.contains("play"))return ORANGE;if(k.contains("people")||k.contains("project")||k.contains("useful")||k.contains("complete")||k.contains("ready"))return LIME;return OLIVE;}
