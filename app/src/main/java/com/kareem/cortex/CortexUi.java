...
public final class CortexUi {
    ...
    public static void medium(TextView v){v.setTypeface(Typeface.create("sans-serif-medium",Typeface.NORMAL));}public static void bold(TextView v){v.setTypeface(Typeface.create("sans-serif",Typeface.BOLD));}
    ...
    public static TextView action(Activity a,String label,int color,boolean filled){TextView b=plain(a,label,12,color);medium(b);b.setGravity(Gravity.CENTER);b.setMinHeight(dp(a,48));b.setMinWidth(dp(a,48));b.setPadding(dp(a,15),0,dp(a,15),0);int wash=Color.argb(filled?28:6,Color.red(color),Color.green(color),Color.blue(color));int stroke=Color.argb(filled?110:48,Color.red(color),Color.green(color),Color.blue(color));pressable(a,b,round(a,wash,stroke,R_CONTROL));return b;}
    ...
    public static LinearLayout header(Activity a,String eyebrow,String title,int accent,String actionLabel,View.OnClickListener action){
        ...
        if(actionLabel!=null&&!actionLabel.trim().isEmpty()){TextView b=chip(a,actionLabel,MUTED,false);b.setMinHeight(dp(a,48));b.setMinWidth(dp(a,48));if(action!=null)b.setOnClickListener(action);row.addView(b,new LinearLayout.LayoutParams(-2,dp(a,48)));}
        return row;
    }
    ...
    public static void addBottomNav(Activity a,LinearLayout root,String selected,Runnable ignored){
        ...
        addNav(a,bar,"now","Now",cur,CortexDestinationRegistry.NOW);addNav(a,bar,"memory","Memory",cur,CortexDestinationRegistry.MEMORY);addCenter(a,bar);addNav(a,bar,"work","Work",cur,CortexDestinationRegistry.WORK);addNav(a,bar,"capture","Capture",cur,CortexDestinationRegistry.CAPTURE);
    ...
    private static void addNav(Activity a,LinearLayout row,String key,String label,String selected,String destinationId){
        ...
        item.setOnClickListener(v->{if(on)return;CortexMotion.haptic(v,false);if(CortexMotion.allowed(a))v.animate().scaleX(.94f).scaleY(.94f).setDuration(80).withEndAction(()->CortexNavigation.openPrimary(a,destinationId)).start();else CortexNavigation.openPrimary(a,destinationId);});row.addView(item,new LinearLayout.LayoutParams(0,-1,1));
    }
