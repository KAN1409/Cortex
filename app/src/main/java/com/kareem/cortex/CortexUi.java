...
public final class CortexUi {
    ...
    public static TextView action(Activity a,String l,int c,boolean filled){TextView b=plain(a,l,14,filled?BG:c);medium(b);b.setGravity(Gravity.CENTER);b.setPadding(dp(a,14),0,dp(a,14),0);GradientDrawable base=filled?round(a,BRAND,Color.TRANSPARENT,14):round(a,Color.argb(10,Color.red(c),Color.green(c),Color.blue(c)),Color.argb(60,Color.red(c),Color.green(c),Color.blue(c)),14);pressable(a,b,base);return b;}
    ...
    public static void addBottomNav(Activity a,LinearLayout root,String selected,Runnable ignored){
        String current=primeKey(selected);LinearLayout bar=new LinearLayout(a);bar.setGravity(Gravity.CENTER);bar.setPadding(dp(a,9),dp(a,4),dp(a,9),dp(a,4));bar.setBackground(round(a,BG,BORDER,23));
        ...
        LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(-1,dp(a,68));p.setMargins(dp(a,16),dp(a,3),dp(a,16),dp(a,8));root.addView(bar,p);fitSystemBars(a,root);
    }
    private static void addCapture(Activity a,LinearLayout bar){
        LinearLayout outer=new LinearLayout(a);outer.setGravity(Gravity.CENTER);outer.setBackground(round(a,Color.rgb(18,19,16),Color.argb(80,185,217,74),999));
        TextView plus=plain(a,"+",33,BG);plus.setTypeface(Typeface.create("sans-serif-light",Typeface.NORMAL));plus.setGravity(Gravity.CENTER);pressable(a,plus,round(a,BRAND,Color.argb(100,245,247,241),999));raised(a,outer,4);
        ...
        outer.addView(plus,new LinearLayout.LayoutParams(dp(a,54),dp(a,54)));LinearLayout wrap=new LinearLayout(a);wrap.setGravity(Gravity.CENTER);wrap.addView(outer,new LinearLayout.LayoutParams(dp(a,60),dp(a,60)));bar.addView(wrap,new LinearLayout.LayoutParams(dp(a,68),-1));
    }
    ...
    private static void addNav(Activity a,LinearLayout bar,String key,String label,String glyph,String selected,Class<?> target){boolean on=key.equals(selected);LinearLayout item=new LinearLayout(a);item.setOrientation(LinearLayout.VERTICAL);item.setGravity(Gravity.CENTER);CortexGlyphView g=glyph(a,glyph,on?BRAND:MUTED,false);item.addView(g,new LinearLayout.LayoutParams(dp(a,29),dp(a,29)));TextView t=plain(a,label,9,on?BRAND:MUTED);if(on)medium(t);t.setGravity(Gravity.CENTER);item.addView(t,new LinearLayout.LayoutParams(-1,dp(a,16)));if(on){View line=new View(a);line.setBackground(round(a,BRAND,Color.TRANSPARENT,999));item.addView(line,new LinearLayout.LayoutParams(dp(a,24),dp(a,2)));}if(!on)item.setOnClickListener(x->{try{Intent i=new Intent(a,target);i.addFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT|Intent.FLAG_ACTIVITY_SINGLE_TOP);a.startActivity(i);}catch(Throwable ignored){}});LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(0,-1,1);p.setMargins(dp(a,1),0,dp(a,1),0);bar.addView(item,p);}
}
