...
/** Cortex premium AMOLED design system: graphite depth, restrained lime, strong information hierarchy. */
public final class CortexUi {
    public static final int BG=Color.rgb(3,4,3), SURFACE=Color.rgb(10,12,10), SURFACE_2=Color.rgb(15,18,15), SURFACE_3=Color.rgb(21,25,20);
    public static final int TEXT=Color.rgb(246,248,242), MUTED=Color.rgb(164,169,159), FAINT=Color.rgb(94,100,91);
    ...
    public static final int BORDER=Color.rgb(42,47,41), BORDER_SOFT=Color.rgb(25,29,25), HAIRLINE=Color.argb(46,220,230,210);
    ...
    public static View pressable(Activity a,View v,GradientDrawable base){
        if(Build.VERSION.SDK_INT>=21)v.setBackground(new RippleDrawable(ColorStateList.valueOf(Color.argb(24,190,221,82)),base,null));else v.setBackground(base);v.setClickable(true);v.setFocusable(true);raised(a,v,1);
    ...
    public static TextView chip(Activity a,String label,int color,boolean strong){TextView v=plain(a,label,strong?11:10,color);if(strong)medium(v);v.setGravity(Gravity.CENTER);v.setMinHeight(dp(a,28));v.setPadding(dp(a,12),0,dp(a,12),0);int fill=Color.argb(strong?22:9,Color.red(color),Color.green(color),Color.blue(color));int stroke=Color.argb(strong?92:48,Color.red(color),Color.green(color),Color.blue(color));v.setBackground(round(a,fill,stroke,999));return raised(a,v,strong?1:0);}
    ...
    public static TextView action(Activity a,String label,int color,boolean filled){TextView b=plain(a,label,12,color);medium(b);b.setGravity(Gravity.CENTER);b.setMinHeight(dp(a,44));b.setPadding(dp(a,15),0,dp(a,15),0);int wash=Color.argb(filled?32:8,Color.red(color),Color.green(color),Color.blue(color));int stroke=Color.argb(filled?118:56,Color.red(color),Color.green(color),Color.blue(color));pressable(a,b,round(a,wash,stroke,R_CONTROL));return b;}
    ...
    public static LinearLayout metric(Activity a,String value,String label,int color){LinearLayout box=new LinearLayout(a);box.setOrientation(LinearLayout.VERTICAL);box.setGravity(Gravity.CENTER_VERTICAL);box.setPadding(dp(a,12),dp(a,10),dp(a,12),dp(a,10));box.setBackground(round(a,Color.argb(6,255,255,255),BORDER_SOFT,R_CONTROL));TextView v=plain(a,value,20,color);medium(v);box.addView(v);TextView l=plain(a,label,9,MUTED);if(Build.VERSION.SDK_INT>=21)l.setLetterSpacing(.04f);l.setPadding(0,dp(a,3),0,0);box.addView(l);return box;}
