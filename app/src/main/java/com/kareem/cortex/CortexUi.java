...
public final class CortexUi {
    // Palette locked from approved reference.
    public static final int BG=Color.rgb(10,10,9);          // #0A0A09
    public static final int SURFACE=Color.rgb(26,28,25);    // #1A1C19
    public static final int SURFACE_2=SURFACE;
    public static final int SURFACE_3=SURFACE;
    public static final int TEXT=Color.rgb(245,247,241);    // #F5F7F1
    public static final int MUTED=Color.rgb(161,163,154);   // #A1A39A
    public static final int FAINT=Color.argb(138,161,163,154);
    public static final int BRAND=Color.rgb(185,217,74);    // #B9D94A
    public static final int AURORA=Color.rgb(190,170,46);   // #BEAA2E
    public static final int LIME=BRAND,OLIVE=AURORA;

    // Semantic dots only; these are not theme accents.
    ...
    /** Preserve IME-safe layout: the composer must stay above the keyboard. */
    ...
    // Reference typography scale: 22–24 display, 18 section, 16 card, 14 body, 12 caption, 10 overline.
    ...
    public static TextView display(Activity a,String s){TextView v=text(a,s,23,BRAND);medium(v);return v;}
    ...
    /** Exact reference dock: Now / Inbox / + / Atlas / Ask. */
    public static void addBottomNav(Activity a,LinearLayout root,String selected,Runnable ignored){
        String current=primeKey(selected);LinearLayout bar=new LinearLayout(a);bar.setGravity(Gravity.CENTER);bar.setPadding(dp(a,8),dp(a,5),dp(a,8),dp(a,5));bar.setBackground(round(a,BG,BORDER,25));
        ...
        LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(-1,dp(a,76));p.setMargins(dp(a,14),dp(a,4),dp(a,14),dp(a,9));root.addView(bar,p);fitSystemBars(a,root);
    }
    private static void addCapture(Activity a,LinearLayout bar){LinearLayout wrap=new LinearLayout(a);wrap.setGravity(Gravity.CENTER);TextView v=plain(a,"+",39,BG);v.setTypeface(Typeface.create("sans-serif-light",Typeface.NORMAL));v.setGravity(Gravity.CENTER);CortexUi.pressable(a,v,round(a,BRAND,Color.argb(105,245,247,241),999));CortexUi.raised(a,v,4);v.setOnClickListener(x->{try{Intent i=new Intent(a,InputActivity.class);i.addFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT|Intent.FLAG_ACTIVITY_SINGLE_TOP);a.startActivity(i);}catch(Throwable ignored){}});wrap.addView(v,new LinearLayout.LayoutParams(dp(a,62),dp(a,62)));bar.addView(wrap,new LinearLayout.LayoutParams(dp(a,72),-1));}
    ...
    private static void addNav(Activity a,LinearLayout bar,String key,String label,String glyph,String selected,Class<?> target){boolean on=key.equals(selected);LinearLayout item=new LinearLayout(a);item.setOrientation(LinearLayout.VERTICAL);item.setGravity(Gravity.CENTER);CortexGlyphView g=glyph(a,glyph,on?BRAND:MUTED,false);item.addView(g,new LinearLayout.LayoutParams(dp(a,34),dp(a,34)));TextView t=plain(a,label,10,on?BRAND:MUTED);if(on)medium(t);t.setGravity(Gravity.CENTER);item.addView(t,new LinearLayout.LayoutParams(-1,dp(a,18)));if(on){View line=new View(a);line.setBackground(round(a,BRAND,Color.TRANSPARENT,999));item.addView(line,new LinearLayout.LayoutParams(dp(a,30),dp(a,3)));}if(!on)item.setOnClickListener(x->{try{Intent i=new Intent(a,target);i.addFlags(Intent.FLAG_ACTIVITY_REORDER_TO_FRONT|Intent.FLAG_ACTIVITY_SINGLE_TOP);a.startActivity(i);}catch(Throwable ignored){}});LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(0,-1,1);p.setMargins(dp(a,1),0,dp(a,1),0);bar.addView(item,p);}
}
