...
public final class SelfContainedReviewService extends Service {
    ...
    @Override public int onStartCommand(Intent intent,int flags,int startId){
        ...
        running=true;
        setState("RUNNING","Starting phone-only review…","");
        startForeground(NOTIFICATION_ID,notification("Starting phone-only review…"));
        try{
            PowerManager pm=(PowerManager)getSystemService(POWER_SERVICE);
            if(pm!=null){wakeLock=pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK,"Cortex:PhoneOnlyReview");wakeLock.setReferenceCounted(false);wakeLock.acquire(45L*60L*1000L);}
        }catch(Throwable ignored){}
        new Thread(()->runOnce(startId),"cortex-phone-review-service").start();
    ...
    private void runOnce(int startId){
        try{
            ...
            String path="Downloads/Cortex/"+root.getName()+".zip";
            setState("DONE","DONE\nSaved to "+path+(uri==null?"":"\nMediaStore: "+uri),path);
            updateNotification("Review complete — ZIP saved");
        }catch(Throwable t){
            setState("FAILED","FAILED\n"+t.getClass().getSimpleName()+": "+String.valueOf(t.getMessage()),"");
            updateNotification("Review failed — open Cortex for details");
        }finally{
            running=false;
    ...
    private void progress(String s){
        ...
        String next=(old==null||old.isEmpty()?"":old+"\n")+s;
        setState("RUNNING",next,"");
        updateNotification(s);
    }

    static void start(Context c){
        Intent i=new Intent(c,SelfContainedReviewService.class);
    ...
    private void ensureChannel(){
        if(Build.VERSION.SDK_INT>=26){NotificationManager nm=(NotificationManager)getSystemService(NOTIFICATION_SERVICE);if(nm!=null)nm.createNotificationChannel(new NotificationChannel(CHANNEL,"Cortex phone-only review",NotificationManager.IMPORTANCE_LOW));}
    }
    private Notification notification(String text){
        Intent open=new Intent(this,StableSelfContainedReviewActivity.class).addFlags(Intent.FLAG_ACTIVITY_NEW_TASK|Intent.FLAG_ACTIVITY_SINGLE_TOP);
        PendingIntent pi=PendingIntent.getActivity(this,0,open,PendingIntent.FLAG_UPDATE_CURRENT|PendingIntent.FLAG_IMMUTABLE);
        Notification.Builder b=Build.VERSION.SDK_INT>=26?new Notification.Builder(this,CHANNEL):new Notification.Builder(this);
        return b.setContentTitle("Cortex self-contained review").setContentText(text).setSmallIcon(android.R.drawable.stat_notify_sync).setOngoing(true).setContentIntent(pi).build();
    }
