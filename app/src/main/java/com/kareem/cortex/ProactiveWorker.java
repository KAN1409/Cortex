...
public final class ProactiveWorker extends Worker {
    ...
    @NonNull @Override public Result doWork(){if(StartupSafetyGate.active())return Result.success();try{VaultDb db=new VaultDb(getApplicationContext());ArrayList<ProactiveSignal> xs=ProactiveEngine.scan(getApplicationContext(),db,4);if(!xs.isEmpty()){CortexNotifications.digest(getApplicationContext(),xs);ProactiveEngine.markSurfaced(getApplicationContext(),xs);}db.close();return Result.success();}catch(Exception e){return Result.retry();}}
}
