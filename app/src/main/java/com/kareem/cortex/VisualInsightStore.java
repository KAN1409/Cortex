...
public final class VisualInsightStore {
    public static final int PIPELINE_VERSION=50;
    // Keep the original background-start preference so an app upgrade does not suddenly cloud-process the historical library.
    ...
    /** New screenshots, resumable rate-limited work, plus one automatic retry for pre-v50 failures. */
    public static KnowledgeItem nextBackground(VaultDb db,long start){ensure(db);Cursor c=db.getReadableDatabase().rawQuery("SELECT k.* FROM knowledge_items k WHERE k.source='screenshot-folder' AND k.type IN ('SCREENSHOT','IMAGE') AND k.status='analyzed' AND k.created_at>=? AND (NOT EXISTS(SELECT 1 FROM visual_insights v WHERE v.item_id=k.id) OR EXISTS(SELECT 1 FROM visual_insights v WHERE v.item_id=k.id AND ((v.status='failed' AND v.pipeline_version<?) OR v.status='rate_limited'))) ORDER BY k.created_at ASC LIMIT 1",new String[]{String.valueOf(start),String.valueOf(PIPELINE_VERSION)});KnowledgeItem k=c.moveToFirst()?item(c):null;c.close();return k;}
