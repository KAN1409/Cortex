...
public final class CognitivePulseProjectionV4 {
    ...
    public static Snapshot current(VaultDb db,int limit){
        ...
        try{
            while(c.moveToNext()){
                String actions=c.getString(14)==null?"":c.getString(14);long createdAt=c.getLong(13);
                ...
                boolean newSinceBrain=CognitiveReasoningFreshnessV4.isNew(createdAt,freshness.latestAppliedAt);
                candidates.add(new Item(c.getString(0),c.getString(1),c.getString(2),c.getString(3),c.getString(4),c.getDouble(5),e.nowScore,c.getDouble(6),c.getDouble(7),c.getLong(8),c.getLong(9),c.getInt(10),e.currentDeepBrain?c.getString(11):"",actions,e.currentDeepBrain,e.brainFreshness,createdAt,newSinceBrain));
            }
        }finally{c.close();}
    ...
    public static final class Item{
        ...
        public final double interruptionScore,confidence,brainFreshness;public final long relevantFrom,relevantUntil,createdAt;public final int deepBrainRank;public final boolean newSinceDeepBrain;private final boolean currentDeepBrain;
        Item(String id,String kind,String state,String headline,String explanation,double canonicalAttention,double nowScore,double interruption,double confidence,long from,long until,int rank,String reason,String actions,boolean currentDeepBrain,double brainFreshness,long createdAt,boolean newSinceDeepBrain){this.situationId=id;this.kind=kind;this.state=state;this.headline=headline==null?"":headline;this.explanation=explanation==null?"":explanation;this.canonicalAttentionScore=canonicalAttention;this.attentionScore=nowScore;this.interruptionScore=interruption;this.confidence=confidence;this.relevantFrom=from;this.relevantUntil=until;this.deepBrainRank=rank;this.deepBrainReason=reason==null?"":reason;this.actions=actions==null?"":actions;this.currentDeepBrain=currentDeepBrain;this.brainFreshness=brainFreshness;this.createdAt=createdAt;this.newSinceDeepBrain=newSinceDeepBrain;}
