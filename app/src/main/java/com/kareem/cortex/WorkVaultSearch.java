...
public final class WorkVaultSearch {
    public static final String VERSION="work_vault_search_005";
    ...
    public static ArrayList<Hit> search(VaultDb vault,String query,int limit){
        ...
        Cursor c=db.rawQuery("SELECT c.file_id,f.display_name,f.document_uri,c.chunk_text,c.sheet_name,c.page_number,c.slide_number,c.row_number FROM work_chunks c JOIN work_files f ON f.id=c.file_id WHERE c.version_id=(SELECT v.id FROM work_file_versions v WHERE v.file_id=c.file_id ORDER BY v.parsed_at DESC,v.id DESC LIMIT 1) AND (c.chunk_text LIKE ? OR f.display_name LIKE ?) LIMIT 80",new String[]{like,like});
        ...
        c=db.rawQuery("SELECT u.file_id,f.display_name,f.document_uri,u.reference_type,u.reference_value,u.item_name,u.status,u.status_normalized,u.owner_name,u.due_text,u.remarks,u.vendor_name,u.sheet_name,u.page_number,u.row_number FROM work_followup_records u JOIN work_files f ON f.id=u.file_id WHERE u.reference_value LIKE ? OR u.item_name LIKE ? OR u.status LIKE ? OR u.status_normalized LIKE ? OR u.owner_name LIKE ? OR u.due_text LIKE ? OR u.remarks LIKE ? OR u.vendor_name LIKE ? OR f.display_name LIKE ? LIMIT 80",new String[]{like,like,like,like,like,like,like,like,like});
