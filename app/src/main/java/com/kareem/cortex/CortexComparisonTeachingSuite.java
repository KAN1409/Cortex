...
/**
 * 1000-case comparison benchmark over the Gmail bridge.
 * Every case comes from CortexTrueScenarioCatalog: 20 semantic domains x 10 intents x 5 domain-aware complications.
 * No canonical writes and no direct policy activation happen here.
 */
public final class CortexComparisonTeachingSuite {
    public static final String VERSION = "comparison_teaching_002_true_1000";
            ...
            try {
                ...
                for (int offset = 0; offset < BATCH_SIZE; offset++) {
                    ...
                    CortexAttentionJudge.Judgment j = CortexAttentionJudge.evaluate(
                            context, c, new CortexAttentionJudge.RuntimeContext(s.contextMatch, s.interruptionCost));
                    inputs.put(s.toJson());
                    references.put(new JSONObject()
                            .put("caseId", s.caseId)
                            .put("referenceTruth", s.referenceTruth));
                    outputs.put(new JSONObject()
                            .put("caseId", s.caseId)
                            .put("surfaceNow", j.surfaceNow)
                            .put("score", j.score)
                            .put("threshold", j.threshold)
                            .put("reason", j.reason)
                            .put("policyVersion", j.policyVersion));
                }

                JSONObject original = new JSONObject()
                        .put("kind", "CORTEX_TRUE_1000_SCENARIO_COMPARISON")
                        ...
                        .put("cases", inputs)
                        .put("instruction",
                                "Solve each case independently from original evidence before looking at Cortex output. " +
                                "Compare final judgment to Cortex and to reference boundaries. For file cases separately verify receive/open/MIME/permission/parser/version/provenance semantics. " +
                                "Do not invent evidence. Return aggregate metrics plus only material disagreements and the top lessons.");

                JSONObject cortex = new JSONObject()
                        .put("engine", CortexAttentionJudge.VERSION)
                        .put("results", outputs);

                JSONObject reference = new JSONObject()
                        .put("caseReferences", references)
                        .put("comparisonContract", new JSONObject()
                                .put("originalEvidenceIsGroundingSource", true)
                                .put("resolvedMustNotInterrupt", true)
                                .put("lowConfidenceMustNotInterrupt", true)
                                .put("technicalEvidenceMustStayBelowAttention", true)
                                .put("unverifiedFileMustNotBeTreatedAsVerified", true)
                        ...
                        .put("requestedResponse", new JSONObject()
                                .put("metrics", new JSONArray()
                                        .put("totalCases").put("agreementCount").put("agreementRate")
                                        .put("falseInterruptions").put("missedUrgentCases")
                                        .put("criticalDisagreements").put("groundingViolations")
                                        .put("fileCases").put("fileFlowFailures").put("continuityFailures"))
                                .put("materialDisagreementsOnly", true)
                                .put("topLessonsLimit", 10)
                ...
                ChatGptBridgeCoordinator.DispatchResult r = coordinator.dispatchTest(
                        runId,
                        String.format(Locale.US, "true-comparison-batch-%02d", batch + 1),
                        original, cortex, reference);
                if (r.sent) sent++; else failed++;
                dispatched.put(new JSONObject()
                        .put("batch", batch + 1)
                        .put("sent", r.sent)
                        .put("requestId", r.request == null ? "" : r.request.optString("requestId", ""))
                        .put("transport", r.transport)
                        .put("error", r.error));
            } catch (Throwable t) {
                failed++;
                try { dispatched.put(new JSONObject().put("batch",batch+1).put("sent",false)
    ...
    private static AttentionDecisionEngine.Candidate candidate(int index, CortexTrueScenarioCatalog.Scenario s) {
