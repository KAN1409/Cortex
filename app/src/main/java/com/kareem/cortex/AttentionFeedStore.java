...
public final class AttentionFeedStore {
    ...
    public static void upsertLoop(VaultDb db,OpenLoopStore.Loop loop,AttentionModels.Decision d){
        ...
        long now=System.currentTimeMillis();String section=section(loop,d),variant="ACTIVE",dot=d.assessment.level==AttentionModels.Level.NONE?"":"NEEDS_ATTENTION",group=loop.threadId>0?"thread:"+loop.threadId:"loop:"+loop.id;
    ...
    public static List<Item> loadNow(VaultDb db,int limit){return loadSection(db,"NOW",limit,true);}
    public static List<Item> loadComingUp(VaultDb db,int limit){return loadSection(db,"COMING_UP",limit,true);}
    public static List<Item> loadSection(VaultDb db,String section,int limit,boolean diverse){CortexAttentionSchema.ensure(db);ArrayList<Item> candidates=new ArrayList<>();int scan=Math.max(limit*6,24);Cursor c=db.getReadableDatabase().query("attention_feed",cols(),"section=? AND rank>=0",new String[]{section},null,null,"rank DESC,updated_at DESC",String.valueOf(scan));while(c.moveToNext())candidates.add(new Item(c));c.close();if(!diverse||candidates.size()<=1)return trim(candidates,limit);ArrayList<Item> out=new ArrayList<>();HashMap<String,Integer> people=new HashMap<>(),projects=new HashMap<>();HashSet<String> groups=new HashSet<>();for(Item x:candidates){if(out.size()>=limit)break;if(!x.semanticGroup.isEmpty()&&!groups.add(x.semanticGroup))continue;if(!x.personKey.isEmpty()&&people.getOrDefault(x.personKey,0)>=2)continue;if(!x.projectKey.isEmpty()&&projects.getOrDefault(x.projectKey,0)>=2)continue;out.add(x);if(!x.personKey.isEmpty())people.put(x.personKey,people.getOrDefault(x.personKey,0)+1);if(!x.projectKey.isEmpty())projects.put(x.projectKey,projects.getOrDefault(x.projectKey,0)+1);}return out;}

    private static ArrayList<Item> trim(ArrayList<Item> xs,int limit){if(xs.size()<=limit)return xs;return new ArrayList<>(xs.subList(0,limit));}
    ...
    private static void recordSurface(VaultDb db,long loopId,long now){SQLiteDatabase sql=db.getWritableDatabase();Cursor c=sql.query("attention_history",new String[]{"first_surfaced_at","surface_count","opened_count","dismissed_count","snoozed_until","last_action_at"},"entity_type='open_loop' AND entity_id=?",new String[]{String.valueOf(loopId)},null,null,null,"1");long first=now,snooze=0,lastAction=0;int count=0,opened=0,dismissed=0;if(c.moveToFirst()){first=c.getLong(0);count=c.getInt(1);opened=c.getInt(2);dismissed=c.getInt(3);snooze=c.getLong(4);lastAction=c.getLong(5);}c.close();ContentValues v=new ContentValues();v.put("entity_type","open_loop");v.put("entity_id",loopId);v.put("first_surfaced_at",first>0?first:now);v.put("last_surfaced_at",now);v.put("surface_count",count+1);v.put("opened_count",opened);v.put("dismissed_count",dismissed);v.put("snoozed_until",snooze);v.put("last_action_at",lastAction);sql.insertWithOnConflict("attention_history",null,v,SQLiteDatabase.CONFLICT_REPLACE);}
