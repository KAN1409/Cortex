...
public final class AttentionFeedStore {
    ...
    public static List<Item> loadNow(VaultDb db,int limit){CortexAttentionSchema.ensure(db);ArrayList<Item> out=new ArrayList<>();Cursor c=db.getReadableDatabase().query("attention_feed",cols(),"section='NOW' AND rank>=0",null,null,null,"rank DESC,updated_at DESC",String.valueOf(Math.max(1,limit)));while(c.moveToNext())out.add(new Item(c));c.close();return out;}
