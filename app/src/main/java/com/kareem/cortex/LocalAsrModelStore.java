...
public final class LocalAsrModelStore {
    // Keep the legacy private filename so an already-imported Medium model survives app updates.
    ...
    public static boolean ready(Context c){return WhisperGgmlModel.inspect(modelFile(c))!=null;}

    public static String profileId(Context c){WhisperGgmlModel.Profile p=WhisperGgmlModel.inspect(modelFile(c));return p==null?"unknown_q8_0":p.id;}

    public static String profileLabel(Context c){WhisperGgmlModel.Profile p=WhisperGgmlModel.inspect(modelFile(c));return p==null?"Local Whisper model":p.label;}

    public static String statusText(Context c){
        File f=modelFile(c);WhisperGgmlModel.Profile profile=WhisperGgmlModel.inspect(f);
        if(profile==null)return "Model: not selected";
        String name=c.getSharedPreferences(PREF,Context.MODE_PRIVATE).getString("source_name",MODEL_FILENAME);
        return String.format(Locale.US,"Model: %s • %s • %.1f MB • Ready",name,profile.label,f.length()/1048576.0);
    }
    ...
    public static File importModel(Context c,Uri uri)throws Exception{
        ...
        if(expected>0&&expected<WhisperGgmlModel.MIN_SUPPORTED_BYTES)throw new IllegalArgumentException("Selected file is too small for a supported Whisper Small Q8_0 model ("+expected+" bytes)");
        ...
        if(written<WhisperGgmlModel.MIN_SUPPORTED_BYTES){tmp.delete();Exception e=new IllegalStateException("Local ASR model copy incomplete ("+written+" bytes)");WhisperRuntimeState.error(app,e);throw e;}
        ...
        WhisperGgmlModel.Profile profile=WhisperGgmlModel.inspect(tmp);
        if(profile==null){tmp.delete();Exception e=new IllegalArgumentException("Selected GGML file is not a supported Whisper Small/Medium Q8_0 model");WhisperRuntimeState.error(app,e);throw e;}
        ...
        app.getSharedPreferences(PREF,Context.MODE_PRIVATE).edit().putString("source_name",sourceName).putString("profile_id",profile.id).putLong("bytes",dst.length()).putLong("imported_at",System.currentTimeMillis()).apply();
