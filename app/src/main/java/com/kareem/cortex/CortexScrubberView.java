...
/** Minimal recessed playback progress track with tap/drag scrubbing. */
public final class CortexScrubberView extends View {
    ...
    public CortexScrubberView(Context c){this(c,null);}public CortexScrubberView(Context c,AttributeSet a){super(c,a);d=getResources().getDisplayMetrics().density;setLayerType(LAYER_TYPE_SOFTWARE,null);track.setColor(CortexUi.BORDER_SOFT);track.setStrokeWidth(6*d);track.setStrokeCap(Paint.Cap.ROUND);played.setColor(CortexUi.ACCENT);played.setStrokeWidth(6*d);played.setStrokeCap(Paint.Cap.ROUND);glow.setColor(CortexUi.ACCENT);glow.setStrokeWidth(7*d);glow.setStrokeCap(Paint.Cap.ROUND);glow.setShadowLayer(7*d,0,0,Color.argb(120,126,158,255));thumb.setColor(CortexUi.ACCENT);thumb.setStyle(Paint.Style.FILL);}
