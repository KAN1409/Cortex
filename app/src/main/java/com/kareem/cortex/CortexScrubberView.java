...
/** Minimal recessed playback progress track with warm raised playback treatment. */
public final class CortexScrubberView extends View {
    ...
    public CortexScrubberView(Context c){this(c,null);}public CortexScrubberView(Context c,AttributeSet a){super(c,a);d=getResources().getDisplayMetrics().density;setLayerType(LAYER_TYPE_SOFTWARE,null);track.setColor(CortexUi.BORDER_SOFT);track.setStrokeWidth(6*d);track.setStrokeCap(Paint.Cap.ROUND);played.setColor(CortexUi.ORANGE);played.setStrokeWidth(6*d);played.setStrokeCap(Paint.Cap.ROUND);glow.setColor(CortexUi.ORANGE);glow.setStrokeWidth(7*d);glow.setStrokeCap(Paint.Cap.ROUND);glow.setShadowLayer(8*d,0,1*d,Color.argb(135,Color.red(CortexUi.ORANGE),Color.green(CortexUi.ORANGE),Color.blue(CortexUi.ORANGE)));thumb.setColor(CortexUi.ORANGE);thumb.setStyle(Paint.Style.FILL);thumb.setShadowLayer(7*d,0,2*d,Color.argb(150,0,0,0));}
