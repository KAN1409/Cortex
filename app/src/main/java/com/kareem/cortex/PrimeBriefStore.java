...
/**
 * Read model for Now. Evidence is append-only, but the product surface is a reconciled set of
 * current situations. SituationLifecycle retires expired/resolved/superseded state before ranking.
 */
public final class PrimeBriefStore {
    ...
    public static Snapshot load(VaultDb db){
        ...
        try{SituationLifecycle.reconcile(db);}catch(Throwable e){DiagnosticsLog.error(db,"PrimeBriefStore","situation_reconcile",e,"SITUATION_LIFECYCLE",0,0,0,0,0,null);}
    ...
    private static ArrayList<Item> attentionCandidates(VaultDb db,int limit){ArrayList<Item> all=new ArrayList<>();Cursor c=db.getReadableDatabase().rawQuery("SELECT id,kind,title,body,source_key,state,confidence,importance,thread_id,anchor_signal_id,updated_at FROM derived_items WHERE state='open' AND kind IN ('ACTION','WAITING','DECISION','REMINDER','ALERT','CHANGE') ORDER BY updated_at DESC LIMIT 400",null);try{while(c.moveToNext()){long rowId=safeLong(c,0);try{Item x=from(c,db);if(!SituationLifecycle.shouldSurface(db,x))continue;if(hardSurfaceNoise(x))continue;if(x.attentionBand!=AttentionEngine.Band.QUIET)all.add(x);}catch(Throwable e){DiagnosticsLog.error(db,"PrimeBriefStore","attention_candidate_row",e,"PRIME_ROW",rowId,0,0,0,0,null);}}}finally{c.close();}all.sort(PrimeBriefStore::compareAttention);return consolidate(all,limit,true);}
    ...
    static boolean hardSurfaceNoise(Item x){
        ...
        if(mechanicalProgress(t))return true;
    ...
    private static boolean mechanicalProgress(String t){
        return (t.contains("deleting item")||t.contains("deleting ")||t.contains("uploading ")||t.contains("downloading ")||t.contains("syncing ")||t.contains("processing "))
                && (t.matches(".*\\b\\d+\\s+of\\s+\\d+\\b.*")||t.contains("%")||t.contains("progress"));
    }
