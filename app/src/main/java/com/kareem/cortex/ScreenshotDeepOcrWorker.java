...
public class ScreenshotDeepOcrWorker extends Worker {
    ...
    @NonNull @Override public Result doWork(){Context ctx=getApplicationContext();VaultDb db=new VaultDb(ctx);OcrPassStore.ensure(db);int done=0;try{
        while(done<BATCH&&!isStopped()){
            ...
            try{MultiPassOcrAnalyzer.Result r=MultiPassOcrAnalyzer.analyze(ctx,k);OcrPassStore.replace(db,k.id,r.passes);db.applyAnalysis(k.id,r.analysis);try{CoreBrainEngine.afterAnalysis(db,k.id);}catch(Exception ignored){}done++;}
            catch(Exception e){OcrPassStore.replace(db,k.id,java.util.Collections.singletonList(new OcrPassStore.Pass("deep_error","multipass_v5","",0,false,e.getClass().getSimpleName()+": "+(e.getMessage()==null?"":e.getMessage()))));done++;}
        }
        if(hasMore(db))ScreenshotDeepOcrScheduler.continueChain(ctx);return Result.success();
    }catch(Exception e){return Result.retry();}finally{db.close();}}
