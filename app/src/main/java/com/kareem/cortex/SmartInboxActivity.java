...
public class SmartInboxActivity extends Activity {
    ...
    void build(){
        ...
        tabNeeds=tab("NEEDS");tabInbox=tab("INBOX");tabWaiting=tab("WAITING");tabActions=tab("ACTIONS");addEq(tabs,tabNeeds,0);addEq(tabs,tabInbox,6);addEq(tabs,tabWaiting,6);addEq(tabs,tabActions,6);root.addView(tabs);
    ...
    void refresh(){int needs=FeatureStore.needsCount(db),inbox=FeatureStore.inbox(db,500).size();statsRow.removeAllViews();addStatChip(needs+" Needs",needs>0?accent:muted);addStatChip(inbox+" Inbox",muted);styleTabs();feed.removeAllViews();ArrayList<FeatureStore.InboxEntry> xs=entries();if(xs.isEmpty()){emptyState();return;}if("needs".equals(mode)){ArrayList<FeatureStore.InboxEntry> urgent=new ArrayList<>(),high=new ArrayList<>(),review=new ArrayList<>();for(FeatureStore.InboxEntry e:xs){if(e.score>=100)urgent.add(e);else if(e.score>=70)high.add(e);else review.add(e);}addGroup("URGENT NOW",urgent,danger);addGroup("HIGH PRIORITY",high,warn);addGroup("NEEDS REVIEW",review,accent);}else for(FeatureStore.InboxEntry e:xs)addEntry(e);}
    ...
    void addEntry(FeatureStore.InboxEntry e){
        ArrayList<String> rawActs=FeatureStore.openActions(db,e.item.id);ArrayList<String> acts=InboxPresentation.actionParts(rawActs);
        ...
        TextView t=tv(InboxPresentation.title(e.item,rawActs),17,text);t.setTypeface(null,1);t.setMaxLines(2);t.setEllipsize(android.text.TextUtils.TruncateAt.END);head.addView(t,new LinearLayout.LayoutParams(0,-2,1));
        ...
        TextView meta=plain(e.bucket+"  •  "+friendlyType(e.item.type)+"  •  "+age(e.item.createdAt),11,muted);meta.setPadding(0,dp(5),0,0);c.addView(meta);addReasonChips(c,e);
        String p=InboxPresentation.preview(e.item,rawActs);if(!empty(p)){TextView body=tv(p,14,text);body.setPadding(0,dp(11),0,0);c.addView(body);}
        if(!acts.isEmpty())addActionRows(c,acts,InboxPresentation.due(rawActs));
        ...
        Button primary=actionButton(acts.isEmpty()?"CLEAR":"DONE",acts.isEmpty()?muted:ok),snooze=actionButton("SNOOZE",muted),pin=actionButton(e.pinned?"UNPIN":"PIN",e.pinned?accent:muted),more=actionButton("⋯",muted);addControl(footer,primary,0,1);addControl(footer,snooze,6,1);addControl(footer,pin,6,1);LinearLayout.LayoutParams mp=new LinearLayout.LayoutParams(dp(48),dp(48));mp.setMargins(dp(6),0,0,0);footer.addView(more,mp);c.addView(footer);
        primary.setOnClickListener(v->{if(acts.isEmpty())FeatureStore.dismissAttention(db,e.item.id);else FeatureStore.markDone(db,e.item.id);Toast.makeText(this,acts.isEmpty()?"Cleared":"Action closed",Toast.LENGTH_SHORT).show();refresh();});snooze.setOnClickListener(v->snooze(e));pin.setOnClickListener(v->{FeatureStore.pin(db,e.item.id,!e.pinned);refresh();});more.setOnClickListener(v->moreMenu(e));LinearLayout.LayoutParams cp=new LinearLayout.LayoutParams(-1,-2);cp.setMargins(0,0,0,dp(12));feed.addView(c,cp);
    }

    void addReasonChips(LinearLayout parent,FeatureStore.InboxEntry e){if(empty(e.reason)&&!e.pinned)return;HorizontalScrollView hsv=new HorizontalScrollView(this);hsv.setHorizontalScrollBarEnabled(false);LinearLayout row=new LinearLayout(this);row.setOrientation(LinearLayout.HORIZONTAL);ArrayList<String> labels=new ArrayList<>();if(!empty(e.reason)){for(String x:e.reason.split("•")){String v=reasonLabel(x.trim());if(!empty(v)&&!labels.contains(v))labels.add(v);}}if(e.pinned&&!labels.contains("Pinned"))labels.add("Pinned");for(String x:labels){TextView ch=chip(x,chipColor(x),false);LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(-2,dp(28));p.setMargins(0,0,dp(6),0);row.addView(ch,p);}hsv.addView(row);LinearLayout.LayoutParams hp=new LinearLayout.LayoutParams(-1,dp(34));hp.setMargins(0,dp(9),0,0);parent.addView(hsv,hp);}
    ...
    void addActionRows(LinearLayout parent,ArrayList<String> acts,String due){TextView h=plain("OPEN ACTIONS",11,accent);h.setTypeface(null,1);h.setPadding(0,dp(13),0,dp(5));parent.addView(h);for(String a:acts){LinearLayout row=new LinearLayout(this);row.setOrientation(LinearLayout.HORIZONTAL);row.setGravity(Gravity.TOP);TextView box=plain("□",18,accent);box.setGravity(Gravity.TOP);row.addView(box,new LinearLayout.LayoutParams(dp(28),dp(34)));TextView body=tv(a,13,text);body.setPadding(dp(2),0,0,dp(6));row.addView(body,new LinearLayout.LayoutParams(0,-2,1));parent.addView(row);}if(!empty(due)){TextView d=chip("Due: "+due,warn,false);LinearLayout.LayoutParams p=new LinearLayout.LayoutParams(-2,dp(28));p.setMargins(0,dp(5),0,0);parent.addView(d,p);}}
    ...
    void moreMenu(FeatureStore.InboxEntry e){ArrayList<String> xs=new ArrayList<>();xs.add("Detail");xs.add("Move bucket");if("failed_retryable".equals(e.item.status)||"analysis_failed".equals(e.item.status))xs.add("Re-analyze");xs.add("Clear from attention");String[] arr=xs.toArray(new String[0]);new AlertDialog.Builder(this).setTitle(InboxPresentation.title(e.item,FeatureStore.openActions(db,e.item.id))).setItems(arr,(d,w)->{String x=arr[w];if("Detail".equals(x))detail(e);else if("Move bucket".equals(x))chooseBucket(e);else if("Re-analyze".equals(x))reanalyze(e.item);else{FeatureStore.dismissAttention(db,e.item.id);refresh();}}).setNegativeButton("Cancel",null).show();}
    void snooze(FeatureStore.InboxEntry e){String[] x={"3 hours","Tomorrow","3 days","7 days"};new AlertDialog.Builder(this).setTitle("Snooze • "+InboxPresentation.title(e.item,FeatureStore.openActions(db,e.item.id))).setItems(x,(d,w)->{long now=System.currentTimeMillis(),until;if(w==0)until=now+3L*60*60*1000;else if(w==1)until=now+24L*60*60*1000;else if(w==2)until=now+3L*24*60*60*1000;else until=now+7L*24*60*60*1000;FeatureStore.snooze(db,e.item.id,until);Toast.makeText(this,"Snoozed until "+fmt(until),Toast.LENGTH_LONG).show();refresh();}).setNegativeButton("Cancel",null).show();}
    ...
    void detail(FeatureStore.InboxEntry e){KnowledgeItem k=db.getById(e.item.id);if(k==null)return;ScrollView sv=new ScrollView(this);LinearLayout box=new LinearLayout(this);box.setOrientation(LinearLayout.VERTICAL);box.setPadding(dp(18),dp(8),dp(18),dp(18));sv.addView(box);addReasonChips(box,e);ArrayList<String> raw=FeatureStore.openActions(db,k.id),acts=InboxPresentation.actionParts(raw);if(!acts.isEmpty())addActionRows(box,acts,InboxPresentation.due(raw));label(box,"WHAT CORTEX UNDERSTOOD");box.addView(tv(empty(e.reason)?e.bucket:e.reason,13,text));if(!empty(k.summary)){label(box,"SUMMARY");box.addView(tv(k.summary,14,text));}if(!empty(k.extractedText)){label(box,"TRANSCRIPT");String x=k.extractedText;if(x.length()>6000)x=x.substring(0,6000)+"…";box.addView(tv(x,14,text));}label(box,"STATE");box.addView(plain("Bucket: "+e.bucket+"\nPriority: "+FeatureStore.priorityLabel(e.score)+"\nStatus: "+k.status+"\nCaptured: "+fmt(k.createdAt),12,muted));new AlertDialog.Builder(this).setTitle(InboxPresentation.title(k,raw)).setView(sv).setNegativeButton("Close",null).show();}
