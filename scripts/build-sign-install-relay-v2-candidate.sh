...
EXPECTED_VERSION_NAME="1.0.0-v51-relay-v2-candidate"
...
SIGNED_APK="${CORTEX_DEVICE_APK:-/sdcard/Download/Cortex-v51-relay-v2-candidate.apk}"
...
echo
echo "==> Build 3/6: unsigned Cortex Relay V2 bridge release"
...
rm -f "$SIGNED_APK"
# Passwords are intentionally not stored in this repository. apksigner will use any configured
# environment/provider or prompt locally in Termux when the keystore requires authentication.
"$APKSIGNER" sign \
  --ks "$KEYSTORE" \
...
TMP_REMOTE="/data/local/tmp/Cortex-v51-relay-v2-candidate.apk"
...
echo
echo "==> Verify 6/6: installed candidate + permanent signer"
...
echo "APK: $SIGNED_APK"
echo
echo "CORTEX_RELAY_V2_BRIDGE_CANDIDATE_UPDATE_SUCCESS"
