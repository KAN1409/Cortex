...
prepare_worktree(){
  ...
  if [ -f "$SIGNER_SOURCE" ] && [ -d "$dir/app" ]; then
    ...
    chmod 600 "$dir/app/cortex-debug.keystore" 2>/dev/null || true
...
ensure_build_env(){
  ...
  if command -v aapt2 >/dev/null 2>&1; then
    ...
    local prop="android.aapt2FromMavenOverride=$PREFIX/bin/aapt2"
...
run_gradle(){
  ...
  if [ -f "$dir/gradlew" ]; then
    ...
    echo "gradle_runner=wrapper"
    (cd "$dir" && ./gradlew --no-daemon --console=plain "$@")
  elif command -v gradle >/dev/null 2>&1; then
    echo "gradle_runner=termux"
    (cd "$dir" && gradle --no-daemon --console=plain "$@")
  else
...
run_capability(){
  local jobfile="$1" job cap ref pkg dir='' apk='' apk_rel='' wait_s rc crashes
  local -a tasks
...
public_log_tail(){
  ...
  case "$cap" in
    LOGCAT|BUILD_INSTALL_SMOKE)
      grep -E 'started_at=|finished_at=|agent_version=|BUILD_SUCCESS|candidate_signer_sha256=|installed_signer_sha256=|apk_sha256=|Success|pid=|mResumedActivity|topResumedActivity|AndroidRuntime|FATAL EXCEPTION|Process: com\.kareem\.|Caused by:|at com\.kareem\.|SMOKE_CRASH|APKSIGNER_|SIGNER_MISMATCH|APK_' "$log" 2>/dev/null | tail -n300 || true
      ;;
    DUMPSYS_PACKAGE)
