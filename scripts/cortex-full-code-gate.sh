...
rows.sort()
out=Path('audit/manifest.tsv')
out.write_text('path\tlines\tbytes\tkind\n' + ''.join(f'{p}\t{l}\t{b}\t{k}\n' for p,l,b,k in rows))
Path('audit/coverage-summary.txt').write_text(
    f'tracked_code_config_files={len(rows)}\n' +
    f'tracked_code_config_lines={sum(r[1] for r in rows)}\n' +
    f'tracked_code_config_bytes={sum(r[2] for r in rows)}\n'
)
...
for name in files:
    ...
    try:
        ...
        elif p.suffix == '.xml' and ('app/src/' in name):
            ET.parse(p)
...
if not p.exists():
    raise SystemExit('missing audit/semgrep.json')
...
for r in errors[:200]:
    s=r.get('start',{})
    print(f"ERROR {r.get('path')}:{s.get('line')} {r.get('check_id')} {r.get('extra',{}).get('message','')}")
raise SystemExit(1 if errors else 0)
PY
else
  echo 'semgrep missing' >&2
  record semgrep_scan MISSING
  FAIL=$((FAIL+1))
fi
...
if command -v gradle >/dev/null 2>&1; then
  ...
  echo 'gradle missing' >&2
  record gradle MISSING
  FAIL=$((FAIL+1))
fi
