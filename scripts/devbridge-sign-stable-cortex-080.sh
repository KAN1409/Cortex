...
OUT="$OUTDIR/Cortex-0.8.0-situations-sources-permanent.apk"
DOWNLOAD="/sdcard/Download/Cortex-0.8.0-situations-sources-permanent.apk"

[ -s "$UNSIGNED" ] || { echo CORTEX_080_UNSIGNED_APK_MISSING >&2; exit 2; }
[ -s "$KEYSTORE" ] || { echo CORTEX_080_SIGNER_OVERLAY_MISSING >&2; exit 3; }
...
[ -n "$APKSIGNER" ] && [ -x "$APKSIGNER" ] || { echo CORTEX_080_APKSIGNER_MISSING >&2; exit 4; }
...
[ "$KEY_CERT" = "$EXPECTED_CERT" ] || { echo "CORTEX_080_SIGNER_SOURCE_MISMATCH:$KEY_CERT" >&2; exit 5; }
...
"$APKSIGNER" verify --verbose --print-certs --min-sdk-version 26 "$OUT" >/tmp/cortex080-apksigner.txt
OUT_CERT="$(sed -n -E 's/^.*certificate SHA-256 digest:[[:space:]]*//p' /tmp/cortex080-apksigner.txt | head -n1 | tr -d ':[:space:]' | tr '[:upper:]' '[:lower:]')"
rm -f /tmp/cortex080-apksigner.txt
[ "$OUT_CERT" = "$EXPECTED_CERT" ] || { rm -f "$OUT"; echo "CORTEX_080_OUTPUT_SIGNER_MISMATCH:$OUT_CERT" >&2; exit 6; }
...
echo CORTEX_080_PERMANENT_APK_READY
