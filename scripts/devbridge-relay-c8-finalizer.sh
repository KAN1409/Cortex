...
FLAG_PATH='.devbridge/relay-candidate8-finalize.json'
...
publish_result(){
  ...
  jq -n \
    ...
    --arg agent_version '3+relay-c8-no-launch-finalizer' \
    --arg job_id "$job" \
    --arg capability 'RELAY_CANDIDATE8_SIGN_INSTALL_NO_LAUNCH' \
    --arg status "$status" \
...
main(){
  ...
  local flag="$ROOT/relay-candidate8-finalize.json"
  git -C "$CONTROL" show "origin/$CONTROL_BRANCH:$FLAG_PATH" > "$flag" 2>/dev/null || return 0

  local job artifact_url artifact_zip_sha unsigned_sha lock log signer ks_pass key_pass key_cert
  local zip unsigned signed signed_cert installed_path installed_probe installed_cert verify staged install_out pkg_info rc=0 artifact_zip actual

  job="$(jq -r '.job_id // empty' "$flag")"
  [ "$job" = 'job_relay_candidate8_install_20260830_1918' ] || return 0
  [ "$(jq -r '.authorized_owner // empty' "$flag")" = 'KAN1409' ] || return 0
  [ "$(jq -r '.package // empty' "$flag")" = "$PKG" ] || return 0
  [ "$(jq -r '.version_code // empty' "$flag")" = "$EXPECTED_VERSION_CODE" ] || return 0
  [ "$(jq -r '.version_name // empty' "$flag")" = "$EXPECTED_VERSION_NAME" ] || return 0
  [ "$(jq -r '.expected_cert_sha256 // empty' "$flag" | tr '[:upper:]' '[:lower:]')" = "$EXPECTED_CERT" ] || return 0
  ...
  echo 'agent_version=3+relay-c8-no-launch-finalizer'
  ...
  zip="$WORK/candidate8-unsigned.zip"
  ...
  if [ $rc -eq 0 ]; then
    installed_probe='/sdcard/Download/.cortex-relay-c8-installed-probe.apk'
    rish -c "cp '$installed_path' '$installed_probe'; chmod 644 '$installed_probe'" >/dev/null 2>&1 || { echo INSTALLED_APK_COPY_FAILED; rc=103; }
    cp -f "$installed_probe" "$WORK/installed-base.apk" 2>/dev/null || { echo INSTALLED_PROBE_IMPORT_FAILED; rc=104; }
    rish -c "rm -f '$installed_probe'" >/dev/null 2>&1 || true
  fi
  if [ $rc -eq 0 ]; then
    installed_cert="$(apk_cert "$signer" "$WORK/installed-base.apk")"
    echo "installed_signer_sha256=$installed_cert"
    [ "$installed_cert" = "$EXPECTED_CERT" ] || { echo INSTALLED_SIGNER_MISMATCH; rc=105; }
    [ "$installed_cert" = "$signed_cert" ] || { echo SIGNER_CONTINUITY_MISMATCH; rc=106; }
  fi

  if [ $rc -eq 0 ]; then
    staged='/data/local/tmp/Cortex-Relay-v2.0.0-candidate8-permanent.apk'
    cat "$signed" | rish -c "cat > '$staged'; chmod 644 '$staged'" || { echo APK_STAGE_FAILED; rc=107; }
  fi
  if [ $rc -eq 0 ]; then
    install_out="$(rish -c "pm install -r '$staged'; install_rc=\$?; rm -f '$staged'; exit \$install_rc" 2>&1)"
    printf '%s\n' "$install_out"
    printf '%s\n' "$install_out" | grep -q 'Success' || { echo UPDATE_INSTALL_FAILED; rc=108; }
  fi
  if [ $rc -eq 0 ]; then
    pkg_info="$(rish -c "dumpsys package '$PKG' | grep -E 'versionCode=|versionName=' | head -n4" 2>/dev/null | tr -d '\r')"
    printf '%s\n' "$pkg_info"
    printf '%s\n' "$pkg_info" | grep -Fq "versionCode=$EXPECTED_VERSION_CODE" || { echo INSTALLED_VERSION_CODE_FAIL; rc=109; }
    [ $rc -ne 0 ] || printf '%s\n' "$pkg_info" | grep -Fq "versionName=$EXPECTED_VERSION_NAME" || { echo INSTALLED_VERSION_NAME_FAIL; rc=110; }
  fi
  if [ $rc -eq 0 ]; then
    cp -f "$signed" '/sdcard/Download/Cortex-Relay-v2.0.0-candidate8-permanent.apk' || { echo DOWNLOAD_COPY_FAILED; rc=111; }
    make_zip "$signed" "$artifact_zip" || { echo ARTIFACT_ZIP_CREATE_FAILED; rc=112; }
    echo "return_artifact_zip_sha256=$(sha256sum "$artifact_zip" | awk '{print $1}')"
    echo "return_artifact_zip_bytes=$(wc -c < "$artifact_zip" | tr -d ' ')"
    echo 'foreground_launch_executed=false'
    echo CORTEX_RELAY_CANDIDATE8_BACKGROUND_UPDATE_SUCCESS
  fi

  rm -f "$ks_pass" "$key_pass" "$WORK/installed-base.apk"
  echo "finished_at=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  exec 1>&9 9>&-

  if [ $rc -eq 0 ]; then
    publish_result "$job" SUCCESS 0 "$log" "$artifact_zip" || return 120
    return 0
  fi
