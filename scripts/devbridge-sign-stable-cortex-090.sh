...
OUT="$OUTDIR/Cortex-0.9.0-chatgpt-bridge-permanent.apk"
DOWNLOAD="/sdcard/Download/Cortex-0.9.0-chatgpt-bridge-permanent.apk"

[ -s "$UNSIGNED" ] || { echo CORTEX_090_UNSIGNED_APK_MISSING >&2; exit 2; }
[ -s "$KEYSTORE" ] || { echo CORTEX_090_SIGNER_OVERLAY_MISSING >&2; exit 3; }
...
[ -n "$APKSIGNER" ] && [ -x "$APKSIGNER" ] || { echo CORTEX_090_APKSIGNER_MISSING >&2; exit 4; }
...
[ "$KEY_CERT" = "$EXPECTED_CERT" ] || { echo "CORTEX_090_SIGNER_SOURCE_MISMATCH:$KEY_CERT" >&2; exit 5; }
...
VERIFY_FILE="$OUTDIR/.cortex090-apksigner.$$"
...
[ "$OUT_CERT" = "$EXPECTED_CERT" ] || { rm -f "$OUT"; echo "CORTEX_090_OUTPUT_SIGNER_MISMATCH:$OUT_CERT" >&2; exit 6; }
...
echo CORTEX_090_PERMANENT_APK_READY
